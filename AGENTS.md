...
- **Current interchange pin: `5a73d3cc`** (approval-suspension runtime, 2026-07-18, CL-3932) — bumped from `6927e7e4`. This was an ADDITIVE pass-through bump: it wires upstream's control-plane suspension seam (`park.notify` → supervisor `onSuspensionRegister` → hub-link `signal.correlation.register`) through the vendored/forked sidecar wiring, but the workbench does NOT adopt the approval-suspension feature anywhere — it passes through inert. No WORKBENCH-LOCAL block was retired (130 before, 130 after). See `docs/VENDORED.md` → "Supersession audit — pin `5a73d3cc`". The `6927e7e4` references below are HISTORICAL (they describe the prior runtime-retirement bump) and are intentionally not rewritten.
