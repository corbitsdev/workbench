...
- **Current interchange pin: `5a73d3cc`** (approval-suspension runtime, 2026-07-18, CL-3932). The pin SHA is duplicated in all three Dockerfiles — bump them together (above).
- **Vendored / forked interchange code has a canonical home: [`docs/VENDORED.md`](docs/VENDORED.md).** The sidecar copies several files from interchange's reference `apps/sidecar` (`workflow-host-wiring.ts`, `workflow-substrate-factory.ts`, `workflow-run-pack-client.ts`, `bin/workflow-child`) and vendors whole packages (`packages/workflow-host`, `packages/storage-isogit`, `packages/hub-agent`, `packages/inference`, `packages/hub-sessions`), each carrying `// WORKBENCH-LOCAL (CL-XXXX)` blocks that must survive every re-sync. **Before touching any of these, or bumping the interchange pin, read `docs/VENDORED.md`** — it holds the full catalogue, the per-file WORKBENCH-LOCAL inventory, the supersession audits, the duck-typed seams to re-verify (these will not surface at compile time), and the step-by-step pin-bump procedure.
  - Fast audit of every divergence in one pass: `git grep 'WORKBENCH-LOCAL' apps/sidecar/ packages/workflow-host/ packages/inference/ packages/storage-isogit/ packages/hub-agent/ packages/hub-sessions/`.
  - Drift check after re-applying locals onto a new upstream: `scripts/check-vendored-drift.sh origin/staging`.
