...
- **Vendored workflow-host wiring (pin-bump gate)** — `apps/sidecar/src/workflow-host-wiring.ts`, `workflow-substrate-factory.ts`, `workflow-run-pack-client.ts`, `workflow-deployment-record.ts`, `atomic-write.ts`, and `bin/workflow-child` are copied (or, for the two newest files, newly written) from interchange's reference `apps/sidecar` (the `createSidecarDeployRouter` supervisor wiring is not packaged in any `@intx/*` — `@intx/workflow-host` ships only the building blocks). They duck-type the `@intx/workflow-host` hand-off, so a contract change won't surface at compile time. As of the `6927e7e4` runtime-retirement pin bump (2026-07-16), single-agent instances (Myra/Oat/triage/gate agents) deploy as **single-step workflow deployments** through this same path — upstream deleted `launchSession` and the in-process per-instance harness runtime with it, so there is no separate launch mechanism left to keep in sync.
  ...
  - **`workflow-deployment-record.ts` + `atomic-write.ts` are NEW at this bump** — sidecar-local persistence for re-establishing a deployment across a sidecar process restart, with no upstream counterpart. `workflow-deployment-record.ts` carries CL-2199 (durable tenant scope + raw deploymentId) and the new **CL-3368 resurrection-guard tombstone**: an undeploy now writes a durable tombstone before its directory is reclaimed, and boot-time restore skips re-spawning a tombstoned deployment — closing the gap where a missed teardown ack plus a sidecar-only restart (not a full redeploy) could otherwise resurrect a terminated deployment forever.
  - **`workflow-substrate-factory.ts` is now an acknowledged FORK, not a verbatim-vendor-with-blocks** (same discipline as `packages/hub-agent`). Upstream `6927e7e4` replaced hub-RPC step-tool resolution with on-disk deploy-tree materialization; the workbench deliberately keeps its existing hub-RPC model (per-tenant registry tarballs, hub-backed `RuntimeCapabilities`, the `/api/internal/tools/manifest` rail) rather than adopt it mid-bump, so this file's re-sync process is now "merge upstream structure onto the fork's tool-resolution branch," not "diff and re-apply a fixed block set." Its existing WORKBENCH-LOCAL tags (CL-2199, CL-2401, CL-2650, CL-3379) are unchanged. See `docs/VENDORED.md` for the full reclassification rationale and the follow-up to evaluate upstream's materialization approach as its own project.
  ...
  - **Audit is mechanical: `git grep 'WORKBENCH-LOCAL' apps/sidecar/ packages/workflow-host/ packages/inference/ packages/storage-isogit/ packages/hub-agent/'`** lists every divergence in one pass (each block is fronted by a `// WORKBENCH-LOCAL (CL-XXXX)` token). Run it before and after a re-sync and confirm no expected block disappeared — but at a runtime-retirement-class bump, blocks tied to the retired runtime (CL-2535, CL-2537, CL-3102, CL-3103, CL-2400, CL-3415, CL-3796 at this bump) are expected to disappear; cross-check disappearances against `docs/VENDORED.md`'s "RETIRED" notes rather than treating every drop as a regression. **`packages/workflow-host` is a full vendor of `@intx/workflow-host`** (the sidecar imports it instead of upstream) and must be re-synced against the new upstream on every pin bump — its `recoverParkedRun` hook (CL-2535) is gone as of `6927e7e4` because upstream's own resume guard now covers the case. **`packages/storage-isogit` is a 100% verbatim vendor of `@intx/storage-isogit`** (re-copy cleanly on every bump; on-disk pack/repo format must stay parity with what the hub consumes) and **`packages/hub-agent` is an untagged long-lived FORK** (never literally re-copy — see its VENDORED.md entry; its CL-3340 assistant-loop-guard is preserved as a file but currently unwired pending a re-home onto the sidecar's inference-event path, since the session-manager seam it used to wire onto is gone). The full catalogue of vendored changes (files + package) and when each was added lives in [`docs/VENDORED.md`](docs/VENDORED.md).
