...
[`packages/connections/src/catalog-seed-data.ts`](packages/connections/src/catalog-seed-data.ts)
...
| `smoke-onboarding.test.ts` | A brand-new signup starts benchless and mints its first tenant through an ordinary `POST /api/tenants`                |
