...
import { createSidecarDeployRouter, deriveDeploymentId } from "./workflow-host-wiring";
...
function isRegistered(transport: ReturnType<typeof createInMemoryTransport>): boolean {
  try {
...
describe("agent signing-key registration lifecycle on the host transport", () => {
  test("single-step deploy registers the agent crypto; undeploy unregisters it", async () => {
    ...
    const tempBase = await fs.mkdtemp(path.join(os.tmpdir(), "sidecar-keylifecycle-"));
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "sidecar-keylifecycle-data-"));
    ...
    const recordFile = path.join(dataDir, "workflow-runs", anchorRunId, "deployment.json");
    const parsedRecord = WorkflowRunRecord(JSON.parse(await fs.readFile(recordFile, "utf8")));
