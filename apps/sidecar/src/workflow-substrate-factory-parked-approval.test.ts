...
const testSigner = (payload: string): Promise<string> => Promise.resolve(`sig:${payload.length}`);
...
describe("readColdParkedApprovalSnapshot", () => {
  test("loads the snapshot from a parked step's on-disk store", async () => {
    ...
    const store = await createIsogitStore(stepStorageRoot(coordinate), testSigner);
...
function createStubSubstrate(baseDir: string): RepoStore {
  const repoDirFor = (repoId: RepoId): string => path.join(baseDir, repoId.kind, repoId.id);
  ...
  return new Proxy(stub as RepoStore, {
    get(target, prop, receiver) {
      ...
      return () => {
        throw new Error(`stub substrate: ${String(prop)} not implemented for this test`);
      };
    },
  });
}
...
describe("toParkedApprovalOps", () => {
  test("projects approval ops to correlationId plus the epoch-ms deadline", async () => {
    ...
    const got = toParkedApprovalOps([pendingApproval("corr-1", snapshot), withDeadline]);
