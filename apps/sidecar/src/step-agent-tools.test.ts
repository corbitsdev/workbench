...
afterEach(async () => {
  await Promise.all(
    tempDirs.splice(0).map((d) => fs.promises.rm(d, { recursive: true, force: true })),
  );
});

async function tempDir(): Promise<string> {
  const d = await fs.promises.mkdtemp(path.join(os.tmpdir(), "step-agent-tools-test-"));
  tempDirs.push(d);
  return d;
}
...
describe("createToolBearingAgentFactory plugin/LSP lifecycle", () => {
  ...
  test("a plugin factory that throws mid-chain disposes the already-built plugins", async () => {
    ...
    await expect(agentFactory(def, { ...env })).rejects.toThrow(/plugin construction failure/);
...
describe("deriveToolMarkFloorGrants", () => {
  ...
  function loadedFactory(id: string, definitions: readonly ToolDeclaration[]): LoadedToolFactory {
    const factory: AnnotatedToolFactory<BaseEnv> = Object.assign(
      (_env: BaseEnv) => ({
        definitions: [],
        run: () => Promise.resolve({ callId: "", content: "", isError: false as const }),
      }),
  ...
  test("a pinned ask-marked tool resolves to ask on its floor alone", async () => {
    const factories = [
      loadedFactory("@intx/tools-posix/sidecar-bundle", [{ name: "run_shell", approval: "ask" }]),
    ];
  ...
  test("a pinned unmarked tool resolves to allow on its floor alone", async () => {
    const factories = [loadedFactory("@intx/tools-mail/sidecar-bundle", [{ name: "mail_send" }])];
  ...
  test("a declared deny still beats the derived floor", async () => {
    ...
    expect(await resolveWithFloor("run_shell", factories, [declaredDeny])).toBe("deny");
  });
  ...
  test("an allow grant at equal specificity does not drop the ask floor", async () => {
    ...
    expect(await resolveWithFloor("run_shell", factories, [declaredAllow])).toBe("ask");
  });
  ...
  test("a broader allow glob loses to the exact ask floor on specificity", async () => {
    ...
    expect(await resolveWithFloor("run_shell", factories, [broadAllow])).toBe("ask");
  });
});
