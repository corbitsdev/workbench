...
import type { ChildOutboundMailBridge, SourcesSnapshotRef, StepEnvBase } from "@intx/workflow-host";
...
function sourcesRefFor(chain: InferenceSource[] = [SOURCE]): SourcesSnapshotRef {
...
describe("createSidecarStepBuildEnv per-step scratch keying", () => {
  test("warm path keys one stable workdir across runs and preserves files", async () => {
    ...
    const env1: StepEnvBase = await buildEnv(requestForRun("run-1"), sourcesRef);
    ...
    const env2: StepEnvBase = await buildEnv(requestForRun("run-2"), sourcesRef);
    ...
    expect(await fs.readFile(path.join(env2.workdir, "turn-1.txt"), "utf8")).toBe("carried");
  });
  ...
  test("cold path keys a distinct per-run workdir under that run's subtree", async () => {
    ...
    expect(env1.workdir).toContain(
      path.join("workflow-step-state", WORKFLOW_RUN_REPO_ID.id, "runs", "run-1"),
    );
  ...
  test("feeds the reactor the whole failover chain with the head pinned as default", async () => {
    ...
    const env: StepEnvBase = await buildEnv(requestForRun("run-1"), sourcesRefFor(chain));
  ...
  test("resolves sources from the mutable ref at build time", async () => {
    ...
    const before: StepEnvBase = await buildEnv(requestForRun("run-1"), sourcesRef);
    ...
    const after: StepEnvBase = await buildEnv(requestForRun("run-2"), sourcesRef);
...
function approvalOp(
  ...
  return {
    ...
    ...(opts.suspendedCall !== undefined ? { suspendedCall: opts.suspendedCall } : {}),
  };
}
...
function approvalResumeRequest(runId: string, correlationId: string): StepInvokeRequest {
  return {
...
describe("createSidecarStepBuildEnv cold-path approval-resume keying", () => {
  ...
  test("rejects a resume whose correlationId has no pending op at all", async () => {
    ...
    await seedColdStore(dataDir, "run-1", [approvalOp("other", { suspendedCall: SUSPENDED_CALL })]);
    const buildEnv = createSidecarStepBuildEnv(buildDeps({ dataDir }));
