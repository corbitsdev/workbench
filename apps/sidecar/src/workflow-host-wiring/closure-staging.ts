 ...
 */
export function deploymentSourceAssetRoot(dataDir: string, deploymentId: string): string {
 ...
 */
export function deploymentSourceGitRoot(dataDir: string, deploymentId: string): string {
 ...
 */
export function deploymentClosureInstanceDir(dataDir: string, deploymentId: string): string {
 ...
 */
export function requireSubstrateByteCap(env: Record<string, string>, key: string): number {
  const raw = env[key];
  if (raw === undefined) {
    throw new Error(
...
export async function materializeDeploymentClosure(args: {
  ...
  const instanceDir = deploymentClosureInstanceDir(args.dataDir, args.deploymentId);
  ...
  return applyFrozenWorkflowClosure({
    ...
    cacheMaxBytes: requireSubstrateByteCap(args.substrateEnv, "SIDECAR_CACHE_MAX_BYTES"),
...
function requireSubstrateEntry(env: Record<string, string>, key: string): string {
  const raw = env[key];
  if (raw === undefined) {
    throw new Error(
      `sidecar deploy router: ${key} must be present in the multi-step substrate env to materialize a frozen workflow closure`,
