...
import { createEd25519Crypto, createNoopCredentialCipher, generateKeyPair } from "@intx/crypto";
...
describe("createSidecarWorkflowSupervisor", () => {
  ...
  test("routeInbound rejects when no subscriber is registered so undelivered mail is withheld", async () => {
    ...
    await expect(wired.routeInbound(new TextEncoder().encode("hello"))).rejects.toThrow(
      /no active mail subscriber/,
    );
  });
  ...
  test("onRunStart fails a poisoned run and passes an unpoisoned one", async () => {
    ...
    const grantsDir = path.join(tempBase, "workflow-run", anchorRunId, "runs", cleanRunId);
    await fs.mkdir(grantsDir, { recursive: true });
    const runGrants = [{ id: "g1", resource: "tool:send-mail", effect: "allow" }];
    await fs.writeFile(path.join(grantsDir, "grants.json"), JSON.stringify({ grants: runGrants }));
    ...
    const wired = createSidecarWorkflowSupervisor({
      ...
      deriveStepAddress: ({ runId: dep, stepId }) => `${dep}-${stepId}@example.com`,
      substrateEnv: {},
      dynamicSpawnEnv: () => ({}),
      subprocessSpawner: () => {
        throw new Error("spawner not invoked in this test");
    ...
    await expect(wired.onRunStart({ runId: "run-poisoned", anchorRunId })).rejects.toThrow(
      /grants write failed/,
    );
...
describe("createSidecarDeployRouter provision-step (no-spawn) mode", () => {
  test("a provisionStep frame inits the repo and records the hub key without spawning", async () => {
    ...
    expect(recordHubKeyCalls).toEqual([{ address: STEP_ADDR, hubKey: HUB_KEY }]);
...
describe("createSidecarDeployRouter multi-step branch", () => {
  async function buildMultistepFixture(opts: {
     ...
     */
    senderKeyCache?: Parameters<typeof createSidecarDeployRouter>[0]["senderKeyCache"];
    multistepSourcesRouter?: MultistepSourcesRouter;
    registerDeployment?: (args: { runId: string; agentAddress: string }) => void;
    /**
     ...
     */
    unregisterDeployment?: (args: { runId: string; agentAddress: string }) => void;
  ...
  test("a second same-address deploy is rejected mid-spawn and never deletes the live run record", async () => {
    ...
    const recordFile = path.join(multiDataDir, "workflow-runs", anchorRunId, "deployment.json");
  ...
  test("caches each co-delivered sender key before the run's grants land", async () => {
    ...
    const { grantsRouter, agentAddress, anchorRunId, tempBase } = await deployMultistepForGrants(
      "wf-sender-key-order",
      senderKeyCache,
    );
    ...
    expect(routed).toBe(true);
    expect(puts).toEqual([{ address: "sender@peer.example", grantsExisted: false }]);
  ...
  test("does not register a multistepMailRouter handler if spawn rejects", async () => {
    ...
    expect(mailRouter.tryRoute(frame.agentAddress, new Uint8Array([1]))).toBeNull();
  });
  ...
  test("a soft-failed deploy (spawn rejects) leaves no restore record", async () => {
    ...
    if (dataDir === undefined) throw new Error("fixture SIDECAR_DATA_DIR unset");
  ...
  test("rejects a deploy whose step pins an unbuildable provider before spawning", async () => {
    ...
    await expect(router.deploy(frame)).rejects.toThrow(/ghost-provider.*not registered/);
  ...
  test("refuses to deploy a workflow frame carrying no approvedWireHash rather than recomputing it", async () => {
    ...
    await expect(router.deploy(frame)).rejects.toThrow(/must carry approvedWireHash/);
    expect(spawnerInvoked).toBe(false);
  });
  ...
  function makeReadyDrivingSpawner(pidBase: number) {
    ...
    return {
      ...
      envFor: (index: number): Record<string, string> | undefined => spawns[index]?.env,
      async recycleRequestFor(index: number): Promise<void> {
        const sender = spawns[index]?.childSender;
        if (sender === undefined) {
          throw new Error(`spawn ${String(index)} has no sender; drive its ready first`);
        }
        await sender.send({
          type: "recycle.request",
          data: { reason: "sources-rotation-recycle" },
  ...
  function recordExists(dataDir: string, anchorRunId: string): Promise<boolean> {
    return fs.access(path.join(dataDir, "workflow-runs", anchorRunId, "deployment.json")).then(
      () => true,
      () => false,
    );
  }

  function singleStepFrame(agentAddress: string, definitionId: string): AgentDeployFrame {
  ...
  test("restore rejects a closure-derived definition whose stepOrder names a step with no matching entry", async () => {
    ...
    await writeWorkflowRunRecord(dataDir, anchorRunId, record, createNoopCredentialCipher());
  ...
  test("restore soft-skips a malformed source-ref record (missing sourceRef) at the scan boundary", async () => {
    ...
    const recordPath = path.join(dataDir, "workflow-runs", deploymentId, "deployment.json");
  ...
  test("restore re-materializes a source-ref deployment's closure and re-spawns it as source-ref", async () => {
    ...
    await writeWorkflowRunRecord(dataDir, deploymentId, record, createNoopCredentialCipher());
    ...
    const workflowJsonPath = path.join(dataDir, "assets", "workflow", "wf-srcref", "workflow.json");
    ...
    > = (args) => {
      if (args.source.kind !== "registry") {
        throw new Error(`applyStub expected a registry source, got ${args.source.kind}`);
      }
      applyCalls.push({
  ...
  test("a second deploy for a live address is rejected without orphaning its restore record", async () => {
    ...
    await expect(router.deploy(singleStepFrame(head, "wf-dup"))).rejects.toThrow(
      /already deployed/,
    );
    expect(spawner.spawnCount()).toBe(1);
    expect(await recordExists(dataDir, anchorRunId)).toBe(true);
  ...
  test("a reclaimed self-terminated address survives a following operator undeploy", async () => {
    ...
    const deployPromise = router.deploy(singleStepFrame(head, "wf-st-undeploy"));
    await spawner.driveReadyFor(0);
    await deployPromise;
  ...
  test("restore skips a record whose address does not derive its directory name", async () => {
    ...
    await writeWorkflowRunRecord(dataDir, wrongDir, record, createNoopCredentialCipher());
  ...
  test("restore soft-fails and keeps the record when the pinned source is no longer buildable", async () => {
    const dataDir = await createTempBaseDir("sidecar-restore-unbuildable-data-");
    ...
    const deployPromise = routerA.deploy(singleStepFrame(head, "wf-unbuildable-restore"));
    await first.driveReadyFor(0);
    await deployPromise;
  ...
  test("restore isolates an unbuildable-provider record: it keeps the record and surfaces the failure while the healthy deployment still restores", async () => {
    const dataDir = await createTempBaseDir("sidecar-restore-unbuildable-isolate-data-");
    ...
    function unbuildableSingleStepFrame(): AgentDeployFrame {
      return makeMultistepFrame({
        ...
        sources: {
          "step-1": [{ ...makeInferenceSource("step-1"), provider: unbuildableProvider }],
        },
      });
    }
    ...
    const deployHealthy = routerA.deploy(singleStepFrame(healthyHead, "wf-healthy-isolate"));
    await first.driveReadyFor(0);
    await deployHealthy;
  ...
  test("a deploy whose child never signals ready times out and rejects", async () => {
    ...
    await expect(router.deploy(singleStepFrame(head, "wf-readytimeout"))).rejects.toThrow(
      /did not emit ready within 40ms/,
    );
  ...
  test("a single-step deploy acks the agent key, not the supervisor key", async () => {
    ...
    const deployPromise = router.deploy(singleStepFrame("run_bkey@example.com", "wf-bkey"));
    await spawner.driveReadyFor(0);
    const result = await deployPromise;
    ...
    expect(result.publicKey).toBe(Buffer.from(headKeyPair.publicKey).toString("hex"));
    expect(result.publicKey).not.toBe(Buffer.from(fixtureKeyPair.publicKey).toString("hex"));
  });
  ...
  test("a source rotation survives a recycle respawn", async () => {
    ...
    expect(JSON.parse(initialEnv[STEP_INFERENCE_SOURCES_ENV_KEY] ?? "null")).toEqual({
      "step-1": [makeInferenceSource("step-1")],
    });
    ...
    expect(JSON.parse(respawnEnv[STEP_INFERENCE_SOURCES_ENV_KEY] ?? "null")).toEqual({
      "step-1": [rotated],
    });
  });
  ...
  test("a source rotation survives a full sidecar restart", async () => {
    ...
    const deployPromise = routerA.deploy(singleStepFrame(addr, "wf-rotrestart"));
    await first.driveReadyFor(0);
    await deployPromise;
    ...
    expect(JSON.parse(restoredEnv[STEP_INFERENCE_SOURCES_ENV_KEY] ?? "null")).toEqual({
      "step-1": [rotated],
    });
  });
  ...
  test("a recycle interleaving the rotation persist respawns on the rotated sources", async () => {
    ...
    const persist: typeof writeWorkflowRunRecord = async (d, id, rec, cipher) => {
    ...
    release();
    await rotatePromise;
    const scanned = await scanWorkflowRunRecords(dataDir, createNoopCredentialCipher());
  ...
  test("a recycle interleaving a failed rotation persist respawns on new sources then rolls back", async () => {
    ...
    const deployPromise = router.deploy(singleStepFrame(addr, "wf-rotinterfail"));
    await spawner.driveReadyFor(0);
    await deployPromise;
    ...
    expect(JSON.parse(secondRespawnEnv[STEP_INFERENCE_SOURCES_ENV_KEY] ?? "null")).toEqual({
      "step-1": [makeInferenceSource("step-1")],
    });
  });
  ...
  test("two addresses whose deriveDeploymentId slugs collide are rejected at the second deploy", async () => {
    ...
    const deployPromise = router.deploy(singleStepFrame("run_col.a@example.com", "wf-collide"));
    await spawner.driveReadyFor(0);
    await deployPromise;
  ...
  test("reEmitParkedCorrelations reaches the live supervisor and sends a parked-correlations.request downstream", async () => {
    ...
    function parkedRequestCount(): number {
      return supervisorToChild
        .flushed()
        .filter((line) => downstreamPayloadType(line) === PARKED_REQUEST_TYPE).length;
    }
...
describe("assembleRunCredentialsSnapshot", () => {
  ...
  const deriveStepAddress = ({ runId: dep, stepId }: { runId: string; stepId: string }) =>
    `${dep}-${stepId}@example.com`;

  async function writeRunGrantsFile(tempBase: string, contents: string): Promise<void> {
  ...
  async function writeDeployTimeStepGrants(
    ...
    const dir = path.join(tempBase, "agent-state", `${anchorRunId}-${stepId}`, "state");
    await fs.mkdir(dir, { recursive: true });
    await fs.writeFile(path.join(dir, "grants.json"), contents);
  }
  ...
  test("a run with a per-run grants file reads those grants over deploy-time", async () => {
    ...
    const runGrants = [{ id: "run-grant", resource: "tool:send-mail", effect: "allow" }];
    ...
    expect(snapshot.steps[0]?.contentHash).toBe(snapshot.steps[1]?.contentHash ?? "");
  });
  ...
  test("a re-dispatched run re-reads the durable per-run grants, not the deploy-time fallback", async () => {
    ...
    expect(second.steps.map((s) => s.contentHash)).toEqual(first.steps.map((s) => s.contentHash));
  });
});
