...
beforeEach(async () => {
  scratchRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sidecar-wf-probe-materialize-"));
...
async function packFixture(
  ...
  await fs.writeFile(path.join(packageDir, "package.json"), JSON.stringify(pkgJson));
  ...
  const proc = Bun.spawnSync(["tar", "-czf", tarballPath, "-C", stagingDir, "package"]);
  if (!proc.success) {
    throw new Error(`tar failed to pack the fixture: ${new TextDecoder().decode(proc.stderr)}`);
  }
...
function probeFrame(closure: ToolPackageManifest, entry: string): WorkflowProbeRequestFrame {
  return {
...
describe("createWorkflowClosureMaterializer", () => {
  test("lays out the frozen closure, resolves node_modules, and imports no author code", async () => {
    ...
    const closure: ToolPackageManifest = {
      schemaVersion: "1",
      topLevel: [{ name: WORKFLOW_PACKAGE_NAME, version: WORKFLOW_PACKAGE_VERSION }],
      entries: [
        {
          name: WORKFLOW_PACKAGE_NAME,
          version: WORKFLOW_PACKAGE_VERSION,
    ...
    try {
      const materialize = createWorkflowClosureMaterializer(materializerConfig(fetchTarball));
      ...
      const pkgJson = JSON.parse(
        await fs.readFile(path.join(materialized.packageDir, "package.json"), "utf8"),
      );
      ...
      const depPkgJson = JSON.parse(
        await fs.readFile(
          path.join(materialized.packageDir, "node_modules", "@fixture", "dep", "package.json"),
          "utf8",
        ),
      );
  ...
  test("fails loud when the closure pins no top-level package", async () => {
    ...
    await expect(materialize(probeFrame(closure, WORKFLOW_ENTRY))).rejects.toThrow(
  ...
  test("fails loud when the frame entry disagrees with interchange.workflow", async () => {
    ...
    const materialize = createWorkflowClosureMaterializer(materializerConfig(fetchTarball));
    await expect(materialize(probeFrame(closure, "./does-not-match.js"))).rejects.toThrow(
