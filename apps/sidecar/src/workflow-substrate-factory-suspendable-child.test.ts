...
function suspendThenComplete(record: { resumeDecision?: unknown }): SidecarChildStepInvoker {
  return async (req) => {
...
describe("createSidecarSpawnSuspendableChild", () => {
  test("surfaces a body approval park, resumes on the grant, and completes", async () => {
    ...
    await seedRunGrants(substrate, parentRunId, [grant(`tool:${BODY_STEP_AGENT_ID}`, "invoke")]);
  ...
  test("threads depth so a body's childWorkflow grandchild trips the ceiling", async () => {
    ...
    const parentRunId = "run-parent";
    await seedRunGrants(substrate, parentRunId, [grant("inference.source:anthropic:m", "invoke")]);
    ...
    const bodyEvents: readonly WorkflowEvent[] = await reader(substrate).read(childRunId);
