...
import type { AuthorizeFn, RepoId, WorkflowRunWorkflowProcessPrincipal } from "@intx/hub-sessions";
import {
...
import { createSidecarRunChild, type SidecarChildStepInvoker } from "./workflow-substrate-factory";
...
type SidecarRunChildDepsEvaluator = Parameters<typeof createSidecarRunChild>[0]["evaluateGrants"];
...
function recordingInvoker(
  ...
  return async (req, authorize, _sourcesRef, _onEvent): Promise<StepInvokeResult> => {
...
describe("createSidecarRunChild grant capping", () => {
  ...
  test("an existing child grants file is read back, not recomputed (write-once)", async () => {
    ...
    await seedRunGrants(substrate, parentRunId, [grant(DECLARED_RESOURCE, "invoke")]);
    ...
    const runChild = makeRunChild(substrate, recordingInvoker(record, [DECLARED_RESOURCE]));
    ...
    expect(record.decisions).toEqual([{ resource: DECLARED_RESOURCE, effect: null }]);
  });
