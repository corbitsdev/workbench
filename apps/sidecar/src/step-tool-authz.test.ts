...
function makeCtx(grants: StepToolContext['grants'], storeDir: string): StepToolContext {
  return {
...
function makeCapturingAgentFactory(): {
  factory: <E extends BaseEnv>(def: AgentDefinition<E>, env: E) => Promise<Agent>;
  captured: { authorize: AuthorizeFn | null };
} {
  const captured: { authorize: AuthorizeFn | null } = { authorize: null };
  const factory = (async <E extends BaseEnv>(_def: AgentDefinition<E>, env: E): Promise<Agent> => {
...
function makeStepEnv(ctx: StepToolContext, storeDir: string): BaseEnv & Record<string, unknown> {
  return {
    sources: [],
    defaultSource: 'src',
    storage: {
...
describe.skipIf(!REAL_AUTHZ)('createStepAgentFactory authorize (real @intx/authz)', () => {
  runRealAuthzTests();
});
...
function runRealAuthzTests(): void {
  test("real evaluateGrants allows a granted tool's invoke", async () => {
    ...
    await stepFactory(def, makeStepEnv(makeCtx([GRANOLA_INVOKE_GRANT], storeDir), storeDir));

    expect(captured.authorize).not.toBeNull();
    const result = await captured.authorize!('tool:granola_get_meeting', 'invoke', {});
  ...
  test("real evaluateGrants denies an ungranted tool's invoke", async () => {
    ...
    expect(captured.authorize).not.toBeNull();
    const result = await captured.authorize!('tool:gamma_generate', 'invoke', {});
