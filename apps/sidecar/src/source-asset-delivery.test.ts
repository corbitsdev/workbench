...
function manifest(entries: ToolPackageManifest["entries"]): ToolPackageManifest {
...
describe("sourceAssetGitDir", () => {
  ...
  test.each([
    ...
    expect(() => sourceAssetGitDir("/root/gits", assetId)).toThrow(/unsafe assetId/);
  });
});
...
describe("indexAssetPackIntoGitDir", () => {
  ...
  async function tempDir(): Promise<string> {
    const d = await fsp.mkdtemp(path.join(os.tmpdir(), "source-delivery-test-"));
    tempDirs.push(d);
    return d;
  }
