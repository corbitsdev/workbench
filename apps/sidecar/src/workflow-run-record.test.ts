...
const OTHER_KEY_CIPHER = createEnvKeyCredentialCipher(new Uint8Array(32).fill(2));
...
async function readRawRecord(dataDir: string, anchorRunId: string): Promise<WorkflowRunRecord> {
...
describe("workflow run record store", () => {
  ...
  test("seals every credential in the cell, top-level and per-body alike", async () => {
    ...
    const sealed = new Map(
      (onDisk.credentials?.materials ?? []).map((m) => [m.credentialId, m.secret]),
    );
  ...
  test("soft-skips a credential decrypt failure, dropping the whole run", async () => {
    ...
    await fs.writeFile(recordPath(dataDir, anchorRunId), JSON.stringify(onDisk, null, 2), "utf8");
...
describe("scanWorkflowRunRecords", () => {
  ...
  test("soft-skips a record that fails schema validation", async () => {
    ...
    await fs.mkdir(dir, { recursive: true });
    await fs.writeFile(path.join(dir, "deployment.json"), JSON.stringify({ version: 2 }), "utf8");
    expect(await scanWorkflowRunRecords(dataDir, CIPHER)).toEqual([]);
    await fs.rm(dataDir, { recursive: true, force: true });
  });
});
