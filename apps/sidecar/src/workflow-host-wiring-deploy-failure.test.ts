...
function stubApplyFrozenWorkflowClosure(
  definitionId: string,
): NonNullable<Parameters<typeof createSidecarDeployRouter>[0]["applyFrozenWorkflowClosure"]> {
  return (applyArgs) =>
    Promise.resolve({
...
describe("deploy-failure registry leak", () => {
  test("a frame carrying neither provisionStep nor a workflow is rejected before any deploy work", async () => {
    ...
    const router = createSidecarDeployRouter({
      ...
      sessions: {} as Parameters<typeof createSidecarDeployRouter>[0]["sessions"],
    ...
    await expect(router.deploy(frame)).rejects.toThrow(/unsupported deploy frame/);
