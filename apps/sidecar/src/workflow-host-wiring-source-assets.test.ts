...
function sourcePin(assetId: string, packageDirs: readonly string[]): SourceRefPin {
  return {
    source: {
      kind: "asset",
      assetId,
...
beforeEach(async () => {
  dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "wf-deploy-source-assets-"));
});
...
describe("resolveDeploymentAssetMounts", () => {
  test("derives the pin's mount map when the store is populated", async () => {
    ...
    await fs.mkdir(
      path.join(dataDir, "workflow-definition-sources", DEPLOYMENT_ID, "source-assets", assetId),
      { recursive: true },
    );
    ...
    expect(assetRoot).toBe(path.join(dataDir, "workflow-definition-sources", DEPLOYMENT_ID));
  ...
  test("fails loud when a pinned asset is absent from the store", async () => {
    await expect(
      resolveDeploymentAssetMounts(dataDir, DEPLOYMENT_ID, assetPin(["asset_missing"])),
  ...
  test("derives the pin's gitDir map when the durable git store is populated", async () => {
    ...
    await fs.mkdir(path.join(dataDir, "workflow-definition-source-gits", DEPLOYMENT_ID, assetId), {
      recursive: true,
    });
    ...
    expect([...gitDirs]).toEqual([
      [assetId, path.join(dataDir, "workflow-definition-source-gits", DEPLOYMENT_ID, assetId)],
    ]);
  });

  test("fails loud when a source pin's git store is absent", async () => {
    await expect(
      resolveDeploymentAssetMounts(dataDir, DEPLOYMENT_ID, sourcePin("asset_missing", ["."])),
