...
import { consumerBindings, type StepCredentialContext } from "../src/step-agent-tools";
...
describe("consumerBindings (sidecar) vs deriveResolvedBindings (@corbits/credential-providers)", () => {
  test("agree on the bound handle's static fields for a matching consumer", () => {
    ...
    const fromGranolaCopy = deriveResolvedBindings(delivery("s3cr3t"), CONSUMER);
  ...
  test("agree on the secret readCurrentMaterial resolves, including after a rotation", () => {
    ...
    const copyBinding = deriveResolvedBindings(originalDelivery, CONSUMER).get("granola");
    ...
    const copyBindingAfterRotation = deriveResolvedBindings(rotatedDelivery, CONSUMER).get(
      "granola",
    );
    expect(copyBindingAfterRotation?.readCurrentMaterial().secret).toBe("rotated-secret");
  });
  ...
  test("agree on an empty delivery yielding no bindings", () => {
    ...
    expect(deriveResolvedBindings({ bindings: [], materials: [] }, CONSUMER).size).toBe(0);
  });
});
