...
import { SIDECAR_SUBSTRATE_CONFIG_KEYS } from "../src/workflow-substrate-factory";
...
describe("tool-registry threading", () => {
  test("SIDECAR_TOOL_REGISTRIES is a child substrate-config key, so the boot edge's registries reach per-step materialization", () => {
    expect(SIDECAR_SUBSTRATE_CONFIG_KEYS).toContain("SIDECAR_TOOL_REGISTRIES");
  });
  ...
  test("parseToolRegistries collapses the wire array into a map keyed by name", () => {
    const registries = parseToolRegistries(
      JSON.stringify([
        {
          name: "internal",
    ...
    expect(registries.get("internal")).toEqual({
      url: "https://npm.example.com",
      auth: { token: "t" },
    });
    expect(registries.get("npmjs")).toEqual({
      url: "https://registry.npmjs.org",
    });
  });

  test("an empty value fails loud instead of falling back to public npmjs", () => {
    expect(() => parseToolRegistries("")).toThrow(/SIDECAR_TOOL_REGISTRIES/);
  });
  ...
  test("malformed JSON, wrong shapes, and duplicate names fail loud", () => {
    ...
    ).toThrow(/duplicate registry name/);
  });
});
