...
mock.module('@workbench/chat', () => ({
  ChatPanel: (props: {
    ...
    React.createElement(
      'div',
      null,
      React.createElement('div', { 'data-testid': 'tagline' }, props.agent.tagline ?? ''),
      React.createElement('div', { 'data-testid': 'count' }, String(props.messages.length)),
      props.notice ? React.createElement('div', { 'data-testid': 'notice' }, props.notice) : null,
      props.inputDisabled ? React.createElement('span', { 'data-testid': 'disabled' }) : null,
      React.createElement('button', { onClick: () => props.onSend('hello') }, 'send')
    ),
}));
...
describe('MyraChatSurface', () => {
  ...
  it('shows the credential notice when no key resolves', () => {
    ...
    expect(screen.getByTestId('notice').textContent).toMatch(/No API credential/);
  });
  ...
  it('renders messages and wires send when ready', () => {
    ...
    render(React.createElement(MyraChatSurface, { session, threadLabel: 'Pricing' }));
