import { type ComponentType, useCallback, useLayoutEffect, useMemo, useRef, useState } from 'react';
...
// Two-screen modal: the grid (agents/workflows catalog) and the per-workflow
// intake form. When a workflow has an IntakeForm the modal transitions to it;
// the form's onSubmit fires the run start and closes the modal.
type ModalScreen =
  | { type: 'grid' }
  | { type: 'intake'; kind: string; label: string; IntakeForm: ComponentType<IntakeFormProps> };
...
export function UnifiedCatalogModal({
  ...
  const [screen, setScreen] = useState<ModalScreen>({ type: 'grid' });
  ...
  const handleKeyDown = useCallback(
    ...
    [handleClose, handleBack, screen.type]
  );

  // Focus the first interactive element whenever the modal opens or transitions
  // between screens. This is a DOM mutation — useLayoutEffect runs synchronously
  // after the DOM updates, before the browser paints, which prevents flicker.
  useLayoutEffect(() => {
  ...
  const handleSelectWorkflow = async (kind: string) => {
    if (loadingKind) return;
    setLoadingKind(kind);
    setError(null);
    try {
      const ui = await loadWorkflowUI(kind);
      if (ui?.IntakeForm) {
        setScreen({
          type: 'intake',
          kind,
          label: ui.label ?? toHumanLabel(kind),
          IntakeForm: ui.IntakeForm,
        });
      } else {
  ...
  const handleIntakeSubmit = async (input: Record<string, unknown>) => {
    if (screen.type !== 'intake') return;
    const { kind } = screen;
    const res = await startWorkflow.mutateAsync({ kind, input });
    handleClose();
    onWorkflowStarted(res.deploymentId);
  ...
  const isGridBusy = deploying !== null || loadingKind !== null;
            ...
            {screen.type === 'intake' ? (
              <>
                ...
                <div className="flex items-center gap-3 border-b border-border px-5 py-4">
                  ...
                  <div className="flex-1 text-[16px] font-bold text-text">{screen.label}</div>
                  <button
                    type="button"
                    onClick={handleClose}
                    aria-label="Close"
                ...
                <div className="min-h-0 flex-1 overflow-y-auto">
                  <screen.IntakeForm onSubmit={handleIntakeSubmit} onCancel={handleBack} />
                </div>
              </>
            ) : (
              <>
                  ...
                  <div className="flex gap-1 rounded-[8px] bg-surface-2 p-[3px]">
                    {(['agents', 'workflows'] as Tab[]).map((t) => (
                      <button
                        key={t}
                        type="button"
                        onClick={() => { setTab(t); setSearch(''); setError(null); }}
                      ...
                      {filteredWorkflowKinds.map((kind) => (
                        <div key={kind} className="flex flex-col justify-between gap-3 rounded-[10px] border border-border p-4 transition-colors hover:bg-[var(--row-hover)]">
                          ...
                          <button
                            type="button"
                            disabled={isGridBusy}
                            onClick={() => void handleSelectWorkflow(kind)}
                            className="self-start rounded-[7px] border border-border px-3 py-1 text-[12px] font-semibold text-text-2 transition-colors active:scale-[0.97] disabled:opacity-50 hover:border-orange hover:text-orange"
                          >
                            {loadingKind === kind ? 'Loading…' : 'Start'}
                          </button>
                        </div>
                      ))}
