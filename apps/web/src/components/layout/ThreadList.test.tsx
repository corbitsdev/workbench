...
mock.module('../../hooks/use-myra-threads', () => ({
  useMyraThreads: () => ({
    data: [
      { id: 't1', instanceId: 'i1', label: 'First', createdAt: '2026-01-01T00:00:00Z' },
      { id: 't2', instanceId: 'i2', label: 'Second', createdAt: '2026-01-02T00:00:00Z' },
    ],
    isLoading: false,
  }),
...
function renderList(path = '/chats/t1') {
  render(
    React.createElement(MemoryRouter, { initialEntries: [path] }, React.createElement(ThreadList))
  );
}
...
describe('ThreadList', () => {
  ...
  it('renames a thread via the options menu', () => {
    renderList();
    const optionButtons = screen.getAllByRole('button', { name: /thread options/i });
    ...
    expect(renameMutate.mock.calls[0]?.[0]).toEqual({ id: 't1', label: 'Renamed' });
  });
