  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setAgentLoadError(
          agentResults.some((result) => result.status === "rejected"),
        );
        setError(false);
      } catch {
        setError(true);
...
export function LibraryRail({
  ...
  const {
    ...
    tenantId: activeWorkbenchSlug
      ? (activeWorkbenchTenantId ?? null)
      : undefined,
  });
  ...
  const completedJobItems = allJobItems.filter(
    (w) => w.workflowStatus === "done",
  );
