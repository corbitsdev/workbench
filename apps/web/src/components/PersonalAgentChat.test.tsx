...
type Thread = { id: string; instanceId: string; label: string; createdAt: string };
...
mock.module('./MyraChatSurface', () => ({
  MyraChatSurface: () => React.createElement('div', { 'data-testid': 'surface' }),
}));
...
beforeEach(() => {
  ...
  threads = [
    { id: 'thr-other', instanceId: 'inst-other', label: 'Other', createdAt: '2026-01-01' },
    { id: 'thr-target', instanceId: 'inst-target', label: 'Target', createdAt: '2026-01-02' },
  ];
...
describe('PersonalAgentChat dock handoff', () => {
  it('delivers a seeded first message to the handed-off thread exactly once', async () => {
    ...
    await waitFor(() => expect(sendSpy).toHaveBeenCalledWith('seeded question'));
