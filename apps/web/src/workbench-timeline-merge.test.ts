...
function thread(overrides: Partial<ChannelThread> = {}): ChannelThread {
  return {
    id: "thread-1",
    kind: "reply",
...
function routineRun(overrides: Partial<RoutineRun> = {}): RoutineRun {
  return {
    ...
    run: { status: "deployed", createdAt: "2026-08-17T09:00:00.000Z", endedAt: "2026-08-17T09:02:00.000Z" },
    ...overrides,
  } as RoutineRun;
}
...
describe("toMessageEvents", () => {
  ...
  test("falls back to the sender's local part, never the raw address", () => {
    const [event] = toMessageEvents([
      message({ sender: { name: null, address: "echo@channel-1" } }),
    ]);
  ...
  test("marks an agent sender via its @-shaped address", () => {
    const [event] = toMessageEvents([
      message({ sender: { name: null, address: "researcher@channel-1" } }),
    ]);
...
describe("routine run derivation", () => {
  ...
  test("computes duration only when the record carries endedAt", () => {
    ...
    expect(routineRunDurationMs(routineRun({ run: { status: "running" } }))).toBeNull();
  });
});

describe("routinesForChannel", () => {
  test("keeps only routines that deliver into this workbench", () => {
    const routines = [
      routine({ id: "r1", deliveryChannelId: "channel-1" }),
      routine({ id: "r2", deliveryChannelId: "channel-2" }),
    ];
    expect(routinesForChannel(routines, "channel-1").map((r) => r.id)).toEqual([
      "r1",
    ]);
  });
});
...
describe("toTaskEvents", () => {
  test("filters to this workbench's channelId", () => {
    const tasks = [
      task({ id: "t1", channelId: "channel-1" }),
      task({ id: "t2", channelId: "channel-2" }),
      task({ id: "t3", channelId: null }),
    ];
    expect(toTaskEvents(tasks, "channel-1").map((e) => e.id)).toEqual(["t1"]);
  });
});
...
describe("mergeTimelineEvents", () => {
  test("sorts every kind onto one oldest-first spine", () => {
    const merged = mergeTimelineEvents({
      ...
      threadForks: toThreadForkEvents([thread({ id: "f1", createdAt: "2026-08-17T09:30:00.000Z" })]),
      routineRuns: toRoutineRunEvents(
        [routine({ id: "r1" })],
        new Map([["r1", [routineRun({ runId: "run-1", createdAt: "2026-08-17T08:00:00.000Z" })]]]),
      ),
      tasks: toTaskEvents([task({ id: "t1", createdAt: "2026-08-17T11:00:00.000Z" })], "channel-1"),
...
describe("groupTimelineByDay", () => {
  test("buckets an already-sorted spine by UTC calendar day, oldest day first", () => {
    const events: TimelineEvent[] = [
      { kind: "message", id: "m1", at: "2026-08-16T23:00:00.000Z", senderName: "A", excerpt: "x", isAgent: false },
      { kind: "message", id: "m2", at: "2026-08-17T01:00:00.000Z", senderName: "A", excerpt: "y", isAgent: false },
      { kind: "message", id: "m3", at: "2026-08-17T02:00:00.000Z", senderName: "A", excerpt: "z", isAgent: false },
    ];
...
describe("computeTimelineDayKpis", () => {
  test("counts messages, agent turns, routine runs, and approvals per day", () => {
    const events: TimelineEvent[] = [
      { kind: "message", id: "m1", at: "2026-08-17T09:00:00.000Z", senderName: "A", excerpt: "x", isAgent: false },
      { kind: "message", id: "m2", at: "2026-08-17T09:05:00.000Z", senderName: "researcher", excerpt: "y", isAgent: true },
      { kind: "routine-run", id: "run-1", at: "2026-08-17T09:10:00.000Z", routineName: "Digest", runId: "run-1", status: "ok", durationMs: null },
      { kind: "approval", id: "a1", at: "2026-08-17T09:15:00.000Z", agentName: "Researcher", headline: "Do it" },
      { kind: "task", id: "t1", at: "2026-08-17T09:20:00.000Z", agentName: "Researcher", prompt: "p", status: "done" },
      { kind: "thread-fork", id: "f1", at: "2026-08-17T09:25:00.000Z", threadKind: "reply", title: "Fork", parentMessageId: null },
    ];
...
describe("filterTimelineEvents", () => {
  const events: TimelineEvent[] = [
    { kind: "message", id: "m1", at: "2026-08-17T09:00:00.000Z", senderName: "A", excerpt: "x", isAgent: false },
    { kind: "thread-fork", id: "f1", at: "2026-08-17T09:01:00.000Z", threadKind: "reply", title: "Fork", parentMessageId: null },
    { kind: "routine-run", id: "run-1", at: "2026-08-17T09:02:00.000Z", routineName: "Digest", runId: "run-1", status: "ok", durationMs: null },
    { kind: "task", id: "t1", at: "2026-08-17T09:03:00.000Z", agentName: "Researcher", prompt: "p", status: "done" },
    { kind: "approval", id: "a1", at: "2026-08-17T09:04:00.000Z", agentName: "Researcher", headline: "Do it" },
  ];
