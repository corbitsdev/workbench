...
describe("ACTION_COMMANDS", () => {
  ...
  test("new-workbench and new-agent speak consumer language, not mint", () => {
    ...
    expect(workbench?.title).toBe("New room");
    expect(agent?.title).toBe("New room");
    expect(workbench?.subtitle).toBe("Start a new room");
    expect(agent?.subtitle).toBe("Start a new room");
