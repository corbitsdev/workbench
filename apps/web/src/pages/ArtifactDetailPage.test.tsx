...
const createMutate = mock(
  (_arg: undefined, opts?: { onSuccess?: (t: CreatedThread) => void }) => {
    opts?.onSuccess?.(createdThread);
  },
);
const generateTitleMutate = mock(
  (_args: { id: string; firstMessage: string }) => {},
);
...
function sendMessage(text: string) {
  fireEvent.change(
    screen.getByPlaceholderText(/ask myra about this artifact/i),
    {
      target: { value: text },
    },
  );
