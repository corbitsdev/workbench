...
export function parsePaletteQuery(raw: string): ParsedPaletteQuery {
  const scope = PALETTE_SCOPES.find((candidate) => candidate.prefix === raw.slice(0, 1)) ?? null;
  const query = (scope === null ? raw : raw.slice(scope.prefix.length)).trim();
  return { scope, query };
}

// True for a bare scope prefix (`"#"`, `"@ "`) — a consumer should use
// this to fetch the scope's unfiltered list directly, since a debounced
// search that skips empty queries can't produce it.
