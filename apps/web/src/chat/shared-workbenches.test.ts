...
function workbench(id: string, title: string, participantAddresses: readonly string[]): Workbench {
  return {
    id,
    title,
    kind: "workbench",
...
describe("sharedWorkbenchesWith", () => {
  test("keeps only workbenches both the viewer and the subject participate in", () => {
    ...
    const result = sharedWorkbenchesWith(workbenches, "viewer", "subject@x.dev");

    expect(result.map((c) => c.id)).toEqual(["c1"]);
  });

  test("matches the viewer by principal id against the participant address's local part", () => {
    const workbenches = [workbench("c1", "DM", ["ins_abc@x.dev", "viewer@x.dev"])];

    const result = sharedWorkbenchesWith(workbenches, "viewer", "ins_abc@x.dev");
  ...
  test("caps the result at the given limit, mock parity default of 4", () => {
    ...
    expect(sharedWorkbenchesWith(workbenches, "viewer", "subject@x.dev")).toHaveLength(4);
    expect(sharedWorkbenchesWith(workbenches, "viewer", "subject@x.dev", 2)).toHaveLength(2);
  });

  test("reports each shared workbench's title and member count", () => {
    const workbenches = [
      workbench("c1", "Launch planning", ["viewer@x.dev", "subject@x.dev", "third@x.dev"]),
    ];

    const result = sharedWorkbenchesWith(workbenches, "viewer", "subject@x.dev");

    expect(result).toEqual([{ id: "c1", title: "Launch planning", memberCount: 3 }]);
  });

  test("falls back to a placeholder title for an untitled workbench", () => {
    const workbenches = [workbench("c1", "", ["viewer@x.dev", "subject@x.dev"])];
