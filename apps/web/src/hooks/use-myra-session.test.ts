...
const launchInstanceSession = mock((_id: string) => Promise.resolve({ launched: true }));
...
mock.module('../lib/instance-transport', () => ({ createHubTransport: () => ({}) }));
...
describe('deliverMessage', () => {
  ...
  it('rethrows a non-409 error without relaunching', async () => {
    ...
    await expect(deliverMessage(session, 'inst-1', 'hi')).rejects.toBeInstanceOf(FakeApiError);
  ...
  it('cannot recover a 409 without an instance id', async () => {
    ...
    await expect(deliverMessage(session, null, 'hi')).rejects.toBeInstanceOf(FakeApiError);
    expect(launchInstanceSession).not.toHaveBeenCalled();
  });
});
