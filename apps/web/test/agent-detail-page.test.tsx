...
describe("AgentDetailPage edits", () => {
  ...
  test("a failed save says so and keeps the person's edit", async () => {
    const node = await mount();
    globalThis.fetch = (() =>
      Promise.resolve(
    ...
    expect(node.textContent).toContain("saving this agent");
    expect(prompt.value).toBe("New instructions.");
  });
});
