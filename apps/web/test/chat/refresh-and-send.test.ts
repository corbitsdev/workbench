...
import {
  ...
  mergeStreamingReply,
  withScrollSnapshot,
} from "../../src/chat/chat-workspace";
import type { PendingSend } from "../../src/chat/chat-workspace";
import {
...
import type { ComposerAttachment } from "../../src/chat/composer";
import { CHAT_STRINGS } from "../../src/chat/strings";
import { backoffDelayMs, shouldConnect } from "../../src/chat/use-workbench-stream";
...
describe("composerPlaceholderFor (names the open recipient)", () => {
  test("names the counterpart for an agent chat from the workbench title — never a hardcoded agent", () => {
    ...
    expect(composerPlaceholderFor({ kind: "chat", title: "Scout" })).toBe(
      CHAT_STRINGS.composerPlaceholderChat("Scout"),
    );
  ...
  test("keeps the generic copy with no workbench resolved yet", () => {
    expect(composerPlaceholderFor(undefined)).toBe(CHAT_STRINGS.composerPlaceholder);
  });
