...
// A `spyOn` mock is per-property and restorable — unlike `mock.module`,
// which patches the shared module registry for the whole test process and
// would otherwise leak this fake "ref_test" id into every other suite that
// imports `@corbits/error-sink` (e.g. useStreamingReply's ref-id tests).
const reportErrorSpy = spyOn(errorSink, "reportError").mockImplementation(
...
const { PeopleSection, PeopleTable } = await import("../../src/settings/people-section");
...
describe("PeopleTable Actions column", () => {
  test("Actions cells opt out of page-fill nowrap-ellipsis clipping", () => {
    ...
    const css = readFileSync(new URL("../../src/settings/styles.css", import.meta.url), "utf8");
