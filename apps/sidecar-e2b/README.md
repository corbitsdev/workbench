# @workbench/sidecar-e2b

The E2B image template that boots `apps/sidecar` inside an
[E2B](https://e2b.dev) sandbox. The `e2b` `SidecarProvisioner` backend
itself lives in `apps/hub/src/provisioners/e2b.ts` — this app holds only
the template build.
...
```sh
E2B_API_KEY=e2b_... bun apps/sidecar-e2b/template/build.ts
```
...
`apps/hub/src/provisioners/e2b.ts`).

## Operator environment variables

Required by the provisioner (`readProvisionerConfig` in
`apps/hub/src/provisioners/e2b.ts`):
...
creates billable infrastructure, so this app's tests do not do it.
