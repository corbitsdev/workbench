...
import { projectWorkflow, type ProjectedStep, type ProjectedWorkflow } from './projection';
    ...
    steps: {
      ...
      analyze: {
        ...
        agent: {
          ...
          inference: { sources: [{ provider: 'openai-compatible', model: 'deepseek-v4-flash' }] },
        },
...
describe('projectWorkflow', () => {
  ...
  test('projects a plain agent step into a reasoning step with source + credentialName', () => {
    ...
    expect(analyze.source).toEqual({ provider: 'openai-compatible', model: 'deepseek-v4-flash' });
  ...
  test('throws on an unsupported step kind', () => {
    ...
    (def.steps as Record<string, unknown>).bad = { kind: 'sleep', id: 'bad', duration: 1 };
    (def as unknown as { stepOrder: string[] }).stepOrder = [...def.stepOrder, 'bad'];
