...
import { advanceRun, resumeRun, type ExecutorDeps, type RunState } from './executor';
...
function makeDeps(overrides: Partial<ExecutorDeps> = {}): {
  ...
  reasoningCalls: Array<{ stepId: string; input: unknown }>;
} {
...
describe('advanceRun', () => {
  test('runs a deterministic tool step, saves its output, completes', async () => {
    ...
    expect(toolCalls).toEqual([{ tool: 'granola_get_note', input: { noteId: 'n1' } }]);
    expect(state.status).toBe('completed');
    expect(state.outputs.fetch).toEqual({ ok: 'granola_get_note', echoed: { noteId: 'n1' } });
  ...
  test('runs a reasoning step with merged input from prior outputs', async () => {
    ...
    expect(reasoningCalls).toEqual([{ stepId: 'analyze', input: { transcript: 'hello' } }]);
  ...
  test('parks at a gate with status awaiting and persists currentStepId', async () => {
    const wf: ProjectedWorkflow = {
      ...
      steps: {
        ...
        select: { kind: 'gate', id: 'select', signalName: 'note-selection', after: ['intake'] },
        fetch: {
          kind: 'tool',
          id: 'fetch',
          tool: 'granola_get_note',
  ...
  test('map step fans out over an array, collecting outputs', async () => {
    ...
    const state = await advanceRun(
      wf,
      deps,
      makeState({ input: { formats: [{ format: 'email' }, { format: 'blog' }] } })
    );

    expect(reasoningCalls).toHaveLength(2);
    expect(reasoningCalls.map((c) => c.input)).toEqual([{ format: 'email' }, { format: 'blog' }]);
  ...
  test('a thrown step fails the run and records the error', async () => {
    const wf: ProjectedWorkflow = {
      id: 'wf',
      order: ['boom'],
      steps: { boom: { kind: 'tool', id: 'boom', tool: 'bad', input: { literal: {} }, after: [] } },
    };
    const { deps } = makeDeps({
      toolRunner: {
        run: async () => {
...
describe('resumeRun', () => {
  ...
  test('applies the gate payload as output and continues from currentStepId', async () => {
    ...
    state = await resumeRun(wf, deps, state, 'note-selection', { noteId: 'n42' });

    expect(state.outputs.select).toEqual({ noteId: 'n42' });
    // fetch ran with the gate output as its input
    expect(toolCalls.find((c) => c.tool === 'granola_get_note')?.input).toEqual({ noteId: 'n42' });
    expect(state.status).toBe('completed');
  });
  ...
  test('rejects a resume when not awaiting', async () => {
    const { deps } = makeDeps();
    await expect(
      resumeRun(wf, deps, makeState({ status: 'running' }), 'note-selection', {})
  ...
  test('rejects a signal-name mismatch', async () => {
    const { deps } = makeDeps();
    const state = await advanceRun(wf, deps, makeState());
    await expect(resumeRun(wf, deps, state, 'wrong-signal', {})).rejects.toThrow(/awaits signal/);
  });
