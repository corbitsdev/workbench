...
const { isCredentialToolEntry, KNOWN_TOOLS } = await import(
  "../lib/tool-registry"
);
...
const authorizer = {
  assertToolGranted: async (_s: RunState, tool: string) => {
    if (tool !== GRANTED) {
      throw new Error(
        `workflow executor: principal not granted tool:${tool}/invoke`,
      );
    }
  },
  authorizeFn: () => async () => ({
...
describe("createHubToolRunner tool-grant gate", () => {
  test("dispatches canonical package tool names through the local hub registry name", async () => {
    ...
    expect(result).toEqual({
      content: JSON.stringify({ ok: "granola_list_notes" }),
    });
  });
  ...
  test("rejects an ungranted tool without invoking it", async () => {
    ...
    await expect(
      runner.run({ tool: UNGRANTED, input: {}, state: state("prn-1") }),
