...
const db = {} as unknown as Parameters<
  typeof createWorkflowAuthorizer
>[0]["db"];
...
describe("createWorkflowAuthorizer (real @intx/authz)", () => {
  test("assertToolGranted allows a granted tool and rejects an ungranted one", async () => {
    ...
    await expect(
      authz.assertToolGranted(state, "gamma_generate"),
    ).rejects.toThrow(/tool:gamma_generate\/invoke/);
  });
