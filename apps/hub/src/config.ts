...
// tree. isomorphic-git walks up from a data dir looking for `.git`; when
// HUB_DATA_DIR lives inside this repo (the `.env` default `.data/hub`), a
// missed nested init authors a genesis commit onto the working branch and a
// checkout of that tree can delete the checkout (see AGENTS.md).
...
function envWithoutGitDir(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const next: NodeJS.ProcessEnv = {};
  for (const [key, value] of Object.entries(env)) {
    if (value === undefined || GIT_DIR_VARS.has(key)) continue;
    next[key] = value;
  }
  return next;
}
...
/**
 * Called at hub boot (`index.ts`), never from `readHubConfig` itself:
 * this does real filesystem/subprocess IO against `dir`, which would
 * make every test parsing a config trip it against whatever `.data/hub`-
 * shaped path a fixture happens to use.
 */
export function assertHubDataDirGitSafety(
  ...
  if (allowInsideWorkTree) return;
  const enclosing = enclosingGitWorkTree(dir, env);
  if (enclosing === null) return;
  throw new Error(
    [
      `HUB_DATA_DIR (${dir}) is inside the git work tree at ${enclosing}.`,
      "The hub refuses to initialize git-on-disk state there: a nested init",
      "that misses its own .git walks up and commits onto the working",
