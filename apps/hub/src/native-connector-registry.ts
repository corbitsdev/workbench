...
import { mcpPresetConnectorIds } from "@corbits/connections/mcp-presets";
import {
  exchangeCodeForToken,
  HUGGINGFACE_AUTHORIZE_URL,
  HUGGINGFACE_SCOPE,
} from "@corbits/connections/huggingface-connect";
import {
...
// without routing the registry through that package's client surface.
// Undefined on any malformed input — the account id is a label, never a
// gate. (The legacy source kept this local so the registry could ship in
// the web bundle; the hub keeps the same local copy so the exchange result
// stays byte-identical through the cutover.)
...
} from "@corbits/connections/github-connect";
import {
  testExaCredential,
...
function inferenceProviderDescriptors(): Record<string, ConnectorDescriptor> {
  ...
  // packages' own constants subpath (pure data — the exchange closures
  // stay plain `fetch`), so the descriptor can never drift from the values
  // the adapter's own refresh path uses.
  entries["codex"] = {
    id: "codex",
    ...
    oauth: {
      ...
      exchange: async ({ code, codeVerifier }) => {
        try {
          ...
          // report-error-ignore: a failed Codex exchange is surfaced to the
          // caller in the returned message — the failure IS the result.
          return {
            ok: false,
    ...
    oauth: {
      ...
      exchange: async ({ code, codeVerifier }) => {
        try {
          ...
          // report-error-ignore: a failed xAI exchange is surfaced to the
          // caller in the returned message — the failure IS the result.
          return {
            ok: false,
...
export const CONNECTOR_REGISTRY: ConnectorRegistry = createConnectorRegistry({
