...
import {
  migrateUserToGlobalTenant,
  type MigrationDeps,
} from "./migrate-to-global-tenant";
...
function makeDb(opts: {
  ...
  const update = mock((table: unknown) => {
    updateCalls.push(
      String((table as { _?: { name?: string } })?._?.name ?? "unknown"),
    );
  ...
  base = {
    query: {
      ...
      agentInstance: {
        findMany: mock(() => Promise.resolve(fm.agentInstance ?? [])),
      },
      workflowRun: {
        findMany: mock(() => Promise.resolve(fm.workflowRun ?? [])),
      },
      artifact: { findMany: mock(() => Promise.resolve(fm.artifact ?? [])) },
      artifactVersion: {
        findMany: mock(() => Promise.resolve(fm.artifactVersion ?? [])),
      },
      enabledWorkflow: {
        findMany: mock(() => Promise.resolve(fm.enabledWorkflow ?? [])),
      },
...
describe("migrateUserToGlobalTenant", () => {
  const COMMON = {
    globalTenantId: "tnt_global",
    globalTenantDomain: "acme.example.com",
  };
