...
export async function generateCollateralWithLLM(
  ...
  const resolvedMaxTokens =
    maxOutputTokens ??
    (LONG_FORM_KINDS.has(type) ? LONG_FORM_MAX_TOKENS : undefined);
  ...
  if (!raw)
    throw new Error("LLM returned empty content for collateral generation");
  ...
  try {
    ...
    if (!retryRaw)
      throw new Error(
        "LLM returned empty content on retry for collateral generation",
      );
