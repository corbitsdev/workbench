...
describe("org credential inheritance for seeded templates (CL-1445)", () => {
  ...
  it("getAncestorChain walks workbench -> global root", async () => {
    ...
    const ancestorRows: AncestorRow[] = [
      { parentId: GLOBAL_TENANT_ID },
      { parentId: null },
    ];
    const db = {
      query: {
        tenant: {
          findFirst: () => Promise.resolve(ancestorRows[tenantCall++]),
        },
      },
    };
  ...
  it("resolves the org-level credential for a tenant-sourced requirement down the chain", async () => {
    ...
    const credentialAnswers: (typeof ORG_CREDENTIAL)[][] = [
      [],
      [ORG_CREDENTIAL],
    ];
    let credCall = 0;
    ...
    const db = {
      query: {
        ...
        credential: {
          findMany: () => Promise.resolve(credentialAnswers[credCall++] ?? []),
        },
      },
    };
