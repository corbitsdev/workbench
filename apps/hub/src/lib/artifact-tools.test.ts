...
describe('artifact tool registry', () => {
  ...
  it('artifact_read schema includes optional tenantId property', () => {
    expect(ARTIFACT_READ_DEFINITION.inputSchema.properties).toHaveProperty('tenantId');
    expect(ARTIFACT_READ_DEFINITION.inputSchema.required).toEqual(['artifactId']);
  });
});
...
describe('artifact_link_file handler', () => {
  ...
  it('reproduces linking file artifact without workflow session_id (sets null for agent tools; DB column must allow it)', async () => {
    ...
    await handler({ title: 'Story', kind: 'story', path: '/home/user/dragon-story.md' });
...
describe('artifact_read handler', () => {
  ...
  it('rejects a cross-tenant read when the owning user is not a member of the target tenant', async () => {
    ...
    await expect(handler({ artifactId: 'art_cross', tenantId: 'tnt_other' })).rejects.toThrow(
      /Artifact not found/
    );
  });
});
...
describe('artifact_link_presentation handler', () => {
  ...
  it('rejects a non-https url', async () => {
    ...
    await expect(handler({ url: 'http://gamma.app/docs/abc', title: 'Deck' })).rejects.toThrow(
      /must use HTTPS/
    );
  });
