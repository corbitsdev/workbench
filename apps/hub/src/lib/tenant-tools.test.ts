...
let listAssetRows: Array<{ id: string; name: string }> = [];
const listAssetsForTenant = (async () =>
  listAssetRows) as unknown as typeof intxDb.listAssetsForTenant;

mock.module('@intx/db', () => ({ ...intxDb, resolveCredentialRequirement, listAssetsForTenant }));

const { listAvailableToolSummaries, getAvailableToolDetail, resolveToolVersions } =
...
async function buildTarball(name: string, version: string): Promise<Uint8Array> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'tenant-tools-test-'));
  const pkgDir = path.join(dir, 'package');
  await fs.mkdir(pkgDir, { recursive: true });
  await fs.writeFile(path.join(pkgDir, 'package.json'), JSON.stringify({ name, version }));
...
describe('resolveToolVersions', () => {
  ...
  it('resolves a real name@version against the tenant package-registry asset', async () => {
    ...
    listAssetRows = [{ id: 'asset-builtins', name: WORKSPACE_BUILTINS_REGISTRY }];
    const service = assetServiceFor({ 'tarballs/tools-attio-0.1.7.tgz': tgz });

    const versions = await resolveToolVersions(db, 'tenant-1', ['attio_query_records'], service);
