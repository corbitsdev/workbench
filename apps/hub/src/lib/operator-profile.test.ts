...
const OPTS = { tenantId: 'tnt_1', instanceId: 'ins_1', provider: 'openai-compatible' };
const PERSONAL_MAPPING = { templateKey: 'myra', memberPrincipalId: 'prn_member' };
...
describe('composePersonalAgentPromptForInstance', () => {
  ...
  it('returns null for a non-personal instance (keeps the seeded prompt)', async () => {
    const db = fakeDb([[{ templateKey: 'oat', memberPrincipalId: 'prn_member' }]]);
