...
function createdInRange(createdAtColumn: AnyColumn, range?: AnalyticsDateRange) {
  if (range?.startDate === undefined && range?.endDate === undefined) {
    return undefined;
  }
  const start = range?.startDate ? new Date(`${range.startDate}T00:00:00.000Z`) : undefined;
  const end = range?.endDate ? new Date(`${range.endDate}T23:59:59.999Z`) : undefined;
  return and(
...
export async function getActivityOverview(args: {
  ...
  const tenantArtifacts = and(eq(artifact.tenantId, tenantId), ne(artifact.status, 'rejected'));

  const [artifactTotalRow, artifactInRangeRow, artifactByStatus, artifactByKind] =
    await Promise.all([
  ...
  const [runTotalRow, runInRangeRow, runActiveRow, runByStatus, runByKind, deploymentRow] =
    await Promise.all([
      ...
      db
        .select({ count: count() })
        .from(workflowRunRecord)
        .where(and(runBase, inArray(workflowRunRecord.status, ['running', 'awaiting']))),
      db
  ...
  const [instanceActiveRow, instanceStartedRow, instanceEndedRow, instanceTotalRow] =
    await Promise.all([
      db
        .select({ count: count() })
        .from(agentInstance)
      ...
      db
        .select({ count: count() })
        .from(agentInstance)
        .where(and(instanceTenant, createdInRange(agentInstance.createdAt, range))),
      db
        .select({ count: count() })
        .from(agentInstance)
  ...
  return {
    ...
    artifacts: {
      ...
      byStatus: artifactByStatus.map((r) => ({ key: r.key, count: Number(r.count) })),
      byKind: artifactByKind.map((r) => ({ key: r.key, count: Number(r.count) })),
    },
    workflowRuns: {
      ...
      byStatus: runByStatus.map((r) => ({ key: r.key, count: Number(r.count) })),
