...
describe("relaunchInstanceIfNeeded", () => {
  ...
  it("leaves a routable instance with an active session untouched", async () => {
    const db = makeMockDb();
    db.query.agentInstance.findFirst = mock(() =>
      Promise.resolve(coldInstance({ status: "running", sessionId: "ses-1" })),
    );
    ...
    const sessionService = { ...mockSessionService, launchSession, endSession };
  ...
  it("tears down a wedged session and relaunches when routable but the bound session is ended", async () => {
    const db = makeMockDb();
    db.query.agentInstance.findFirst = mock(() =>
      Promise.resolve(
        coldInstance({ status: "running", sessionId: "ses-dead" }),
      ),
    );
    db.query.agentSession.findFirst = mock(() =>
