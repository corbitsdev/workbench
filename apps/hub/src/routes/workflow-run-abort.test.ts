...
const loadRunRecord = mock(async (_db: unknown, runId: string) => records.get(runId) ?? null);
...
function abortApp(db: ReturnType<typeof makeDb>) {
  ...
  app.post('/workflow-exec/records/abort-active', abortActiveRunsHandler({ db }));
  return app;
}
...
describe('abortRunHandler', () => {
  test('marks an existing run terminal (failed + aborted error)', async () => {
    ...
    const res = await abortApp(makeDb([])).request('/workflow-exec/records/wfr_1', {
      method: 'DELETE',
    });
    ...
    expect(saved[0]).toEqual({ runId: 'wfr_1', status: 'failed', error: ABORTED_ERROR });
  });

  test('404 for an unknown run, no save', async () => {
    const res = await abortApp(makeDb([])).request('/workflow-exec/records/wfr_ghost', {
      method: 'DELETE',
    });
...
describe('abortActiveRunsHandler', () => {
  ...
  test('returns 0 when no active runs match', async () => {
    seed({ runId: 'wfr_done', status: 'completed' });
    const res = await abortApp(makeDb([])).request('/workflow-exec/records/abort-active', {
      method: 'POST',
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as { aborted: number; ids: string[] };
...
describe('operator gate on abort routes', () => {
  ...
  function gatedApp(grants: GrantRule[]) {
    ...
    app.post('/workflow-exec/records/abort-active', guard, abortActiveRunsHandler({ db }));
    return app;
  }
  ...
  test('denies (403) a non-operator bulk-abort; no save', async () => {
    seed({ runId: 'wfr_run', status: 'running' });
    const res = await gatedApp([]).request('/workflow-exec/records/abort-active', {
      method: 'POST',
    });
    expect(res.status).toBe(403);
    expect(saved).toHaveLength(0);
...
describe('OpenAPI discovery', () => {
  test('both abort routes appear under the Workflows tag', async () => {
    ...
    expect(body.paths['/api/v1/workflow-exec/records/{runId}']?.delete?.tags).toEqual([
      'Workflows',
    ]);
    expect(body.paths['/api/v1/workflow-exec/records/abort-active']?.post?.tags).toEqual([
      'Workflows',
    ]);
  });
});
