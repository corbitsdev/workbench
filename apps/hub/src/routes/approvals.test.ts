...
import { createApprovalsRouter, createInternalApprovalsRouter } from './approvals';
...
describe('POST /tenants/:tenantId/approvals/:approvalId/reject', () => {
  ...
  it('rejects a pending approval and persists the rejection message', async () => {
    ...
    db.update = mock(() => ({
      set: mock((vals: Record<string, unknown>) => {
        setValues.push(vals);
        return { where: mock(() => ({ returning: mock(() => Promise.resolve([rejectedRow])) })) };
      }),
    }));
    ...
    expect(setValues[0]).toMatchObject({ status: 'rejected', message: 'Not allowed' });
  });
  ...
  it('stores a null message when the reject body omits one', async () => {
    ...
    const app = buildApp(db);
    const res = await app.fetch(
      new Request('http://localhost/tenants/tenant-1/approvals/apr-1/reject', { method: 'POST' })
    );
    expect(res.status).toBe(200);
  ...
  it('returns 404 when the approval to reject does not exist', async () => {
    ...
    const app = buildApp(db);
    const res = await app.fetch(
      new Request('http://localhost/tenants/tenant-1/approvals/apr-missing/reject', {
        method: 'POST',
      })
    );
    expect(res.status).toBe(404);
  ...
  it('returns 403 when rejecting an approval owned by another principal', async () => {
    ...
    db.select = mock(() => ({
      from: mock(() => ({
        where: mock(() => ({
          limit: mock(() => Promise.resolve([{ id: 'apr-1', principalId: 'prn-other' }])),
        })),
      })),
    }));
  ...
  it('returns 409 when the approval was already resolved', async () => {
    ...
    db.select = mock(() => ({
      from: mock(() => ({
        where: mock(() => ({
          limit: mock(() => Promise.resolve([{ id: 'apr-1', principalId: PRINCIPAL.id }])),
        })),
      })),
    }));
...
describe('createInternalApprovalsRouter', () => {
  ...
  it('GET /approvals/:id requires the tenantId query param', async () => {
    const app = buildInternalApp(makeMockDb());
    const res = await app.fetch(
      new Request('http://localhost/approvals/apr-1', authed({ method: 'GET' }))
    );
    expect(res.status).toBe(400);
    const json = await res.json();
  ...
  it('GET /approvals/:id returns 404 when not found', async () => {
    ...
    const app = buildInternalApp(db);
    const res = await app.fetch(
      new Request('http://localhost/approvals/apr-1?tenantId=tenant-1', authed({ method: 'GET' }))
    );
    expect(res.status).toBe(404);
  });
  ...
  it('GET /approvals/:id returns the formatted approval when found', async () => {
    ...
    db.select = mock(() => ({
      from: mock(() => ({ where: mock(() => ({ limit: mock(() => Promise.resolve([row])) })) })),
    }));
    const app = buildInternalApp(db);
    const res = await app.fetch(
      new Request('http://localhost/approvals/apr-1?tenantId=tenant-1', authed({ method: 'GET' }))
