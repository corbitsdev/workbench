...
function makeRouter(opts: { toolPackages: unknown; agentFound?: boolean; blobBytes?: number }) {
  const db = {
    query: {
      agent: {
        findFirst: async () =>
          opts.agentFound === false ? undefined : { id: 'a1', toolPackages: opts.toolPackages },
      },
    },
  ...
  return createToolManifestRouter(db, 'sidecar-token', assetService, fakeListAssets, fakeResolver);
}
...
describe('POST /tools/manifest', () => {
  ...
  test('returns 404 when the agent row is missing', async () => {
    const res = await post(makeRouter({ toolPackages: [], agentFound: false }), req);
  ...
  test('resolves the manifest and serves each asset tarball with its mount', async () => {
    ...
    expect(body.manifest.entries.map((e) => e.name)).toEqual(['@workbench/tools-granola']);
    ...
    expect(Buffer.from(tarball.bytesBase64, 'base64').toString()).toBe('TARBALL-BYTES');
  });
