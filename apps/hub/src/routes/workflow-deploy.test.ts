...
const { createWorkflowDeployRouter, createWorkflowDeployGrantGuard, deleteWorkflowHandler } =
...
function makeDeps(opts: {
  ...
  } as unknown as Parameters<typeof createWorkflowDeployRouter>[0]['workflowDeployService'];

  const sessionService = {
    endSession: async (address: string) => {
      opts.endedSessions?.push(address);
    },
  } as unknown as Parameters<typeof createWorkflowDeployRouter>[0]['sessionService'];
...
function deleteApp(rec: TeardownRecorder) {
  ...
  app.delete('/workflows/:deploymentId', deleteWorkflowHandler(makeTeardownDeps(rec)));
  return app;
}

function del(app: ReturnType<typeof deleteApp>, deploymentId: string): Promise<Response> {
  return Promise.resolve(app.request(`/workflows/${deploymentId}`, { method: 'DELETE' }));
}
