...
export function createAgentProvisioningRouter(
  ...
  app.post(
    ...
    async (c) => {
      ...
      // Routability alone is not health: after a sidecar restart the agent can be
      // restored from sidecar-persisted state while its bound session is ENDED (the
      // disconnect reconciler ended the orphaned session). Such a "wedged" agent has
      // no active turn and silently drops tool parts and reply mail. Only treat the
      // instance as live when it is routable AND its bound session is still active;
      // otherwise tear the wedged agent down and fall through to a clean launch. (CL-2304)
      if (sidecarRouter.getRoutableAddresses().includes(instance.address)) {
        ...
        if (!boundSessionEnded) {
          // Healthy (or a launch race with no recorded session): idempotent grant
          // refresh, no restart. sendGrantsUpdate does not churn the agent.
