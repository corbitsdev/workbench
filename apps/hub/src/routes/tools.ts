...
const ToolDetailResponse = type({
  tool: ToolSummary.merge({ inputSchema: "unknown" }),
});
...
export function createToolsRouter(
  ...
  router.get(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": { schema: resolver(ToolDetailResponse) },
          },
        },
        403: {
          description: "Caller is not a member of the requested tenant",
    ...
    async (c) => {
      ...
      const toolDetail = await getAvailableToolDetail(
        db,
        context.tenantId,
        toolName,
      );
