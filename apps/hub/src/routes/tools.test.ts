...
const listAvailableToolSummaries = mock(async (_db: unknown, _tenantId: string) => [
  ...
  { name: 'list_artifacts', providerName: 'workbench', description: 'List artifacts.' },
]);
...
const resolveToolVersions = mock(
  async (_db: unknown, _tenantId: string, _toolNames: string[], _assetService: unknown) =>
...
describe('createToolsRouter', () => {
  ...
  it('includes resolved registry version on the list endpoint', async () => {
    ...
    const res = await call('/tools');
    expect(res.status).toBe(200);
    const body = (await res.json()) as { tools: Array<{ name: string; version: string | null }> };
  ...
  it('returns version: null when no registry version resolves', async () => {
    ...
    resolveToolVersions.mockImplementation(async () => new Map([['attio_query_records', '0.1.7']]));
  });
  ...
  it('returns version: null on the detail endpoint for a hub-backed tool', async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { tool: { name: string; version: string | null } };
