...
import { readMemberPreferences, mergeMemberPreferences } from '../lib/member-preferences';
...
export function createMePreferencesRouter(db: HubDb): Hono<{ Variables: { userId: string } }> {
  ...
  app.get(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          description: 'The caller preferences (empty object when none are set)',
          content: { 'application/json': { schema: resolver(MemberPreferences) } },
        },
      },
    }),
    async (c) => {
      ...
      const prefs = await readMemberPreferences(db, member.tenantId, member.principalId);
      return c.json(prefs);
    }
  );
  ...
  app.patch(
    ...
    describeRoute({
      ...
      requestBody: {
        content: { 'application/json': { schema: requestBodySchema(MemberPreferences) } },
      },
      responses: {
        200: {
    ...
    async (c) => {
      ...
      const merged = await mergeMemberPreferences(db, member.tenantId, member.principalId, patch);
