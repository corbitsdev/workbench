...
export function createMyraThreadsRouter(
  ...
  app.post(
    ...
    describeRoute({
      ...
      requestBody: {
        content: { 'application/json': { schema: requestBodySchema(CreateMyraThreadBody) } },
      },
      responses: {
        201: {
    ...
    async (c) => {
      const userId = c.get('userId');
      const body = CreateMyraThreadBody.assert(await c.req.json().catch(() => ({})));
      const ctx = await resolveMyraThreadContext(hubDb, userId);
      if (!ctx) {
        return c.json({ error: 'Member not provisioned' }, 503);
      }
  ...
  app.patch(
    ...
    describeRoute({
      ...
      requestBody: {
        content: { 'application/json': { schema: requestBodySchema(RenameMyraThreadBody) } },
      },
      responses: {
        200: {
          description: 'Thread renamed',
          content: { 'application/json': { schema: resolver(type({ thread: MyraThread })) } },
        },
      },
    }),
    async (c) => {
  ...
  app.post(
    ...
    describeRoute({
      ...
      parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
      requestBody: {
        content: { 'application/json': { schema: requestBodySchema(TitleMyraThreadBody) } },
      },
      responses: {
        200: {
          description: 'Thread titled, or no-op (thread null) when titling was skipped or failed',
          content: {
            'application/json': {
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: { 'application/json': { schema: resolver(type({ deleted: 'true' })) } },
        },
      },
    }),
    async (c) => {
