...
const runRows = new Map<
  string,
  { id: string; tenantId: string; principalId: string }
>();
const principals = new Map<
  string,
  { id: string; tenantId: string; refId: string }
>();
...
const getSkillAsset = mock(
  async (_db: unknown, _viewer: unknown, assetId: string) =>
...
const { createInternalWorkflowSkillsRouter } = await import(
  "./workflow-skills"
);
...
describe("POST /workflow-skills/resolve", () => {
  test("resolves selected skill IDs for the run owner", async () => {
    ...
    skillAssets.set("skill_1", {
      id: "skill_1",
      name: "hammy",
      displayName: "Hammy",
    });
    skillFiles.set("skill_1", [
  ...
  test("requires the sidecar bearer token", async () => {
    const res = await post(
      { tenantId: "ten_1", runId: "run_1", skillIds: [] },
      "wrong",
    );
