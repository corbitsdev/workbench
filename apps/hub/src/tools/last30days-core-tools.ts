  ...
  return {
    ...
    handler: async (args, _signal) => {
      ...
      if (typeof (args as Record<string, unknown>)._raw === 'string') {
        const parsed: unknown = JSON.parse((args as Record<string, unknown>)._raw as string);
...
function createLast30daysCoreReportTool(): AgentTool {
  return {
    ...
    handler: async (args, _signal) => {
      ...
      const days = typeof effectiveArgs.days === 'number' ? effectiveArgs.days : 30;
      const topK = typeof effectiveArgs.topK === 'number' ? effectiveArgs.topK : 20;
