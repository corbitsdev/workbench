...
describe("LIST_AGENTS_DEFINITION", () => {
  it("takes no required arguments and is registered under list_agents", () => {
    ...
    expect(LIST_AGENTS_HUB_TOOLS.list_agents?.definition).toBe(
      LIST_AGENTS_DEFINITION,
    );
  });
});
...
describe("resolveOwnedInstanceIds", () => {
  ...
  it("uses explicit member principals without resolving the caller", async () => {
    ...
    expect(await resolveOwnedInstanceIds(db, CONTEXT, ["prn_other"])).toEqual([
      "ins_a",
      "ins_b",
    ]);
  });
  ...
  it("scopes the caller lookup to the tenant and calling principal", async () => {
    ...
    expect(hasEq(wheres[0]!, intxSchema.agentInstance.tenantId, "tnt_1")).toBe(
      true,
    );
    expect(
      hasEq(
        wheres[0]!,
        intxSchema.agentInstance.principalId,
        "prn_caller_agent",
      ),
    ).toBe(true);
  });
...
describe("list_agents handler", () => {
  it("returns the caller's owned running agents filtered by instance ids", async () => {
    ...
    expect(JSON.parse((await handler(db)({})) as string)).toEqual({
      agents: finalRows,
    });
  ...
  it("lists another operator agents when given member principals", async () => {
    ...
    expect(
      JSON.parse((await handler(db)({ principals: ["prn_other"] })) as string),
    ).toEqual({
      agents: finalRows,
    });
  });
  ...
  it("rejects an unknown status", async () => {
    const { db } = makeDb([[{ id: "ins_caller" }]]);
    await expect(handler(db)({ status: "banana" })).rejects.toThrow(
