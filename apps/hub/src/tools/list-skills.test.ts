...
const listSkills = mock(
  async (_db: unknown, viewer: { userId: string }) => visibleByUser.get(viewer.userId) ?? []
);
...
const getSkillContent = mock(
  async (_repoStore: unknown, assetId: string) => filesByAsset.get(assetId) ?? []
);

mock.module('../services/skill-library', () => ({ listSkills, getSkillAsset, getSkillContent }));

const { createSkillTools } = await import('./list-skills');

const principals = new Map<string, { id: string; tenantId: string; refId: string } | null>();
...
function tool(name: string) {
  ...
  if (!found || found.kind !== 'string') throw new Error(`missing tool ${name}`);
  return found;
}
...
describe('list_skills', () => {
  test('returns the index shape only — no skill body', async () => {
    ...
    const result = await tool('list_skills').handler({}, new AbortController().signal);
  ...
  test('fails closed to an empty list when the principal is not a resolvable user', async () => {
    ...
    visibleByUser.set('usr_1', [{ id: 'a1', name: 'deck', displayName: 'Deck' }]);
    const result = await tool('list_skills').handler({}, new AbortController().signal);
...
describe('search_skills', () => {
  ...
  test('rejects a missing query', async () => {
    await expect(tool('search_skills').handler({}, new AbortController().signal)).rejects.toThrow(
      'search_skills'
    );
  });
});
...
describe('load_skill', () => {
  test('returns the stripped body plus sibling files', async () => {
    assetByIdForUser.set('usr_1:a1', { id: 'a1', name: 'deck', displayName: 'Deck' });
    filesByAsset.set('a1', [
      { path: 'SKILL.md', content: 'Build a deck.' },
      { path: 'examples/sample.md', content: '# sample' },
    ]);
    const result = await tool('load_skill').handler({ id: 'a1' }, new AbortController().signal);
    expect(JSON.parse(result)).toEqual({
      id: 'a1',
      name: 'deck',
      displayName: 'Deck',
  ...
  test('truncates oversized content and flags binary siblings, with a notice', async () => {
    ...
    const parsed = JSON.parse(
      await tool('load_skill').handler({ id: 'a1' }, new AbortController().signal)
    );
    expect(parsed.body.length).toBeLessThan(huge.length);
    expect(parsed.notice).toBeDefined();
    const binary = parsed.files.find((f: { path: string }) => f.path === 'logo.png');
  ...
  test('errors when the skill is not visible to the viewer', async () => {
    ...
    await expect(
      tool('load_skill').handler({ id: 'private_to_someone_else' }, new AbortController().signal)
