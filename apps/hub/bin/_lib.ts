...
export async function resolveTargetTenant(opts: {
  ...
  const globalSlug = opts.globalSlug ?? process.env['GLOBAL_TENANT_SLUG'] ?? 'abklabs';
  const res = await api(opts.base, 'GET', '/api/me/principals', undefined, opts.cookies);
  if (res.status !== 200) {
    throw new Error(`resolveTargetTenant: /api/me/principals failed: ${String(res.status)}`);
  }
  const principals = parsePrincipals(res.data);
  if (principals.length === 0) {
    throw new Error('resolveTargetTenant: caller is not a principal of any tenant');
  }
  ...
  const flagSlug =
    readTenantFlag(opts.argv) ?? (opts.envVar ? process.env[opts.envVar] : undefined) ?? undefined;

  if (flagSlug !== undefined && flagSlug !== '') {
    const match = principals.find((p) => p.tenantSlug === flagSlug);
    if (!match) {
      throw new Error(`resolveTargetTenant: caller is not a principal of tenant "${flagSlug}"`);
    }
    return toTarget(match);
  }

  const globalMatch = principals.find((p) => p.tenantSlug === globalSlug) ?? principals[0];
  if (!globalMatch) {
  ...
  const answer = prompt(`Tenant [1-${principals.length}, default ${globalMatch.tenantSlug}]:`);
  ...
  if (!chosen) throw new Error(`resolveTargetTenant: invalid selection "${answer}"`);
