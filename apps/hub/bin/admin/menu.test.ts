...
describe('operationInputs', () => {
  ...
  it('derives a required path param for a DELETE', async () => {
    ...
    expect(inputs).toEqual([{ name: 'id', kind: 'path', required: true, valueType: 'string' }]);
  });
...
describe('findListOperation', () => {
  ...
  it('picks the collection GET (fewest path params)', () => {
    expect(findListOperation(ops, 'Providers')?.path).toBe('/api/tenants/{tenantId}/providers');
  });

  it('returns undefined when the tag has no GET', () => {
    expect(findListOperation([{ tag: 'X', method: 'post', path: '/x' }], 'X')).toBeUndefined();
  });
...
describe('itemLabel', () => {
  ...
  it('falls back to email then to the bare id', () => {
    expect(itemLabel({ id: 'p1', email: 'a@b.com' }).label).toBe('a@b.com [p1]');
