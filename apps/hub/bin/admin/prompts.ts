...
export function parseSingleSelection(answer: string | null, length: number): number | null {
...
export function selectOne<T>(label: string, items: T[], render: (item: T) => string): T | null {
  if (items.length === 0) return null;
  console.log(`\n${label}`);
  items.forEach((item, i) => console.log(`  ${i + 1}) ${render(item)}`));
  const index = parseSingleSelection(prompt(`Select [1-${items.length}]:`), items.length);
