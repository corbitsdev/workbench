...
async function pickReference(
  ...
  for (;;) {
    ...
    const rows = items.map((item) => itemLabel(item)).filter((r) => r.id !== undefined);
    ...
    if (nextCursor !== undefined) choices.push({ label: 'Load more…', value: 'more' });
    ...
    const chosen = selectOne(`${input.name} — pick a ${singular(tag)}:`, choices, (c) => c.label);
...
async function promptInput(
  ...
  if (input.valueType === 'boolean') {
    const chosen = selectOne(`${input.name}${opt}${desc}:`, ['true', 'false'], (v) => v);
    if (chosen !== null) return chosen;
    return input.required ? null : SKIP;
  }
...
async function runSingleLocalAction(action: LocalAction, tenant: TenantChoice): Promise<void> {
...
async function runOnce(client: HubClient, tenant: TenantChoice): Promise<void> {
  ...
  const chosen = selectOne(
    'Resource:',
    resources,
    (r) => `${r.name} (${r.localActions.length + (r.specGroup?.operations.length ?? 0)})`
  );
  if (!chosen) return;
