...
export function mergeCookies(cookies: CookieJar, setCookies: readonly string[]): CookieJar {
  const next = [...cookies];
...
export function resolveCredentialBinding(
  ...
  if (!cred) {
    throw new Error(`credential "${credentialName}" not found — run seed-credentials first`);
  }
...
async function seedCatalog(tenantId: string, cookies: CookieJar): Promise<void> {
  const credsRes = await api('GET', `/api/tenants/${tenantId}/credentials`, undefined, cookies);
  if (credsRes.status !== 200) fail('list credentials', credsRes.status, credsRes.data);
  ...
  if (provRes.status !== 200) fail('list catalog providers', provRes.status, provRes.data);
  ...
  for (const provider of FULL_CATALOG.providers) {
    ...
    if (res.status !== 201) fail(`create provider (${provider.name})`, res.status, res.data);
  ...
  const modelRes = await api('GET', `/api/tenants/${tenantId}/catalog/models`, undefined, cookies);
  if (modelRes.status !== 200) fail('list catalog models', modelRes.status, modelRes.data);
  for (const existing of listData<{ id: string; canonicalName: string }>(modelRes.data)) {
  ...
  for (const model of FULL_CATALOG.models) {
    ...
    if (res.status !== 201) fail(`create model (${model.canonicalName})`, res.status, res.data);
  ...
  const offRes = await api('GET', `/api/tenants/${tenantId}/catalog/offerings`, undefined, cookies);
  if (offRes.status !== 200) fail('list catalog offerings', offRes.status, offRes.data);
  ...
  for (const offering of FULL_CATALOG.offerings) {
    ...
    if (res.status !== 201)
      fail(`create offering (${offering.model}/${offering.provider})`, res.status, res.data);
