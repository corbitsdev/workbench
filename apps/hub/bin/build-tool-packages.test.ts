...
import { buildToolPackages, TOOL_PACKAGES, type BuiltToolPackage } from './build-tool-packages';

const scratch = path.join(os.tmpdir(), `wb-tool-packages-${process.pid}`);
const spec = TOOL_PACKAGES.find((s) => s.name === '@workbench/tools-hackernews');
if (spec === undefined) {
...
async function extractTarball(tarballPath: string, into: string): Promise<string> {
...
describe('build-tool-packages', () => {
  ...
  test('the bundled entry is a self-contained ESM file with no bare @intx import', async () => {
    ...
    const bundled = await fs.readFile(path.join(pkgRoot, 'dist', 'interchange-tools.js'), 'utf-8');
