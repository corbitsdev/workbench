...
beforeEach(async () => {
  ...
  await fs.writeFile(
    path.join(fromDir, "@workbench-tools-hackernews-0.1.0.tgz"),
    "fake-bytes-a",
  );
  await fs.writeFile(
    path.join(fromDir, "@workbench-tools-exa-0.1.0.tgz"),
    "fake-bytes-b",
  );
});
...
function installFetchStub(): void {
  globalThis.fetch = (async (
    ...
    if (url.endsWith("/api/auth/sign-up/email"))
      return json(422, { code: "USER_ALREADY_EXISTS" });
    ...
    if (url.includes("/tarballs/"))
      return json(200, { commit: "c1", integrity: "sha512-xyz" });
...
describe("publishToolPackages", () => {
  test("creates the registry asset and PUTs every tarball", async () => {
    ...
    expect(
      summaries.every((s) => s.commit === "c1" && s.integrity === "sha512-xyz"),
    ).toBe(true);
    ...
    expect(
      puts.every((c) =>
        c.url.includes("/api/tenants/t1/assets/ast_reg/tarballs/"),
      ),
    ).toBe(true);
  });
  ...
  test("reuses an existing registry asset instead of creating one", async () => {
    ...
    expect(calls.some((c) => c.url.includes("ast_existing/tarballs/"))).toBe(
      true,
    );
  });
  ...
  test("uses a bare session token and skips admin sign-in", async () => {
    ...
    const principalsCall = calls.find((c) =>
      c.url.includes("/api/me/principals"),
    );
    expect(principalsCall?.cookie).toContain(
