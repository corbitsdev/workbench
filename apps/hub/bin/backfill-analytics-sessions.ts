...
const tenantSlug =
  tenantSlugIdx >= 0 ? args[tenantSlugIdx + 1] : process.env["WORKBENCH_SLUG"];
...
async function main(): Promise<void> {
  if (!tenantSlug) {
    console.error(
      "[backfill-analytics-sessions] Pass --tenant <slug> or set WORKBENCH_SLUG",
    );
    process.exit(1);
  }
  ...
  if (!tenant) {
    console.error(
      `[backfill-analytics-sessions] Unknown tenant slug: ${tenantSlug}`,
    );
    process.exit(1);
  }
  console.log(
    `[backfill-analytics-sessions] Tenant ${tenant.slug} (${tenant.id})`,
  );
  ...
  if (rows.length === 0) {
    console.log(
      "[backfill-analytics-sessions] No supervisor instances need backfill.",
    );
    await sqlClient.end();
    return;
  }
  ...
  console.log(
    `[backfill-analytics-sessions] Found ${rows.length} supervisor instance(s):`,
  );
  ...
  for (const row of rows) {
    ...
    console.log(
      `[backfill-analytics-sessions] ${row.instanceId} → ${sessionId}`,
    );
  }
