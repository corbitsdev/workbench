...
test("the step pins GitHub reach and sets an explicit turn timeout", () => {
  ...
  expect(primitive.agent.toolPackagePins).toEqual(CODE_REVIEW_TOOL_PACKAGE_PINS);
...
test("the prompt commits to reading the diff, posting once, and never merging", () => {
  ...
  expect(CODE_REVIEW_SYSTEM_PROMPT).toContain("never approve, request changes, or merge");
});

test("the webhook input template names the pull request from the event", () => {
  expect(CODE_REVIEW_WEBHOOK_INPUT_TEMPLATE).toContain("{{pull_request.html_url}}");
});

test("the definition survives the workflow-asset round trip", () => {
  const definition = buildCodeReviewWorkflow(INPUT);
  const round = JSON.parse(serializeCodeReviewWorkflow(definition)) as WorkflowDefinition;
...
test("a blank trigger address or a bad timeout is refused at build time", () => {
  expect(() => buildCodeReviewWorkflow({ ...INPUT, triggerAddress: "" })).toThrow(/triggerAddress/);
  expect(() => buildCodeReviewWorkflow({ ...INPUT, turnTimeoutMs: 0 })).toThrow(/turnTimeoutMs/);
});
