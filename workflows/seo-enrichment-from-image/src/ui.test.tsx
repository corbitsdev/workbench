...
function makeEnrichOutput(rows: { id: string; field: string; value: string; note?: string }[]) {
...
const sampleEnrichOutput = makeEnrichOutput([
  { id: 'title', field: 'title', value: 'Best Widget Ever', note: 'strong hook' },
  { id: 'meta_description', field: 'meta_description', value: 'Buy the best widget.' },
]);
...
describe('Panel', () => {
  ...
  it('fires onSignal with intake payload when both URLs are filled and submitted', () => {
    ...
    expect(onSignal.mock.calls[0]).toEqual([
      'intake',
      { imageUrl: 'https://example.com/product.jpg', pageUrl: 'https://example.com/page' },
    ]);
  });

  it('shows the in-flight message while enrich is running', () => {
    renderPanel({ state: makeState({ intake: 'completed', enrich: 'in-flight' }) });
  ...
  it('shows checkboxes for each row, pre-checked, in review step', () => {
    ...
    const titleCheckbox = screen.getByLabelText('Select title') as HTMLInputElement;
    const metaCheckbox = screen.getByLabelText('Select meta_description') as HTMLInputElement;
  ...
  it('fires onSignal with row-selection payload containing only checked ids', () => {
    ...
    expect(onSignal).toHaveBeenCalledTimes(1);
    expect(onSignal.mock.calls[0]).toEqual(['row-selection', { selectedIds: ['title'] }]);
  });
