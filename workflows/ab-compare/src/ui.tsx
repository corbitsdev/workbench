...
// `persist` is a deterministic `artifact_create` tool step: its output is the
// agent-runtime `ToolResult` envelope whose `content` is the hub handler's
// JSON string ({ artifactId, title, kind, version }).
...
function PhaseBadge({ phase }: { phase: StepPhase | undefined }) {
  ...
  if (phase === "failed" || phase === "cancelled") {
    return (
      <span className="text-xs font-medium text-orange-deep">{phase}</span>
    );
  }
  if (phase === "awaiting-signal") {
    return (
      <span className="text-xs font-medium text-orange">Awaiting review</span>
    );
  }
...
function InputSection({
  ...
  const canSubmit =
    connected && content.trim().length > 0 && prompt.trim().length > 0;
      ...
      <SectionCard title="Execute — provider branches" phase={phase}>
        {phase === "completed" ? (
          <p className="text-sm text-orange">
            Couldn’t read the provider outputs for this step.
          </p>
        ) : (
          ...
          {branches.map((branch, index) => (
            ...
            >
              <div className="mb-1 flex items-center justify-between gap-2">
                ...
                {branch.model ? (
                  <span className="text-xs text-text-3">{branch.model}</span>
                ) : null}
              </div>
              {branch.output ? (
...
function CompareSection({
  ...
  if (parsed instanceof type.errors) {
    return (
      <SectionCard title="Compare — blind ranking" phase={phase}>
        {phase === "completed" ? (
          <p className="text-sm text-orange">
            Couldn’t read the ranking for this step.
          </p>
        ) : (
  ...
  return (
    <SectionCard title="Compare — blind ranking" phase={phase}>
      {parsed.summary ? (
        <p className="mb-3 text-sm text-text-2">{parsed.summary}</p>
      ) : null}
      {ranking.length === 0 ? (
        <p className="text-sm text-text-3">No ranking yet.</p>
      ) : (
      ...
      {phase === "awaiting-signal" ? (
        <div className="space-y-3">
          <p className="text-sm text-text-2">
            Review the blind ranking above. Approve to persist the comparison
            results.
          </p>
          <Button
            variant="primary"
          ...
          {connected ? null : (
            <p className="text-xs text-text-3">
              Reconnecting — approval is unavailable.
            </p>
          )}
        </div>
      ) : phase === "completed" ? (
        ...
        <p className="text-sm text-text-3">
          Waiting for the comparison to finish.
        </p>
      )}
    </SectionCard>
  );
...
function PersistSection({
  ...
  if (artifact === undefined) {
    return (
      <SectionCard title="Persist — saved artifact" phase={phase}>
        {phase === "completed" ? (
          <p className="text-sm text-orange">
            Couldn’t read the saved artifact for this step.
          </p>
        ) : (
          <p className="text-sm text-text-3">No artifact saved yet.</p>
        )}
      </SectionCard>
    );
  }
    ...
    <div className="flex h-full flex-col bg-surface">
      <header className="flex items-center justify-between gap-3 border-b border-border px-6 py-4">
        <div>
          ...
          <p className="text-xs text-text-3">
            Blind ranking across provider variants
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-6">
        ...
        />
        <ExecuteSection
          phase={stepPhase(state, "execute")}
          output={stepOutputs.execute}
        />
        <CompareSection
          phase={stepPhase(state, "compare")}
