...
describe("ab-compare native workflow", () => {
  ...
  test("execute is an inline-inference step receiving the input signal output", () => {
    ...
    expect(execute.agent.tags?.[STEP_KIND_TAG]).toBe(INLINE_INFERENCE_KIND);
    expect(execute.agent.tags?.[STEP_TOOL_TAG]).toBeUndefined();
    expect(execute.agent.capabilities).toEqual([]);
    expect(execute.agent.inference.sources).toEqual([]);
    expect(execute.agent.systemPrompt.length).toBeGreaterThan(0);
    expect(execute.input).toEqual({ from: "steps.input.output" });
    expect(execute.after).toContain("input");
  });

  test("compare is an inline-inference step receiving the execute step output", () => {
