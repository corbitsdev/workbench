...
test("the definition binds the web-search package's exa handle to the tenant's connection", () => {
  const definition = buildDiligenceBriefWorkflow(INPUT);
  expect(definition.credentialBindings).toEqual(DILIGENCE_BRIEF_CREDENTIAL_BINDINGS);
...
test("the prompt commits to checking memory before the web, and to the two wired sources", () => {
  ...
  expect(prompt.indexOf("memory_search")).toBeLessThan(prompt.indexOf("web_search"));
...
test("the definition survives the workflow-asset round trip", () => {
  const definition = buildDiligenceBriefWorkflow(INPUT);
  const round = JSON.parse(serializeDiligenceBriefWorkflow(definition)) as WorkflowDefinition;
...
test("a blank trigger address or a bad timeout is refused at build time", () => {
  expect(() => buildDiligenceBriefWorkflow({ ...INPUT, triggerAddress: "" })).toThrow(
    /triggerAddress/,
  );
  expect(() => buildDiligenceBriefWorkflow({ ...INPUT, turnTimeoutMs: 0 })).toThrow(
    /turnTimeoutMs/,
  );
});
