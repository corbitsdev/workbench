// The channel-digest workflow: a single-step, mail-triggered definition
// meant to be deployed against a channel's own timeline address so its
// reply posts a deterministic summary line — the trigger's own message
// count and timestamp — back into the channel, the same way a channel
// host's reply becomes a channel mail post (see
// `packages/chat/src/channel-workflow.ts` and `platform-adapter.ts`'s
// `connectorReplyContent` handling). The digest line itself is
// computed by the caller that sends the trigger mail (a scheduler, in
// the common case) and carried in the trigger body; this definition's
// only job is to relay that already-deterministic line back out
// verbatim, so nothing about the reply's content is left to the model.
//
...
// and channel-mail-posting paths stay alive at zero cost, without
...
export const CHANNEL_DIGEST_WORKFLOW_ID = "wf_channel_digest";
export const CHANNEL_DIGEST_STEP_ID = "channel-digest";

export const CHANNEL_DIGEST_SYSTEM_PROMPT =
  "You post a single deterministic summary line into a channel. The " +
 ...
 * workflow, ordinarily a channel's own mail address — so a definition
 * built here is per-deployment by construction.
 */
export interface ChannelDigestWorkflowInput {
...
/**
 * Builds the channel-digest definition. Exactly one step, matching the
 ...
 */
export function buildChannelDigestWorkflow(
  input: ChannelDigestWorkflowInput,
): WorkflowDefinition {
  if (input.triggerAddress === "") {
    throw new Error(
      "buildChannelDigestWorkflow requires a non-empty triggerAddress",
    );
  }
  if (!Number.isInteger(input.turnTimeoutMs) || input.turnTimeoutMs <= 0) {
    throw new Error(
      "buildChannelDigestWorkflow requires turnTimeoutMs to be a positive integer",
    );
  }
      ...
      "channel-digest": step({
        agent: defineAgent({
          ...
          description:
            ...
            "back into the channel it is deployed against",
 ...
 */
export function serializeChannelDigestWorkflow(
