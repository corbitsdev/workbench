...
describe("databaseIsRequired", () => {
  ...
  test("is true when CI=true and the job is not a named ci.yml job", () => {
    process.env["CI"] = "true";
    unset("GITHUB_JOB");
    expect(databaseIsRequired()).toBe(true);
    process.env["GITHUB_JOB"] = "some-other-job";
    expect(databaseIsRequired()).toBe(true);
  });

  test("is false on every ci.yml job — none of them provision Postgres", () => {
    process.env["CI"] = "true";
    for (const job of ["setup", "lint", "typecheck", "structural", "unit"]) {
