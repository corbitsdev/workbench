  ...
  () => {
    test("provisioning a personal bench without a seed model reports bench_unseeded", async () => {
      ...
      const dataDir = await mkdtemp(
        path.join(tmpdir(), "e2e-smoke-onboarding-hub-data-"),
      );
      ...
      try {
        ...
        await hop(
          ...
          async () => {
            ...
            expect((res.data as { kind: string }).kind).toBe(
              "needs-onboarding",
            );
          },
        );
        ...
        await hop(
          ...
          async () => {
            ...
            const own = rows.find(
              (row) => row.tenantId === provisioned.tenantId,
            );
            if (own === undefined) {
              throw new Error(
        ...
        await hop(
          "re-provisioning the same account is idempotent",
          async () => {
            const res = await api(
              baseUrl,
              "POST",
