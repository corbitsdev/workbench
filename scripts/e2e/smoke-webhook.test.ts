// Smoke scenario 5/5: webhook trigger. A workflow is bound to
...
describe.skipIf(databaseUrl === undefined)("smoke: webhook trigger", () => {
  test("a signed webhook delivery launches a run against the bound trigger", async () => {
...

    {
      ...
      // hub (dropped its noop-inference mount) and never a real
      // model.
      ...
      await hop("the run's insights trace reads real spans once the run settles ", async () => {
      ...
      console.log(
        "smoke-webhook: a signed delivery to the public ingress route " +
          ...
          "and its insights trace reads back real spans.",
      );
    }
  }, 180_000);
});
