// Workbench's product-supplied connector set (CL-7384): every api-key
// connector and curated MCP preset this build actually ships, built from
// `@corbits/connections`' generic descriptor shape and route factories.
// `@corbits/connections` itself carries no default connector set — that
// would be exactly the kind of fallback this repo's ground rules
// forbid — so a caller (the hub, the web app) imports `CONNECTOR_REGISTRY`
// and `MCP_PRESETS` from here and passes them into the route factories
// that need one.
//
...
// `oauth` config shape `@corbits/connections`' `oauth-routes.ts` reads.
import {
