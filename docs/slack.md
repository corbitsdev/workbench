...
Workbench can be mentioned in Slack. A Slack channel gets bound, on first
contact, to one workbench channel with one agent already in it; every
mention or thread message in that Slack channel is posted into the bound
workbench channel exactly the way a person's message from the web app
...
Slack channel maps to one workbench channel with one agent. There is no
...
watching) in a given Slack channel, workbench:
...
2. Creates a new workbench channel, titled after the Slack channel's name,
   with the agent named by `SLACK_DEFAULT_AGENT_DEFINITION_ID` already
   invited into it.
3. Records the binding: this Slack channel now always resolves to that
   same workbench channel.

Every later message in that Slack channel reuses the same binding — no new
channel is created. The binding is scoped per bench (see `SLACK_WORKBENCH_TENANT_SLUG`
below), so two Slack workspaces configured against two different benches
can never see each other's bindings even if their Slack channel ids
happened to collide.

## Identity

Being in the Slack channel is the authorization: the Slack app is only
installed into channels a workspace admin chose, so anyone who can reach
...
are never resolved to a principal, and Slack guest/shared-channel accounts
...
- Every reply the agent posts into the bound workbench channel through its
  normal delivery path is also visible in the web app for that channel —
...
- `packages/slack-tag` — the mount composition: principal resolution,
  channel binding, and the dispatch flow. Nearly all of the behavior
