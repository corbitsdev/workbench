...
| **Myra**                 | —                   | The `assistant` workflow: every bench's default agent and the coordinator of its other agents. When asked for a capability the bench lacks, she checks for an existing teammate that already fits, then — reusing rather than duplicating — authors a new agent, workflow, or skill as code, deploys it, grants it only the access it needs, and adds it to the bench or a DM |
...
| **Timeline**             | —                   | A workbench's own message rows, read back in order, as the conversation record. Human copies of agent mail land in `@corbits/mailbox`; the timeline is the room's ordered record, not Interchange `session_mail`                                                                                                                                                              |
| **Participant**          | —                   | An address (human or agent) a workbench's settings list as able to post or be mentioned. An invited agent's address is the Interchange run address `prepareProvisionedDeployment` returned                                                                                                                                                                                    |
...
| **Reply bridge**         | —                   | Retired. An invited agent's reply reaches humans through the hub `persistMail` wrapper into `@corbits/mailbox`; dispatch and reply correlate by RFC 5322 `Message-ID` / `In-Reply-To` / `References`. There is no chat-owned event-stream hop and no `correlationId`                                                                                                          |
