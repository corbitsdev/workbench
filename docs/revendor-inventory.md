# CL-5514 re-vendor dry run: pin delta, breakage, retirement

Rehearsal only — nothing here is fixed beyond what `bun install` needed to
run the one allowed full typecheck honestly. This is the inventory that pass
produced, not a remediation.
...
| --- | --- | --- |
| Self-anchoring, `packages/folded-runs/launch.ts` + marker table + `reconnect.ts` | **CHANGED** | `packages/folded-runs/src/launch.ts:225-247` self-anchors, citing `vendor/intx/hub-api/src/routes/instances.ts` as its reference implementation — that route is now deleted upstream by `b5c1525b` "Retire the folded agent-instance launch route", and `e2732d32` collapsed all deployments onto the self-anchored-run model workbench already imitates. The concept converges with upstream but the cited reference file is gone and the marker table (`packages/folded-runs/src/schema.ts:1-13`) is still needed — upstream has no per-chat-launch concept at all. |
...
This pass reduces vendored *code volume* only modestly (no upstream package
