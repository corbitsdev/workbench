...
- **WORKBENCH-LOCAL (CL-3104) — hibernate undeploy flavor** (`src/ws/hub-link.ts`):
  ...
  The ack is withheld when the router cannot hibernate — acking would tell the
  hub the child is gone while it still runs. The reason constant has a
  byte-identical copy in `apps/hub/src/services/workflow-reconciler.ts` (the hub
  does not depend on this package); change both together. Guarded by
  `src/ws/hub-link-hibernate.test.ts`.
