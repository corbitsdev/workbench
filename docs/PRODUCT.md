...
**Approvals in chat.** When Myra or another agent requests a gated action (for example outbound mail), the approval card shows **real names** for principals and agents (members roster + agent instances), not opaque `prn_` / `ins_` ids or raw mailbox locals where a display name is known, plus a readable summary of the action and its arguments (humanized field labels, resolved names instead of raw ids). The card offers three resolutions: **Approve Once**, **Deny**, and **Auto Approve Always** — a caution-styled, two-step-confirm option that approves the pending action and stops future prompts for that specific tool. Those standing decisions are the member's own: an **"Always-approved actions"** section in Settings lists them and lets the member revoke one, restoring the prompt. Pending approvals also surface as indicators on the notification bell, the Inbox nav, and the sidebar chat list, so a parked request is visible without opening the thread.
