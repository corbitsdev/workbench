...
- `POST /api/internal/approvals` — human approval callback from sidecar (ask_principal tool); emits a `created` notification on the approvals event bus. The backing table is `workbench_approval` (renamed from the legacy `approval` at the 5a73d3cc pin bump: interchange's new approval-suspension runtime creates its own `approval` table, so `scripts/db-setup.ts` runs a pre-interchange reconcile that renames the workbench-shaped table + its PK, migration `0070` creates `workbench_approval` on fresh DBs, and a migrations guard test fails on any future table-name intersection between the two migration sources). Interchange's suspension rail itself (ask-effect authz → durable park → decision-driven resume) is activated by the `native-approvals` owner feature grant, **default off** — with it off no workbench grant resolves to `effect: "ask"` and the legacy poll path above is unchanged.

#### Native approval-suspension activation (`native-approvals` feature, deny-by-default)

- **Feature gate — deliberately NOT owner self-serve.** `native-approvals` is a resolvable feature name (`FEATURE_NAMES`) but is intentionally **absent from `FEATURE_GRANT_CATALOG`**, so it never renders on the Owner → Capabilities surface and the `PUT /owner/features/:name` toggle rejects it with 404 (guarded by an owner-route test). Its enable paths are staff-only: the global env override `NATIVE_APPROVALS_ENABLED` (`config.nativeApprovalsEnabled`, default off) or an admin-provisioned member-role `feature:native-approvals`/`enable` grant for a per-tenant staged rollout. It stays staff-only through the decision-surface soak (it does not return to the owner catalog in this increment). Off = legacy behavior exactly; on = ask-grant activation. `resolveAskToolNamesForTenant` reads this through the memoized `isFeatureEnabledForTenantCached` so a bulk grant reconcile issues one feature read, not N.
- **Ask-grant minting (`apps/hub/src/lib/tool-grants.ts`, `native-approvals.ts`).** When the feature is on for a tenant, `persistInstanceToolGrants` mints the approval-gated write set (`APPROVAL_GATED_TOOL_NAMES` from `@workbench/agents`) with `effect: "ask"` instead of `allow`; `resolveAskToolNamesForTenant` returns that set (empty when off). Read tools and internal-write exclusions stay `allow`. **Workflow step grants stay `allow`** (`buildStepGrantRules` is untouched) so unattended runs never block.
...
- **Decision surface (CL-3934) — mounted native routes + workbench list.** Interchange's `createApprovalRoutes` (mounted by `createApp`/`mountHubRoutes` at `/api/tenants/:tenantId/approvals`, behind `resolveTenant`) ships the working **approve/reject** halves but leaves its **list**/**get** a 501 stub, and `ApprovalStore` exposes no list read. The workbench therefore adds **only the read side**: `createNativeApprovalsRouter` (`apps/hub/src/routes/native-approvals.ts`) mounted on `hubApp` at `/api/tenants/:tenantId/native-approvals`, inheriting the same `resolveTenant` (active-membership; tenant-scoped, cross-tenant 403/404). Scope is tenant-membership during the staff-only soak — the reactor's suspend-time co-write leaves `toolDefinition`/`toolArguments` null (no per-member payload to leak yet); per-member narrowing is deferred with that upstream plumbing. The web `ReviewGate` (`apps/web/src/components/ReviewGate.tsx`) lists the native rail via `listNativeApprovals`, renders each with `NativeApprovalCard` (tool name from the snapshot when present, else the originating agent), and resolves through `approveNativeRequest`/`rejectNativeRequest` (Interchange's mounted routes). Legacy `workbench_approval` cards render unchanged; both rails coexist in the same gate during the soak.
- **Notifications (native rail on the existing SSE bus).** The shared `approvals-events` bus now carries native change signals: the `registerSignalCorrelation` **wrapper** in `apps/hub/src/index.ts` publishes `created` after the native co-write commits (tenant re-read from the deployment via `publishNativeApprovalCreated`), and an **outer middleware** ahead of the `hubApp` mount publishes `resolved` after a successful POST to the mounted approve/reject route (`publishNativeApprovalResolved`). Both live in `apps/hub/src/lib/native-approval-notify.ts` and emit from the workbench mount/wrapper layer — **no `@intx/*` change**. `ReviewGate` refetches the native list on each event.
- **Retirement.** The dead legacy sidecar gate `apps/sidecar/src/approval-gate.ts` (`createApprovalGatedRunner`/`createApprovalClient` — the only historical writer of `workbench_approval`, unwired since the runtime-retirement pin bump) is **deleted** (its only importer was its own test). Nothing double-gates with the flag on: the sidecar catalog gate already treats `ask` as granted and the reactor owns the suspension. **Deferred to post-soak:** the legacy `workbench_approval` table + its `createApprovalsRouter`/`createInternalApprovalsRouter` read/resolve/create routes and the legacy ReviewGate rendering path stay until the native rail soaks (default-off tenants still use them; there is no data migration — a legacy row cannot become a native suspension).
