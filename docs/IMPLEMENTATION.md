...
- **Inbox intake (CL-3577..3586)** — `apps/hub/src/services/inbox-intake.ts` (`createInboxIntake`) runs a 60s tick (`DEFAULT_TICK_INTERVAL_MS`, overridable via `INBOX_INTAKE_TICK_MS`) gated by the same `scheduler` feature grant used by the automations above. Every source is **default off** at the catalog level (`INBOX_SOURCE_CATALOG` in `packages/workbench-shared/src/preferences-registry.ts`, derived from `CREDENTIAL_PROVIDER_CATALOG` + `TOOL_CREDENTIAL_SUPPLEMENTS`'s `inboxSource` tag) — intake is strictly opt-in end to end. Two scopes:
  ...
  - **Workspace scope** (Granola): `granola-workspace.ts`, constructed at runtime and concatenated onto the static registry (`registry: [...INBOX_SOURCE_REGISTRY, createGranolaWorkspaceInboxSource(...)]`) because it needs the job queue injected. Gated by the same owner-ceiling grant, keyed `inbox-source:granola`; no per-member preference — a Granola call reaches a member only via the fan-out match below. **CL-3627 / CL-4153**: the tick is enqueue-only — it lists notes and calls `queue.enqueue(tenantId, noteId)` on the durable `work_unit` queue via the thin `granola-call-job-queue.ts` facade (`kind = granola_call`, idempotency `note:{noteId}`); it does **not** fetch the full note (transcript) or invoke the LLM turn, so a slow/failing Granola call no longer blocks the shared 60s tick loop. `enqueue` is idempotent per note — a re-listed note upserts onto its existing work unit rather than duplicating work.
  ...
  - **Granola call job queue and runner (CL-3627 / CL-4153 / CL-3647)** — the LLM turn no longer runs on the intake tick. `granola-call-job-queue.ts` (`createGranolaCallJobQueue`) is a thin facade over `work_unit` kind `granola_call` (migration `0076` cutover; specialized table dropped in `0077`): `enqueue` upserts a pending unit (dedupe via idempotency `note:{noteId}`), `claimDue` leases due units, and `complete`/`fail` resolve a claim with owner-fenced lease checks. A failed attempt reverts to pending with `next_attempt_at` pushed out by an exponential backoff (1 minute base, doubling, capped at 30 minutes) and is marked `dead` (not retried) once attempts reach `MAX_ATTEMPTS` (8) — a permanently-broken note is logged loudly rather than retried forever or silently dropped. `granola-call-job-runner.ts` (`createGranolaCallJobRunner`) polls the queue on its own 15s interval (`DEFAULT_TICK_INTERVAL_MS`, independent of the 60s intake tick), claims one job at a time (with lease heartbeat), and starts a `granola-call` workflow run via the run-start seam with `{ noteId, tenantDomain? }` (best-effort root tenant domain for classification). It completes or fails the job from the terminal run status — no credential resolve, note fetch, or pipeline call remains in the runner. Workflow artifact dedupe and fan-out messageKey dedupe make a retried or duplicate-claimed job safe to re-run.
