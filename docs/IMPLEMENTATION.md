...
**`sideEffect` colocation (CL-3897).** A tool's `read`/`write` classification lives on its `ToolDefinition` itself, not in a parallel hand-authored map. A package's `tool-manifest.ts` builds its manifest `entries` from that same definitions array via `hubToolEntriesFromDefinitions` (`@workbench/tool-manifest`), so the classification is authored once, next to the tool it describes, and cannot drift from the runtime array the way a separately hand-maintained `entries: { <name>: { sideEffect } }` map could. A package whose tools dispatch through more than one path — `tools-agents`, split across the sidecar `interchange-tools.ts` factory and a hub-native `ContextToolEntry` (`search_agents`) — has no single runtime array to derive from, so it keeps a discovery-based set-equality test (every `ToolDefinition`-shaped export vs. the manifest's declared names) instead.
...
**`packages/tools-granola`** (`@workbench/tools-granola`): Granola notes API plus pure workflow tools. Hub tools: `granola_list_notes`, `granola_get_note`, `granola_list_folders` (providerName: `'granola'`). Pure workflow tools (CL-3647): `granola_normalize_note`, `granola_classify_call`, `granola_build_analysis_prompt`, `granola_parse_analysis`, `granola_prepare_artifacts`, `granola_emit_run_outputs`. Hub-only write tools for the deployed workflow: `granola_create_tasks`, `granola_fanout_call`. `granola_list_notes` paginates with `page_size` (default 10, max 30) and accepts `created_after`/`created_before`/`updated_after`/`folder_id` filters; `granola_list_folders` surfaces folder IDs for that `folder_id` filter.

#### granola-call workflow (CL-3647)

Processes a single Granola note end-to-end as a deployed workflow
(`workflows/granola-call`): deterministic fetch → normalize → classify →
`granola_build_analysis_prompt` → one `agentStep` extract →
parse → prepare → three parallel `write_artifact` persists (pain /
summary / brief, each with per-kind `sourceRef` idempotency) → tasks
(with artifact links + `sourceRef`) → fan-out → emit (`artifactsByKind`
on completion for CL-3648 chaining). Classify never skips when
`tenantDomain` is missing — empty domain yields classification
`unknown`. Emit peels projected step envelopes so all three artifact
IDs survive reshape. The hub job runner only starts the run via the
run-start seam (passing `noteId` + best-effort root `tenantDomain`) and
completes/fails from the run outcome — no in-process pipeline, LLM, or
transcript handling remains on the hub path.
`apps/hub/src/services/granola-call-pipeline.ts` is a thin re-export of
shared pure helpers for legacy import sites only.
...
| Area                       | Modules                                                                    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Composer                   | `ChatInput.tsx`, `attachments.ts`, `composer-voice-dictation.ts`           | File picker, drag-drop, clipboard paste (`filesFromClipboard`), attachment validation chips; optional Web Speech dictation with auto-send on stop, gated by the owner-governed `voice-input` feature grant (CL-3909; replaced the old `VITE_MYRA_VOICE_INPUT` build-time flag — no emergency env override, owner-grant-only) via `useMyraVoiceInput`/`useMeFeatures`. Draft-and-queue while busy (CL-2988, below).                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
**Deploy** (`apps/hub/src/routes/workflow-deploy.ts`) — service-token-gated. Validates the posted definition, resolves the tenant deploy config, and runs the `@intx/workflow-deploy` orchestrator via `apps/hub/src/services/workflow-deploy.ts` (`createWorkflowDeployService` → `deployWorkflow`, defined at `workflow-deploy.ts:185`; orchestrator built at `:337`, run at `:359`). The service writes `workflow.json` + `capability-declarations.json` to a git-backed `workflow` repo and sends the multi-step deploy frame to the sidecar (`toSendMultiStepDeploy` over `SidecarRouter`; the `as AgentDeployWorkflow['definition']` cast reflects only exactOptional variance — see AGENTS.md § Vendored workflow-host wiring). The orchestrator calls its per-step hook for every `stepOrder` entry; on a production provision the hub injects interchange's `SessionService.stageWorkflowStep`, which stages each step's pinned tool closure on disk (single-step definitions collapse onto `deploySingleStepAtHead`). Every step gets uniform `agent` + `agent_instance` rows — the former inline/deterministic partitions are retired because interchange's pack phase records a `session_asset` row per staged step with a hard FK chain to `agent_instance`. Per-run deploys additionally write the native identity projection (a `workflow`-kind `asset` row, the `workflow_deployment` row the deploy-ack public key and reconnect ownership challenge resolve against for `dep_`-prefixed addresses, and the creator's workflow-run read grant), mirroring interchange's own `deployWorkflowDefinition`. Each deployed step's `state/grants.json` is written into its agent-state repo by `writeStepGrantFiles` (`:1026`, called at `:259`).

**Single-agent (launched) deployment identity (CL-3931).** A single-agent instance — Myra, Oat, triage/gate agents, ephemeral per-item Myra — deploys through interchange's `deployInstanceAtHead`, which keeps the instance's REAL address and lands its Ed25519 key on `agent_instance` (a non-workflow-derived `ins_<hex>` address routes its reconnect challenge to `agent_instance.public_key`, never `workflow_deployment`). It writes NO `workflow_deployment` row of its own, so before CL-3931 the projection table was empty against ~20 live launched agents. Interchange's `registerSignalCorrelation` (5a73d3cc pin) resolves a suspended write tool's tenancy by looking `workflow_deployment` up by the deployment mail address (the instance's real address), and the supervisor derives its `deploymentId` as `deriveWorkflowRunRepoId(address)`; without a matching row the suspension co-write throws and the tool call parks until timeout (the CL-3934 native-approvals blocker). `launchAgentSession` now writes `writeInstanceDeploymentProjection` after a successful `deployInstanceAtHead` — the `workflow`-kind asset (`wf_<agentId>`), the `workflow_deployment` row keyed by `(id = deriveWorkflowRunRepoId(address), address)` with a null `publicKey`, and the creator read grant — best-effort (a projection failure never fails a live launch) and idempotent across mail-wake redeploys (the address-derived id is stable). Permanent teardown (`DELETE /instances/:id`, `teardownThreadRows`) drops the row; idle-sleep leaves it so a woken agent can still register. Hub boot runs `reconcileInstanceDeploymentProjections` to converge the projection to the live launched agents (backfilling rows the pre-CL-3931 deployments never had, so native-approvals can be enabled without redeploying, and reclaiming leaked rows); per-run (`ins_dep_...`) rows are skipped in both directions. Pushed via the admin CLI's **Workflows → Push (deploy) a workflow** (which spawns `apps/hub/bin/deploy-workflow.ts` internally; see [ADMIN_CLI.md](./ADMIN_CLI.md)), which imports `@workbench/workflow-<kind>`, serializes its `workflow`, and POSTs it with `HUB_SERVICE_TOKEN`.
...
Each kind is a package under `workflows/<kind>/` named `@workbench/workflow-<kind>`, exporting `kind` + `workflow` (`defineWorkflow` with deployed `agentStep` / `deterministicToolStep` steps and `awaitSignal` HITL gates; the step classes live in `packages/agents/src/deterministic-step.ts`). `inlineInferenceStep` is deleted; `packages/ab-compare-presets`' A/B variant steps (its one caller, for the nonFatal+retry degrade) are migrated to `agentStep`. Shipped kinds (the `workflows/` directory): `ab-compare-quality`, `ab-compare-speed`, `ab-compare-standard`, `attio-task-agent`, `gamma-presentation-creator`, `granola-call`, `heartbeat`, `last30days-research`, `pain-point-collateral`, `reddit-opportunity-scanner`, `smoke-test`, `sumble-account-intel`. The hub imports none of them — its only workflow imports are `@intx/workflow-deploy` (the orchestrator) and the `WorkflowDefinition` _type_ from `@intx/workflow`. Adding a kind is a new package + a push.
...
- `apps/sidecar/src/step-tool-harness.ts` — the workbench's step-tool harness. Tool RESOLUTION is upstream's on-disk model: a step reads its pinned tool closure from the deploy tree the hub stages on disk (`readStepDeployTree` → `readDeployTree` at `<dataDir>/<sanitizeAddress(stepAddress)>/deploy/`, located by `stepDeployTreeDir` in `workflow-substrate-factory.ts`). Staged at deploy time by interchange's `deployInstanceAtHead` (single agents / head) and `SessionService.stageWorkflowStep` (multi-step steps, wired in `apps/hub/src/services/workflow-deploy.ts`). The harness then layers the workbench's credential + hub-backed injection AROUND the native loader: it resolves tenant tool credentials via `/api/internal/tools/credentials` (`tool-credentials.ts:63`) and builds the `HUB_RPC` context for hub-backed `RuntimeCapabilities` tools (`/api/internal/hub-tools/run`). The retired hub-RPC manifest rail (`/api/internal/tools/manifest`, `fetchStepToolManifest`) is deleted.
- `apps/sidecar/src/workflow-substrate-factory.ts` — builds the per-child substrate and `createSidecarStepInvoker` (`:888`). It dispatches by the step agent's `workbench.stepKind` tag: `deterministic-tool` → `runDeterministicToolStep` (tool called directly, no reactor/inference), and every other (deployed reasoning) step → the real tool-capable agent harness. The former `inline-inference` → bare-`createAgent` branch is deleted; every reasoning step (native `agentStep` or hand-built `defineAgent`) now dispatches through the same tool-capable harness.
...
- **Boot reconciler — REMOVED (CL-3955).** The boot-time sweep (`apps/sidecar/src/boot-reconciler.ts`, deleted) pruned on-disk dirs whose `(ses|dep)_<32hex>` deployment-id token was absent from the hub-reported live set — but that set (`GET /api/internal/deployments/live`) is built from the `workflow_run` catalog-deployment table, and **per-run (single-use) deployments never write a row there**, so every sidecar restart deleted the run history of any run parked at an `awaitSignal` gate, making it permanently unresumable (the fresh workflow-child finds an empty `runs/<runId>/events/` prefix and the signal channel refuses delivery forever: "signal deliver refused: run log has no events yet"). This defeated the CL-3104 hibernate design, which tears down a long-parked run's process with `reclaimDirs: false` precisely so its dirs survive for resume, and violated interchange's durability contract (deployment state persists until explicit undeploy). Terminal-run cleanup does not depend on it: the hub fires `tearDownDeployment` on every terminal transition (projection bridge → sidecar undeploy hook, which reclaims `ownedDirs`), and `WorkflowReconciler.reclaimOrphanedDeployments` re-sweeps terminal runs within a 24h window for teardowns lost to a hub crash. What is deliberately given up: a backstop for dirs whose teardown misses both layers — a slow drip, accepted until a correct reclaim design lands (complete live set including `running`/`awaiting` rows from `workflow_run_record`, and/or a hub→sidecar run-history reseed on re-establish). The read-only hub endpoint `GET /api/internal/deployments/live` (`apps/hub/src/routes/internal-deployments.ts`, sidecar-token gated; shared schema in `packages/tool-credentials`) is **kept** for that follow-up.

**Workflow-child recycle policy (CL-3885).** Every multi-step supervisor arms interchange's recycle-policy bounds for its child process: max uptime plus a max-RSS bound (default 3 GiB, env-overridable via `WORKFLOW_CHILD_MAX_RSS_BYTES`-style override in `resolveDefaultRecyclePolicy`; `"0"`/empty disables). RSS is read host-side by `apps/sidecar/src/workflow-child-rss.ts` — Linux `/proc/<pid>/statm` only, `undefined` (bound skipped) on any failure or non-Linux — injected through the supervisor's `readRssBytes` seam via a pid-tracking wrapper around the subprocess spawner. A bound trip drains and respawns the child through the supervisor's existing recycle sequence (policy timer stopped before the kill window, re-armed after install), so a runaway child is recycled instead of growing the shared host multi-GB.

Since CL-3884 the sidecar performs **no boot-time deployment restore at all** — it boots empty (no `deployment.json` records, dormant markers, or tombstones exist anymore) and the hub re-drives residency on demand: mail-wake re-deploys idle single-step agents, gate signals plus the awaiting prewarm re-establish parked runs, and the run liveness sweep fails a running run whose child died with the process.
...
**Title humanization (CL-3907).** `skillTitle({ name, displayName })` (`packages/workbench-shared/src/tool-labels.ts`) is the single rule the UI renders through: it humanizes `name` when `displayName` is `null`, humanizes `displayName` itself when `isSlugShaped(displayName)` (the string already equals its own kebab slug), and otherwise returns `displayName` verbatim. `isSlugShaped` and `toAssetName` share one `slugify()` helper so the write side and this read-side rule agree on "what counts as a slug." The write-side guard lives in `createSkill` (`apps/hub/src/services/skill-library.ts`): a user-supplied name that is already slug-shaped is not persisted as `displayName` at all (`displayName = isSlugShaped(name) ? undefined : name`), so it falls into the humanize-the-asset-`name` path on render instead of a raw kebab slug leaking into the UI as a stored `displayName`.
...
- `id` (UUID, primary key)
- `deploymentId` (text, nullable) — the `@intx/workflow-deploy` deploymentId (`dep_…`; legacy rows `ses_…`) for natively deployed runs; null for legacy pipeline-session rows. The `GET /api/v1/workflow-runs` index filters on it
...
- `scheduled_trigger` (migration `0047`) — durable per-user schedules fired by the daily scheduler (`apps/hub/src/services/scheduler.ts`) and seeded by `scheduled-trigger-seeder.ts` (one heartbeat schedule per Myra instance, at `scheduler.heartbeatHourUtc`). The scheduler requires an enablement predicate (feature grant, below) — the old static `enabled` flag is gone. Heartbeat payloads are enriched **at fire time** (`enrichHeartbeatTriggerPayload`): the member's `briefSource:*` preferences become `enabledSources`, and `createdAfter` is stamped from the schedule's last fire (24h fallback, clamped to 7 days) so day-2 briefs never re-brief the same Granola notes. Workflow schedules auto-deliver the `intake` gate when configured (CL-3509); multi-gate kinds with post-intake human gates require `ALLOWS_SCHEDULED_POST_INTAKE_DRIVE` on the workflow package (or test allowlist) before attach — see **Scheduled runs, intake, and post-intake gate drive** in [WORKFLOWS.md](./WORKFLOWS.md) (CL-3528 Myra gate agent, queue cap, stalled-run reconciler).
  ...
  **Trigger-payload enrichment registry (CL-3891).** `enrichHeartbeatTriggerPayload` and every other kind's fire-time enrichment now apply from a single kind-keyed registry, `apps/hub/src/workflow-executor/trigger-payload-enrichment-registry.ts` (`enrichTriggerPayloadForStart`, `TRIGGER_PAYLOAD_ENRICHERS`). Both run-start implementations call into it — `startWorkflowRun` (`workflow-executor/run-exec.ts`, backing `POST /workflow-exec/:kind/start` and the `workflow_start` hub tool) and `createWorkflowRunStarter` (`services/workflow-run-starter.ts`, backing webhook triggers, the scheduler, and `POST /me/brief-run`) — so no caller builds its own enrichment anymore; a kind with no registered enricher passes its input through unchanged. Member preferences and mail identity are resolved once inside the registry rather than by each caller. The scheduler is the only caller with a genuine since-last-fire window, so it forwards its schedule row's `lastFiredDayUtc`/`hourUtc` through `StartRunInput.heartbeatFire`, giving `lookback: "scheduled"`; every other source (manual brief-run, webhook, the generic start route) defaults to `lookback: "manual-refresh"` (the flat 7-day window). Failures are loud: `POST /me/brief-run` wraps its `startRun` call in a try/catch and refunds the caller's rate-limit slot on any failure inside enrichment (e.g. an identity-lookup error), rather than losing the slot to an unhandled 500.
...
- **Inbox intake (CL-3577..3586)** — `apps/hub/src/services/inbox-intake.ts` (`createInboxIntake`) runs a 60s tick (`DEFAULT_TICK_INTERVAL_MS`, overridable via `INBOX_INTAKE_TICK_MS`) gated by the same `scheduler` feature grant used by the automations above. Every source is **default off** at the catalog level (`INBOX_SOURCE_CATALOG` in `packages/workbench-shared/src/preferences-registry.ts`, derived from `CREDENTIAL_PROVIDER_CATALOG` + `TOOL_CREDENTIAL_SUPPLEMENTS`'s `inboxSource` tag) — intake is strictly opt-in end to end. Two scopes:
  ...
  - **Workspace scope** (Granola): `granola-workspace.ts`, constructed at runtime and concatenated onto the static registry (`registry: [...INBOX_SOURCE_REGISTRY, createGranolaWorkspaceInboxSource(...)]`) because it needs `db` and the run-starter injected. Gated by the same owner-ceiling grant, keyed `inbox-source:granola`; no per-member preference — a Granola call reaches a member only via the fan-out match below. **CL-4213**: the tick lists notes and, per note, checks whether the call's three typed artifacts already exist (`granolaCallArtifactsProcessed`, `granola-call-artifacts.ts` — see below) before starting a `granola-call` workflow run directly via the run-start seam with `{ noteId, tenantDomain? }` (best-effort root tenant domain for classification); it does **not** fetch the full note (transcript) or invoke the LLM turn on the tick itself — that happens inside the started run. There is no queue, lease, or claim protocol between "listed" and "started": a note whose artifacts already exist is skipped, and a note that's missing one or more is (re-)started, relying on artifact `sourceRef` dedupe for correctness. This replaces the CL-3627/CL-4153 `granola-call-job-queue.ts` + `granola-call-job-runner.ts` work-unit facade, both deleted.
  ...
  - **Granola call processing + fan-out (CL-3647)** — processing is the deployed `workflows/granola-call` workflow (see packages/tools-granola section): fetch → classify → extract (agent) → parse → persist three typed artifacts → tasks → fan-out → emit. `granola-call-pipeline.ts` is a thin re-export of pure helpers from `@workbench/shared` for legacy import sites only — no in-process LLM/transcript path remains. `granola-call-fanout.ts` (`createGranolaCallFanout`) matches root-tenant Myra members to the call by participant or mentioned-person email (falling back to name only when the match is unambiguous), requires each matched member to have their Granola inbox capability enabled, and delivers one deduped mail per `(call, recipient)` containing only that recipient's items. The workflow's `granola_fanout_call` hub tool invokes this service.
  - **Granola call dedupe on artifact sourceRef (CL-4213)** — replaces the CL-3627/CL-4153 work-unit queue + off-tick runner entirely; no queue, lease, or claim protocol remains for Granola. Each processed call writes three typed artifacts (pain points, summary, brief; `granola_prepare_artifacts` in `packages/tools-granola/src/workflow-tools.ts`), each keyed by a deterministic per-kind `sourceRef` — `granolaCallSourceRef(noteId, kind)` → `granola:call:<noteId>:<kind>` (`packages/workbench-shared/src/granola-call.ts`, in place since the workflow's introduction under CL-3647) — derived from the stable upstream note id, not the call title (titles collide and get renamed; ids don't). `apps/hub/src/services/granola-call-artifacts.ts` (`granolaCallArtifactsProcessed`) reports a call "processed" only when all three sourceRefs already have an artifact row; the Granola inbox source (above) uses this as a pre-flight check before starting a run, so an already-processed call is never re-run. **Partial-write behavior is explicit, not implicit**: a call with some but not all three artifacts landed (crash mid-run, a transient `write_artifact` failure) reads as "not processed," so a re-run is allowed and reprocesses the whole call — no atomic three-artifact write is built. That re-run is still safe because `write_artifact` (`apps/hub/src/tools/write-artifact.ts`) dedupes per artifact on `(tenantId, sourceRef)` before inserting (select-for-update, insert-if-absent), and the partial unique index `artifact_tenant_source_ref_uniq` (migration `0055`) is the database-enforced backstop that rejects a genuinely concurrent duplicate insert outright — so correctness never depends on the pre-flight check winning a race, only on the index. Migration `0078` deletes now-orphaned `granola_call` `work_unit` rows and drops `granola_call` from the app-level `workUnitKinds`; no artifact backfill was needed because every Granola artifact has carried the correct per-kind `sourceRef` since the workflow's inception. The generic `work_unit` table and its `FOR UPDATE SKIP LOCKED` claiming machinery (`work-unit-queue.ts`, `work-unit-worker.ts`) stay — `knowledge_capture` (`knowledge-capture-outbox.ts` / `knowledge-capture-hook.ts`) and `agent_task_turn` (`agent-task-auto-pickup.ts`) are both still-live consumers with no natural output key to dedupe on the way Granola's artifacts do.
...
- `GET /api/v1/tenants/:tenantId/approvals/stream` — SSE stream of approval **change notifications** (`created` / `resolved`), keyed by tenant. Carries only a change signal (`tenantId`, optional `sessionId`, `kind`) — never approval rows or tool-call arguments; the web `ReviewGate` refetches the ownership-scoped native list (`GET .../native-approvals`) on each event instead of polling on an interval. Served by `createApprovalNotificationsRouter` (`apps/hub/src/routes/approval-notifications.ts`), a stream-only router backed by the in-process pub/sub (`apps/hub/src/lib/approvals-events.ts`). The **native rail is the sole emitter** now (CL-3938 deleted the legacy create/resolve routes): `native-approval-notify` publishes `created` after the suspension co-write commits and `resolved` after the mounted approve/reject route. No `@intx/*` changes.
...
The legacy `POST /api/internal/approvals` create callback (and the user-facing
`GET`/approve/reject routes) were **deleted** (CL-3938) — the `ask_principal`
tool that was its only caller is gone, so the native rail is the only approver.
Its former backing table, `workbench_approval` (renamed from the legacy
`approval` at the 5a73d3cc pin bump so it would not collide with interchange's
...
#### Native approval-suspension (always-on write gating)

- **No feature gate — write approvals are unconditional (CL-3940).** The `native-approvals` feature gate that guarded the initial soak (env override `NATIVE_APPROVALS_ENABLED` + admin-provisioned member-role grant, CL-3934) is **retired**: native write-tool approvals now gate unconditionally via `resolveAskToolNamesForTenant`, nothing reads the env var (it is removed from `.env.example`), and `featureEnvOverridesFromConfig` hardwires the entry to `false`. `native-approvals` remains a resolvable feature name (`FEATURE_NAMES`) purely as residue — it stays absent from `FEATURE_GRANT_CATALOG`, so the Owner → Capabilities surface never renders it and `PUT /owner/features/:name` rejects it with 404 (guarded by an owner-route test).
- **Ask-grant minting (`apps/hub/src/lib/tool-grants.ts`, `native-approvals.ts`).** `persistInstanceToolGrants` mints the approval-gated write set (`APPROVAL_GATED_TOOL_NAMES` from `@workbench/agents`) with `effect: "ask"` instead of `allow`. `resolveAskToolNamesForTenant` returns that set **minus the tools the instance's principal has durably auto-approved** (CL-3942, below). Read tools and internal-write exclusions stay `allow`. **Workflow step grants stay `allow`** (`buildStepGrantRules` is untouched) so unattended runs never block.
- **Durable per-principal auto-approve (CL-3942).** The approval card offers three resolutions: reject, approve once, and **always approve**. "Always" records an `auto_approved_tool` row (unique on principal + tool, `apps/hub/src/db/schema.ts`) via the native-approvals router; `resolveAutoApprovedToolNames` (`apps/hub/src/lib/auto-approved-tools.ts`) feeds the exclusion into ask-grant minting, so the next grant reconcile mints that tool `allow` for that principal and it stops prompting. The trust decision is the member's own and reviewable: `GET .../native-approvals/auto-approved-tools` lists the caller's auto-approved tools (newest first) and `DELETE .../auto-approved-tools/:id` revokes one, restoring the prompt on the next reconcile.
- **Runtime path (all native, `@intx/*`).** An `ask` grant makes Interchange's `createAuthzExtension` (`@intx/inference`) return `ask`; the reactor suspends the call, registers a gate, and persists a `PendingOperation` with the captured `suspendedCall`. The supervisor's `park.notify` → `onSuspensionRegister` (the CL-3932 sidecar pass-through) → hub-link `signal.correlation.register` → hub `sidecar-handler` → `registerSignalCorrelation` co-writes an `approval` row (status `pending`) and a `signal_correlation` row. Resolution is Interchange's `createApprovalRoutes` → `resolveApproval`: claim the correlation, flip the approval, and `sidecarRouter.sendSignalDeliver` the `ApprovalDecision` to the parked run — approve re-dispatches the call once (one-shot bypass token), reject answers it with an error result. The sidecar catalog gate treats `ask` as granted so a write tool is not dropped from the dynamic catalog when its grant flips.
...
- **Decision surface (CL-3934) — mounted native routes + workbench list.** Interchange's `createApprovalRoutes` (mounted by `createApp`/`mountHubRoutes` at `/api/tenants/:tenantId/approvals`, behind `resolveTenant`) ships the working **approve/reject** halves but leaves its **list**/**get** a 501 stub, and `ApprovalStore` exposes no list read. The workbench therefore adds **only the read side**: `createNativeApprovalsRouter` (`apps/hub/src/routes/native-approvals.ts`) mounted on `hubApp` at `/api/tenants/:tenantId/native-approvals`, inheriting the same `resolveTenant` (active-membership; cross-tenant 403/404). The list is **ownership-scoped**: it joins `member_agent_instance` to the agent instances the caller owns and filters the `approval` rows to those instances' addresses, so one tenant member never sees another member's pending approval metadata (agent address / runId / deploymentId / timing). **Tool action + args are populated by snapshot enrichment (CL-3940):** the reactor's suspend-time co-write still writes `toolDefinition`/`toolArguments` as null, but the sidecar emits a `custom.approval.requested` inference-event snapshot keyed by the same correlation — emitted **before** `gate.blocked` on ask-suspend so the card never races its context — and `native-approval-notify` applies it (buffered if it arrives first) after the co-write commits, best-effort. The web `ReviewGate` (`apps/web/src/components/ReviewGate.tsx`) is now **native-only** (CL-3938 deleted the legacy card path): it lists the rail via `listNativeApprovals` and renders each with `NativeApprovalCard` (action + args from the snapshot when present, else the originating agent), ordered newest-first by `createdAt`. In chat, the card is scoped to the raising thread, sources args from that thread's transcript, and reconstructs from server state on a cold reload (CL-3940); pending approvals also surface as indicators on the notification bell, the Inbox nav, and the sidebar chat list. The rail is tenant-wide (a suspension has no session linkage), so `ReviewGate` takes only a `tenantId`.
...
- **Attachment-ref persistence (CL-3671 / CL-3680)** — Interchange stores mail as raw MIME and `createInboundTurn` (`interchange/packages/inference/src/turns.ts`) converts every attachment into an inline content block with no store-but-don't-forward flag, so real attachments cannot ride on the mail wire for a text-only model. The workbench-owned seam keeps chips without re-introducing that path: diverted mails send `attachments: []` (parsed text only in the body), and `POST /api/v1/instances/:instanceId/mail-attachments` (`apps/hub/src/routes/mail-attachments.ts`) writes reference-only rows to `mail_attachment_ref` (migration `0059`, keyed by mail id + artifact id, outside the MIME). On reload, `GET .../mail-attachments` feeds `mergeAttachmentRefs` (`apps/web/src/hooks/mail-attachment-refs.ts`), which stamps both flat `attachments` (chip render) and re-lifted `parts` with matching `file` parts (`liftToParts`) so the parts model and the chip path stay in sync. Chip bytes resolve via the artifact download route (parse-file artifact ids) rather than the interchange mail-attachment blob route. Agent live `inference.image_output` images remain a live-only SSE path and do not persist here.
...
### Artifact gallery previews (CL-3908 / CL-3920)

`ArtifactCard` (`packages/artifact/src/ArtifactCard.tsx`) always renders through `ArtifactCardPreview`, keyed off `artifactPreviewFamily(kind)` (`packages/artifact/src/artifact-preview-family.ts`), which classifies every artifact kind into one of a small set of families (document, social, email, research, comparison, presentation, data, web). The family drives a content-shaped preview even with no `thumbnailUrl` — e.g. a comparison artifact renders two mini variant panels plus a winner excerpt — replacing the former fallback of a generic icon-glyph placeholder (`ArtifactViz`, removed) when no thumbnail existed. Card chrome was pared down to the preview, title, and a small creator-initials avatar badge; the family-label pill/icon and the provenance/origin chip row under the title were dropped as redundant with the preview itself.

### Agents page (definitions library)

`AgentsPage` (`apps/web/src/pages/AgentsPage.tsx`, CL-3916/CL-3904) leads with agent **definitions** (`useAgentTemplates`, `apps/web/src/hooks/use-agents.ts`) rather than the caller's deployed instances. Each definition merges in its 0..N matching deployed instances (by name) into one card — showing the definition's tool chips plus a per-instance status/address list (or "Not deployed") — so the page reads as a library of what agents exist, with deployment state as a per-card detail rather than the page's organizing axis. An instance with no matching definition still surfaces separately.
