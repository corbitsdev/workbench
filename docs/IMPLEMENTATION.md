...
- **Feature gate — deliberately NOT owner self-serve.** `native-approvals` is a resolvable feature name (`FEATURE_NAMES`) but is intentionally **absent from `FEATURE_GRANT_CATALOG`**, so it never renders on the Owner → Capabilities surface and the `PUT /owner/features/:name` toggle rejects it with 404 (guarded by an owner-route test). Its enable paths are staff-only: the global env override `NATIVE_APPROVALS_ENABLED` (`config.nativeApprovalsEnabled`, default off) or an admin-provisioned member-role `feature:native-approvals`/`enable` grant for a per-tenant staged rollout. It stays staff-only through the decision-surface soak (it does not return to the owner catalog in this increment). Off = no `ask` grants resolve, so write tools run `allow` (no approval gate at all, now that the legacy poll rail is deleted); on = ask-grant activation. `resolveAskToolNamesForTenant` reads this through the memoized `isFeatureEnabledForTenantCached` so a bulk grant reconcile issues one feature read, not N.
