...
- **Feature grant.** `native-approvals` is a deny-by-default owner feature grant (`FEATURE_GRANT_CATALOG` in `@workbench/shared`, toggled from Owner → Capabilities → Features, resolved via `isFeatureEnabledForTenant`). Off = legacy behavior exactly; on = ask-grant activation.
