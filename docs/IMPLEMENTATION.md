...
- **KNOWN GAP — single-agent instances have no `workflow_deployment` row.** `registerSignalCorrelation` resolves tenancy by looking up a **deployed `workflow_deployment` by address**, but a single-agent instance (Myra/Oat) is deployed via `sessionService.deployInstanceAtHead`, which by design **writes no `workflow_deployment` row** (`apps/hub/src/services/agent-provisioning.ts`, ~L722). So with the flag on, a Myra write tool suspends but the native register co-write throws `No deployed workflow deployment for address …` (logged, no approval row surfaced) and the call parks until its gate times out. The native rail as pinned therefore only registers approvals for **multi-step workflow deployments** (which have `workflow_deployment` rows). Giving single-agent deploys a resolvable native deployment identity for the register co-write is the prerequisite for the decision surface and for Myra coverage, and is the reason the flag ships off. Tracked as the next increment.
- **Decision surface + retirement — deferred.** The ownership-scoped list/resolve hub routes over the native `approval` store, the ReviewGate rewiring to the native shape, and the physical retirement of the legacy `workbench_approval` rail all follow once the deployment-identity gap above is closed and the native path soaks. Pending `workbench_approval` rows remain answerable via the legacy read/resolve routes; there is no data migration (a legacy row cannot become a native suspension).
