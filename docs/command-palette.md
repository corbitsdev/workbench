...
already renders, plus channels, agents, workflow runs, routines, skills, and
...
What workbench actually shows — which pages exist, and which channels,
...
`useEntitySearch` to live fetchers for channels (`@corbits/chat-ui`'s
`listChannels`), agents (`listAgentDefinitions`), and workflow runs
...
Commands, Channels, Pages, Runs, Routines, Skills, Library, then
People & agents — matching the mock's `buildCmdkEntries` ordering. Selecting
a channel, run, agent, routine, or skill result navigates to its real route
...
placeholder text (`Search or jump to… (# channels · @ people · > actions ·
