...
**Tool narrative and logos.** Friendly tool lines come from `@workbench/agents` `friendly-tool-summary.ts` (catalog phrases + mail_send humanization). `@workbench/chat` `ToolNarrative` renders collapsed/expanded tool groups; `apps/web` injects `ToolCallProviderMarker` + Brand API logos via `tool-providers.ts` for supported integrations.
