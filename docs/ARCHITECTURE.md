...
Each workflow is its own package under `workflows/<kind>/` named `@workbench/workflow-<kind>`, exporting `kind` and a `workflow` built with `@intx/workflow`'s `defineWorkflow`. Steps are one of two classes — deployed (reasoning-with-tools) `agentStep`/plain `defineAgent` agents, or `deterministicToolStep` — plus `awaitSignal` human-in-the-loop gates (see [WORKFLOWS.md](./WORKFLOWS.md) § Step classes). The `inlineInferenceStep` kind and its sidecar dispatch branch are deleted; `packages/ab-compare-presets`' A/B variant steps (the one caller that used its nonFatal+retry degrade) are migrated to `agentStep` — a variant that still fails after retry now fails the run. The shipped kinds are `ab-compare-hitl`, `attio-task-agent`, `gamma-presentation-creator`, `last30days-research`, `pain-point-collateral`, `reddit-opportunity-scanner`, and `smoke-test` (the `workflows/` directory). Adding a workflow is a new `workflows/<kind>/` package plus a push — **no hub change**.
...
Deploy is an operator action, not a user action. The admin CLI's **Workflows → Push (deploy) a workflow** imports the package, serializes its `workflow`, and POSTs the definition (operator session path `POST /api/v1/workflows/deploy`, or service-token path `POST /api/internal/workflows/deploy`). The hub's workflow-deploy service (`apps/hub/src/services/workflow-deploy.ts`) validates the definition, resolves the tenant deploy config, and runs the `@intx/workflow-deploy` orchestrator. Deploy is **hub-only** (`persistCatalog`): the orchestrator commits `workflow.json` + `capability-declarations.json` to a git-backed `workflow` repo and writes the per-step DB/grant rows, but sends **no** `agent.deploy` frame and spawns **no** supervisor. Because publish never touches the sidecar, it succeeds with the sidecar disconnected — and boot autopublish no longer waits for a sidecar connection. The supervisor + `workflow-child` subprocess is spawned **per run** by `provisionRunDeployment` (see Per-run deployment below), where every step's pinned tool closure is staged on disk through interchange's per-step stager and every step gets uniform `agent`/`agent_instance` rows (the earlier inline/deterministic no-op partitions are retired — interchange's pack phase records a `session_asset` row per staged step with a hard FK to `agent_instance`; see [WORKFLOWS.md](./WORKFLOWS.md) § The execution model). See [DEPLOYING_WORKFLOWS.md](./DEPLOYING_WORKFLOWS.md) for the full deploy guide.
...
Each **run** provisions its own ephemeral deployment (CL-2582). Run-start (`provisionRunDeployment` in `apps/hub/src/services/workflow-deploy.ts`, called from `/workflow-exec/:kind/start`) reads the published definition for the kind, mints a fresh **`dep_`-prefixed** `deploymentId` (interchange's native workflow-deployment id space — deploy acks and reconnect key lookups resolve against the native `workflow_deployment` projection row via `isWorkflowDerivedAddress`, never against per-step `agent_instance` rows), writes the native projection interchange's own `deployWorkflowDefinition` writes (a `workflow`-kind `asset` row anchoring the definition, the `workflow_deployment` row, and the creator's workflow-run read grant), deploys a dedicated supervisor + `workflow-run` repo for that single run, and triggers it. This gives same-kind run **concurrency by construction**: Interchange's native model is one per-deployment supervisor that dispatches a deployment's runs **serially**, so without per-run deployment a parked `awaitSignal` run would starve every other run of its kind. The change uses Interchange's deploy primitive **unmodified** (zero `@intx/*`/vendored changes) — all logic is hub-layer.

Under this model the operator deploy (above) is the **definition registry**: it publishes `workflow.json` + the `workflow_run` index row but does not serve runs. A per-run deployment writes **no** `workflow_run` row, so registry resolution never picks up an ephemeral per-run deployment. A run's deployment is **torn down on terminal status** — deleting its `workflow_deployment` projection row so a dead deployment's key can no longer satisfy a reconnect challenge — by the projection bridge (`tearDownDeployment`, gated on the first non-terminal→terminal transition via the shared terminal-status set in `apps/hub/src/workflow-executor/run-status.ts` — never on `awaiting`, the CL-2575 invariant). On restart the reconciler re-establishes per-run deployments from non-terminal `workflow_run_record` rows (recovering the deploy principal from the kind's registry row, since the per-run deploymentId lives on the record, not on `workflow_run`); a crash-orphan janitor (`reclaimOrphanedDeployments`) reclaims deployments whose teardown did not fire. Because the per-run repo is reclaimed on terminal, `workflow_run_record` is the durable run record after the log is gone. Follow-ups (trim the now-unused shared supervisor; widen the janitor window) in CL-2584.
...
> **Run-start** is wired on the user path `POST /api/v1/workflow-exec/:kind/start` (`apps/hub/src/routes/workflow-run-records.ts`), which provisions a per-run deployment (see § Per-run deployment) and triggers it; the seeded `workflow_run_record` row is the polled state the projection bridge advances. Interchange stays pinned to plain upstream (no fork). Every run-start path — this route, the `workflow_start` hub tool, webhook triggers, the scheduler, and the manual brief-run route — funnels its input through one kind-keyed trigger-payload enrichment registry rather than each caller building its own (CL-3891; see IMPLEMENTATION.md § Mailbox, routines, and tasks).
...
> **Forward path (substrate for multi-tenant SaaS):** this builds the per-org shape SaaS needs — root tenant → member principals → sub-tenant workbenches → tenant+principal-scoped data + credential inheritance. Going multi-tenant later is additive: replace the single env-seeded org with per-customer org provisioning and resolve the user's org by membership instead of the one constant. Treat the `GLOBAL_TENANT_*` env seed as a deliberately temporary v1 mechanism. The current deployment model is **isolated-infra-per-client** — one Railway stack per customer, each its own env-seeded org — with a committed non-secret manifest (`clients/<slug>.toml`) and a provisioning script that sets the per-service variables (see [IMPLEMENTATION.md](./IMPLEMENTATION.md) § Client deployment and [CLIENT_STANDUP.md](./CLIENT_STANDUP.md)). Collapsing many customers into one multi-tenant hub (per-customer org provisioning + membership-resolved org) is the additive future step, not yet built.
...
**Dynamic tool exposure (CL-2808).** Myra opts into a second director layer (`@workbench/agents/dynamic-tools`) that changes what is _advertised_ to the model per inference call — never what is loaded or authorized. The sidecar harness materializes every pinned tool package exactly as before; the director advertises `platform ∪ exposed`, where `platform` is the small always-visible set (`PERSONAL_AGENT_PLATFORM_TOOLS`: `search_tools`/`load_tools`, memory, the core artifact surface, workflow entry points, skill discovery) and everything else the pinned packages ship is catalog-managed and starts hidden. The catalog and the grant list derive from one source (`MYRA_CATALOG_PACKAGES` in `catalog.ts`) — a package's tools are never hand-listed twice, so pins, grants, and catalog cannot drift, and the partition (`platform ∪ catalog` covers every materialized tool) is structural rather than hand-maintained (CL-3190). Two always-advertised catalog tools (`search_tools`, `load_tools` in `@workbench/tools-catalog`) let the model discover hidden tools by deterministic keyword search and enable them by name or whole package; exposure is sticky for the session. The seam is a single mutable `ToolExposureState` the harness constructs and shares in-process between the catalog tool runner (mutates on `load_tools`) and the director (reads per infer via `env[DYNAMIC_TOOLS_ENV_KEY]`). Grants and the credential rail are untouched — a hidden tool remains authorized and dispatchable, so advertisement is purely a context-size optimization. The advertised catalog is gated to the tools that actually loaded: a catalog package whose tenant credential is unresolved is dropped fail-soft at launch, so it is omitted from `search_tools` results (and the director's env catalog) rather than advertised as a package the model can never call — the source of a repeated-search loop (CL-3133). Opt-in is a control-plane personal-agent identity marker (`<!-- workbench:personal-agent -->`, stamped by `buildPersonalAgentSystemPrompt` and stripped before inference — CL-3194) read by `resolveDynamicToolConfig`; matching model-facing role prose was brittle and broke on role rewrites. All other agents keep full advertisement.
...
**Inference routing is model-driven, ordered, and self-failing-over**: an agent's `credentialRequirement` names its credential, but the _inference source list_ is resolved from the agent's **model** against the tenant model catalog (`resolveModelSources`), which returns an ordered `InferenceSource[]` — head = default, tail = automatic failover chain — ordered by each offering's `priority`. The workbench routes every model through a **Bifrost gateway as the primary source**, with the direct providers (opencode-zen, native Anthropic/Google/OpenAI) kept as the failover tail, purely by giving Bifrost's offerings a lower priority — no runtime/adapter change. The provider rows, priority scheme, one-virtual-key-per-wire-format topology, and re-seed semantics (offerings reconcile priority in place; only a changed provider baseURL needs delete + re-seed) live in [ADMIN_CLI.md](./ADMIN_CLI.md) § Bifrost as the primary gateway. **Setting an Owner inference credential auto-seeds that provider's catalog slice** (`reconcileProviderCatalog` in `apps/hub/src/services/catalog-provider-seed.ts`): the `PUT /owner/credentials/:providerName` handler materializes the `model_provider` (bound to the new credential) + `model` + `model_offering` rows from `@workbench/catalog` and pushes sources to live sidecars, so the provider's models resolve without a manual **Seed model catalog** run. Owners enable/disable/remove the offerings their tenant owns from the Owner → Catalog page (native owned-offerings route). See [ADMIN_CLI.md](./ADMIN_CLI.md) § Owner-set inference providers.
...
**Tool availability is credential-gated, not per-client configured**: every tool package ships to every workbench via code — the set is identical across deployments and is never toggled per client. What a tool can be _used_ is gated purely by whether its credential resolves: a tool-only provider whose credential is unconfigured for the tenant is not constructed by the sidecar harness, so it is never in the runner and never advertised to the model (the hub omits unresolved providers from the credential env; `getToolCredential` then fails the factory, which the harness skips). Configuring a credential in the Owner UI is therefore what makes a tool available — there is no separate per-client tool enable/disable, and tools never appear in a client manifest.

**Tenant package-registry topology (CL-3656).** Builtin tool tarballs live in a single `package-registry` asset on the org root tenant, named `workspace-builtins` (`WORKSPACE_BUILTINS_REGISTRY`). Hub boot autopublish (`TOOL_REGISTRY_AUTOPUBLISH_ON_BOOT`) is the sole feeder for that asset in staging/prod: embedded tarballs from the hub image sync by integrity (same `name@version` overwrites in place — no semver auto-bump). Agents and sidecars resolve tools from that asset via tenant inheritance (`listAssetsForTenant`); a second asset on the same tenant (e.g. legacy `workbench-builtins`) with the same `name@version` but different bytes causes partial tool loads and must be removed in ops. When autopublish is on, hub startup fails if cross-asset `name@version` collisions remain or sync errors occur.
...
- **On-disk state**: Maintains per-agent git repositories and key pairs in `SIDECAR_DATA_DIR`. This directory must be backed by a persistent volume in production — loss of this data prevents the sidecar from reconnecting its agents to the hub. A deployment's footprint here is reclaimed when it is undeployed/superseded — the hub fires a teardown on every terminal run transition and a 24h janitor re-sweeps missed teardowns — so deployment churn cannot exhaust the volume (CL-2231); reclamation is sidecar-local and never mutates the hub's durable state. There is deliberately **no boot-time orphan sweep** (CL-3955 removed it: its hub-reported live set structurally excluded per-run deployments, so it deleted parked runs' history on every restart); dirs whose teardown misses both layers persist until a future reclaim design. The sidecar keeps **no deployment memory** beyond this durable run state (CL-3884): it boots empty — no boot-time deployment restore, records, dormant markers, or tombstones — and the hub control plane re-drives residency on demand (mail-wake for idle single-step agents, gate signals plus the awaiting prewarm for parked runs, the run liveness sweep failing running runs whose child died with the process). See IMPLEMENTATION.md § Sidecar deployment reclamation.
...
**Turn activity block.** `@workbench/chat` `AgentTurn` wraps each turn's reasoning and tool calls in one collapsible `ActivityBlock` (CL-3637) above the response. `reasoning-summary.ts` derives the collapsed one-line label (`rollingReasoningLabel`) — the latest meaningful reasoning step, with `isLowSignalReasoning` / `dedupeReasoningSteps` filtering tool-discovery narration (CL-3638). The same component renders for streaming and reloaded turns — but only while a turn is still live: for a **settled** turn, `ChatThread` no longer hands `AgentTurn` a message carrying reasoning/tool parts at all. `settled-turn-projection.ts` groups a turn's committed-segment agent messages by the `turnId` group key `composeChatMessages` stamps on each exchange's segments (never by adjacency — agent-initiated mail such as gate mail or briefs carries no key and is never folded into a reply) and projects the settled group down to the final answer, any UI-block-bearing segment, and any files it produced; reasoning, tool calls, and interstitial narration segments are dropped, so no activity block renders in chat once a turn settles. A group containing a failed segment is not projected — the failure state stays visible. **Live turns collapse interstitials the same way (CL-3948):** `projectLiveTurn` mirrors the settled-turn rule for a turn's already-settled segments — keep the final settled text line plus any segment carrying real output (UI block / file / attachment / image), drop earlier pure-narration lines — which matters for turns parked on a native approval gate, where the turn never settles and "Let me check…" narration would otherwise linger above the pending card; the same change deduplicates the final answer so it renders once, not as both an interstitial and the settled bubble. The full trace stays available in Insights → Trace, with a subtle per-turn "View trace" link as the escape hatch (`getTurnTraceHref`).
