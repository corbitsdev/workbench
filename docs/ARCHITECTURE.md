...
**Gate-parked runs hibernate** (CL-3104): a run sitting at an `awaitSignal` gate does not keep its workflow-child subprocess and supervisor resident for the whole human wait. The reconciler's awaiting sweep hibernates a routable deployment parked past `WORKFLOW_HIBERNATION_GRACE_MS` (default 120s) by sending the existing `agent.undeploy` frame with a well-known hibernate reason; the sidecar link routes it to a state-preserving `DeployRouter.hibernate` that kills the child and releases routing/residency while keeping the workflow-run repo, step agent-state repos, scratch, and durable conversation on disk. The undeploy ack drops the address from the hub's routable set, so the next gate signal's `ensureDeploymentRoutable` re-establishes the deployment and the fresh child resumes the run from the durable log (the same `recoverParkedRun` rail as restart recovery). The awaiting pre-warm is scoped to the same horizon — it re-establishes only runs parked less than the grace (crash recovery), never a hibernated one — and `reconcileAll`'s reconnect pass applies the same age scope, so nothing fights the hibernation sweep. Wake for a long-parked run is purely signal-driven.

**Accepted gate signals are durable** (CL-3104): both signal-delivery paths persist a pending-signal record (`workflow_run_record.pending_signal`: signalId/name/payload/receivedAt) BEFORE the fire-and-forget sidecar dispatch; the projection clears it once the run log folds a matching `SignalReceived` (by signalId) or the run reaches terminal. The reconciler's sweep re-delivers any pending signal older than a short landing delay — waking the deployment first, reusing the same signalId, and refreshing the record's timestamp conditionally in SQL (a refresh can never resurrect a record the projection just cleared) so a stalled pack pipeline gets one duplicate per backoff window (the duplicate-delivery bound is projection lag: the clear lands only when the receipt folds) — and never hibernates a run with a signal in flight; the scan covers `running` records too because resume optimistically flips the record before delivery. Acceptance onto this durable rail is guarded on both routes: the run must belong to the authorized deployment and be parked on an open gate for the named signal, so an unmatchable signal can never be persisted and re-delivered forever. The non-terminal clear is conditional in SQL on the log-proven signalId, so a newly-accepted next-gate signal is never erased by the previous gate's fold. The invariant: a 202-accepted signal is eventually delivered or the run fails visibly; it is never silently parked forever. Hibernation itself CAS-re-reads the record immediately before sending the teardown (still `awaiting`, unchanged `updatedAt`, no pending signal), shrinking the resume-then-kill race to the undeploy frame's flight time; the residual — a signal lands and the run resumes mid-flight, then the teardown kills the child mid-step — leaves a `running` run with a gone supervisor, which the run liveness sweep fails visibly (that run is not resumable). A hibernate whose ack path fails can leave the child resident while the hub unroutes the address; the sidecar deploy branch self-heals by tearing a resident supervisor down (state-preserving) before a wake re-deploy.
...
**Dynamic tool exposure (CL-2808).** Myra opts into a second director layer (`@workbench/agents/dynamic-tools`) that changes what is _advertised_ to the model per inference call — never what is loaded or authorized. The sidecar harness materializes every pinned tool package exactly as before; the director advertises `base ∪ catalog-tools ∪ exposed`, where the catalog-managed long-tail packages (Attio, Linear, Granola, Vercel, file parsing) start hidden. Two always-advertised catalog tools (`search_tools`, `load_tools` in `@workbench/tools-catalog`) let the model discover hidden tools by deterministic keyword search and enable them by name or whole package; exposure is sticky for the session. The seam is a single mutable `ToolExposureState` the harness constructs and shares in-process between the catalog tool runner (mutates on `load_tools`) and the director (reads per infer via `env[DYNAMIC_TOOLS_ENV_KEY]`). Grants and the credential rail are untouched — a hidden tool remains authorized and dispatchable, so advertisement is purely a context-size optimization. Opt-in is prompt-marker based (`resolveDynamicToolConfig`); all other agents keep full advertisement.
