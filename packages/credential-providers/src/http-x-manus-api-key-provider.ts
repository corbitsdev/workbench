...
// `http-x-api-key-provider.ts`. Manus authenticates via `x-manus-api-key`,
// not `x-api-key` (Exa/ScrapeCreators) and not `authorization` in any
// shape. Registering this as its own plugin, keyed `http-x-manus-api-key`,
// lets a tenant's Manus provider row opt into the header its API actually
// wants without changing the existing `http-x-api-key` plugin.
//
...
export interface HttpXManusApiKeyCredentialProviderOptions {
  /**
 ...
 * binding into an `x-manus-api-key` header shape.
 */
export const HTTP_X_MANUS_API_KEY_PROVIDER_KEY = "http-x-manus-api-key";

/**
 * Build the `http-x-manus-api-key` credential provider: an
 * `HttpMediatedCredential` whose `fetch` sends the secret in the
 * `x-manus-api-key` header. Register it alongside `@intx/harness`'s
 * `builtinCredentialProviders()` in a `CredentialProviderRegistry` and
 * point a provider row's `plugin` column at
 * `HTTP_X_MANUS_API_KEY_PROVIDER_KEY` to opt that credential's bindings
 * into this header shape.
 */
export function createHttpXManusApiKeyCredentialProvider(
  opts?: HttpXManusApiKeyCredentialProviderOptions,
): CredentialProvider {
  const fetchImpl: FetchLike = opts?.fetch ?? globalThis.fetch;

  return {
    key: HTTP_X_MANUS_API_KEY_PROVIDER_KEY,
      ...
      return {
        ...
        async fetch(
          ...
          if (target.origin !== pinnedOrigin) {
            throw new Error(
              `http-x-manus-api-key credential is pinned to ${pinnedOrigin}; refusing cross-origin request to ${target.origin}`,
            );
          }
          ...
          if (input instanceof Request) {
            ...
            headers.set("x-manus-api-key", secret);
            return fetchImpl(
        ...
        dispose(): void {
          // An x-manus-api-key http handle allocates no resources; nothing
          // to release.
        },
      };
    },
