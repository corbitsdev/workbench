...
function scratchUrlFor(e2eUrl: string): string {
  ...
  url.pathname = `/${database}_agent_directory_migrations_test`;
  return url.toString();
}
...
describeIfDb("applyAgentDirectoryMigrations", () => {
  ...
  test("applies the table into its own schema and is idempotent on a second run", async () => {
    ...
    const second = await applyAgentDirectoryMigrations(scratchUrl);
    ...
    try {
      ...
      const inPublic = await sql.unsafe(
        `SELECT table_name FROM information_schema.tables ` +
          `WHERE table_schema = 'public' AND table_name = 'definition_skills'`,
      );
