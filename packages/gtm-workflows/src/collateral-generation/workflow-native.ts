...
const intakeAgent = defineAgent({
  ...
  tools: [],
  capabilities: ['granola_list_notes', 'granola_get_note'],
  inference: {
...
const generateAgent = defineAgent({
  ...
  tools: [],
  capabilities: ['artifact_create', 'write_artifact'],
  inference: {
    sources: [{ provider: 'openai-compatible', model: LLM_DEFAULT_MODEL }],
  },
  tags: { credentialName: LLM_CREDENTIAL_NAME },
...
export const collateralGenerationWorkflowNative = defineWorkflow({
  id: 'collateral-generation',
  trigger: { type: 'manual' },
  steps: {
