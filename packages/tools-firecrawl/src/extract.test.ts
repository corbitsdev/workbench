...
describe('createExtractTools', () => {
  ...
  it('throws when apiKey is empty', () => {
    expect(() => createExtractTools({ apiKey: '' })).toThrow('Firecrawl apiKey is required');
  });
});
...
describe('firecrawl_extract_start handler', () => {
  it('posts urls and schema and returns the job id', async () => {
    ...
    const runner = createToolRunner(createExtractTools({ apiKey: 'test-key', fetcher }));
  ...
  it('requires a non-empty urls array', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_extract_start', arguments: { urls: [] } },
      new AbortController().signal
    );
...
describe('firecrawl_extract_status handler', () => {
  it('gets the job status by id', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_extract_status', arguments: { id: 'job-123' } },
      new AbortController().signal
    );
