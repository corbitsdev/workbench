...
function buildCrawlStartBody(args: Record<string, unknown>): Record<string, unknown> {
  ...
  if (typeof args.maxDepth === 'number') {
    body.maxDiscoveryDepth = optionalPositiveInteger(args.maxDepth, DEPTH_FALLBACK, MAX_DEPTH);
  }
...
async function crawlActive(
  ...
  return firecrawlFetchJSON(config, { method: 'GET', path: '/crawl/active' }, signal);
}
  ...
  inputSchema: {
    ...
    properties: {
      ...
      includePaths: {
        type: 'array',
        items: { type: 'string' },
        description: 'URL pathname regex patterns; only matching URLs are crawled.',
      },
      excludePaths: {
        type: 'array',
        items: { type: 'string' },
      ...
      scrapeOptions: {
        type: 'object',
        description: 'Scrape options applied to each crawled page (formats, content extraction).',
      },
    },
    required: ['url'],
  },
