...
describe('firecrawl_batch_scrape_start handler', () => {
  it('starts a batch scrape job', async () => {
    ...
    const runner = createToolRunner(createBatchScrapeTools({ apiKey: 'test-key', fetcher }));
    ...
    expect(JSON.parse(String(result.content))).toEqual({ success: true, id: 'batch_1' });
  ...
  it('requires a non-empty urls array', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_batch_scrape_start', arguments: { urls: [] } },
      new AbortController().signal
    );
...
describe('firecrawl_batch_scrape_status handler', () => {
  it('gets batch scrape status by id', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_batch_scrape_status', arguments: { id: 'batch 1' } },
      new AbortController().signal
    );

    expect(result.isError).toBeUndefined();
    const call = fetcher.mock.calls[0];
    expect(call?.[0]).toBe('https://api.firecrawl.dev/v2/batch/scrape/batch%201');
