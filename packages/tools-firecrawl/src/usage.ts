...
async function creditUsage(
  ...
  return firecrawlFetchJSON(config, { method: 'GET', path: '/team/credit-usage' }, signal);
}
...
async function tokenUsage(
  ...
  return firecrawlFetchJSON(config, { method: 'GET', path: '/team/token-usage' }, signal);
}
...
export const FIRECRAWL_HISTORICAL_CREDIT_USAGE_DEFINITION: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      byApiKey: {
        type: 'boolean',
        description: 'When true, break historical credit usage down by API key.',
      },
    },
  },
};
...
export const FIRECRAWL_HISTORICAL_TOKEN_USAGE_DEFINITION: ToolDefinition = {
  ...
  description: 'Get historical Firecrawl token usage by billing period for the authenticated team.',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
export const FIRECRAWL_ACTIVITY_DEFINITION: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      endpoint: {
        type: 'string',
        description: 'Filter activity by endpoint name (e.g. "scrape", "crawl").',
      },
      limit: {
        type: 'number',
...
export function createUsageTools(config: FirecrawlToolsConfig): AgentTool[] {
  ...
  return [
    ...
    stringTool(FIRECRAWL_ACTIVITY_DEFINITION, (args, signal) => activity(resolved, args, signal)),
  ];
}
