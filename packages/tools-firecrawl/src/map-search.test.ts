...
describe('createMapSearchTools', () => {
  ...
  it('throws when apiKey is empty', () => {
    expect(() => createMapSearchTools({ apiKey: '' })).toThrow('Firecrawl apiKey is required');
  });
});
...
describe('firecrawl_map handler', () => {
  it('maps a site and returns the parsed response', async () => {
    ...
    const runner = createToolRunner(createMapSearchTools({ apiKey: 'test-key', fetcher }));

    const result = await runner.run(
      {
        id: 'call_1',
        name: 'firecrawl_map',
        arguments: { url: 'https://example.com', search: 'docs', sitemapOnly: true },
      },
