...
describe('createFirecrawlTools', () => {
  ...
  it('throws when apiKey is empty', () => {
    expect(() => createFirecrawlTools({ apiKey: '' })).toThrow('Firecrawl apiKey is required');
  });
});
...
describe('FIRECRAWL_HUB_TOOLS', () => {
  it('registers one hub entry per Firecrawl definition', () => {
    ...
    expect(FIRECRAWL_HUB_TOOLS.firecrawl_scrape?.providerName).toBe('firecrawl');
  });
  ...
  it('creates only the requested tool handler', async () => {
    ...
    expect(tools?.map((tool) => tool.definition.name)).toEqual(['firecrawl_scrape']);
