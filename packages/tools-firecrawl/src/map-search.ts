...
async function mapSite(
  ...
  return firecrawlFetchJSON(config, { method: 'POST', path: '/map', body }, signal);
}
...
async function searchWeb(
  ...
  const body: Record<string, unknown> = {
    query,
    limit: optionalPositiveInteger(args.limit, SEARCH_DEFAULT_LIMIT, SEARCH_MAX_LIMIT),
    ...(sources !== null ? { sources } : {}),
    ...(tbs !== null ? { tbs } : {}),
    ...(scrapeOptions !== null ? { scrapeOptions } : {}),
  };

  return firecrawlFetchJSON(config, { method: 'POST', path: '/search', body }, signal);
}
...
export const FIRECRAWL_MAP_DEFINITION: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      search: {
        type: 'string',
        description: 'Optional search term used to filter and rank discovered links.',
      },
      limit: {
        type: 'number',
        description: 'Maximum number of links to return (1-100000, default 5000).',
      },
      includeSubdomains: {
        type: 'boolean',
        description: 'Include links on subdomains of the target site. Default true.',
      },
      sitemapOnly: {
        type: 'boolean',
        description: 'Only return links found in the site sitemap. Default false.',
      },
    },
    required: ['url'],
  },
  ...
  inputSchema: {
    ...
    properties: {
      ...
      sources: {
        type: 'array',
        items: { type: 'string' },
        description: 'Source types to search: web, news, images. Defaults to web.',
      },
      tbs: {
        type: 'string',
      ...
      scrapeOptions: {
        type: 'object',
        description: 'Firecrawl scrape options applied to each result to fetch its content.',
      },
    },
    required: ['query'],
  },
...
export function createMapSearchTools(config: FirecrawlToolsConfig): AgentTool[] {
  const resolved = resolveConfig(config);

  return [
    stringTool(FIRECRAWL_MAP_DEFINITION, (args, signal) => mapSite(resolved, args, signal)),
    stringTool(FIRECRAWL_SEARCH_DEFINITION, (args, signal) => searchWeb(resolved, args, signal)),
  ];
}
