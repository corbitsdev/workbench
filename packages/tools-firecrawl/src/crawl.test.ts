...
describe('createCrawlTools', () => {
  ...
  it('throws when apiKey is empty', () => {
    expect(() => createCrawlTools({ apiKey: '' })).toThrow('Firecrawl apiKey is required');
  });
});

describe('firecrawl_crawl_start handler', () => {
  it('posts to /crawl with mapped body and parses the response', async () => {
    const stubResponse = { success: true, id: 'job_1', url: 'https://example.com' };
    const fetcher = makeFetchStub(stubResponse);
    const runner = createToolRunner(createCrawlTools({ apiKey: 'test-key', fetcher }));
  ...
  it('surfaces API errors', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_crawl_start', arguments: { url: 'https://example.com' } },
      new AbortController().signal
    );
...
describe('firecrawl_crawl_status handler', () => {
  it('gets /crawl/{id} and parses the response', async () => {
    const stubResponse = { status: 'completed', total: 1, completed: 1, data: [] };
    const fetcher = makeFetchStub(stubResponse);
    const runner = createToolRunner(createCrawlTools({ apiKey: 'test-key', fetcher }));

    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_crawl_status', arguments: { id: 'job 1' } },
      new AbortController().signal
    );
...
describe('firecrawl_crawl_cancel handler', () => {
  it('deletes /crawl/{id} and parses the response', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_crawl_cancel', arguments: { id: 'job_1' } },
      new AbortController().signal
    );
