...
describe('createParseTools', () => {
  ...
  it('throws when apiKey is empty', () => {
    expect(() => createParseTools({ apiKey: '' })).toThrow('Firecrawl apiKey is required');
  });
});

describe('firecrawl_parse handler', () => {
  it('downloads a document and parses it', async () => {
    const parseResponse = { success: true, data: { markdown: '# Parsed Title' } };
    const fetcher = makeFetchStub(parseResponse);
    const runner = createToolRunner(createParseTools({ apiKey: 'test-key', fetcher }));

    const result = await runner.run(
      {
        id: 'call_1',
        name: 'firecrawl_parse',
        arguments: { url: 'https://example.com/document.pdf', options: { formats: ['markdown'] } },
      },
  ...
  it('surfaces API errors', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_parse', arguments: { url: 'https://example.com/doc.pdf' } },
      new AbortController().signal
    );
