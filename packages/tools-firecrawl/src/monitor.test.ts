...
describe('firecrawl_monitor_create handler', () => {
  it('creates a monitor', async () => {
    ...
    const runner = createToolRunner(createMonitorTools({ apiKey: 'test-key', fetcher }));

    const result = await runner.run(
      {
        id: 'call_1',
        name: 'firecrawl_monitor_create',
        arguments: { config: { name: 'Homepage watcher', url: 'https://example.com' } },
      },
...
describe('firecrawl_monitor_get handler', () => {
  it('gets a monitor by id', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_monitor_get', arguments: { id: 'mon_1' } },
      new AbortController().signal
    );
...
describe('firecrawl_monitor_delete handler', () => {
  it('deletes a monitor', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_monitor_delete', arguments: { id: 'mon_1' } },
      new AbortController().signal
    );
...
describe('firecrawl_monitor_run handler', () => {
  it('runs a monitor check', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_monitor_run', arguments: { id: 'mon_1' } },
      new AbortController().signal
    );
...
describe('firecrawl_monitor_check handler', () => {
  it('lists monitor checks', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_monitor_check', arguments: { id: 'mon_1' } },
      new AbortController().signal
    );
