...
describe('firecrawl_credit_usage handler', () => {
  it('gets current credit usage', async () => {
    const fetcher = makeFetchStub({ success: true, credits: { used: 100, remaining: 400 } });
    const runner = createToolRunner(createUsageTools({ apiKey: 'test-key', fetcher }));
...
describe('firecrawl_historical_credit_usage handler', () => {
  it('gets historical credit usage with byApiKey', async () => {
    ...
    const result = await runner.run(
      { id: 'call_1', name: 'firecrawl_historical_credit_usage', arguments: { byApiKey: true } },
      new AbortController().signal
    );
...
describe('firecrawl_historical_token_usage handler', () => {
  it('gets historical token usage', async () => {
    ...
    expect(result.isError).toBeUndefined();
    const call = fetcher.mock.calls[0];
    expect(call?.[0]).toBe('https://api.firecrawl.dev/v2/team/token-usage/historical');
    expect(call?.[1].method).toBe('GET');
  });
});
