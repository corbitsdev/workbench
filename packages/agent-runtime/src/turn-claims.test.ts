...
describe("createInMemoryTurnClaimStore", () => {
  ...
  test("claims on different workbenches never contend", async () => {
    const store = createInMemoryTurnClaimStore({ ttlMs: 60_000 });
    await expect(store.tryClaim({ workbenchId: "wb_1" })).resolves.not.toBe(false);
    await expect(store.tryClaim({ workbenchId: "wb_2" })).resolves.not.toBe(false);
  });

  test("release frees the claim for a fresh tryClaim", async () => {
    const store = createInMemoryTurnClaimStore({ ttlMs: 60_000 });
    const token = await store.tryClaim({ workbenchId: "wb_1" });
    await expect(store.release({ workbenchId: "wb_1" }, token as string)).resolves.toBe(true);
    await expect(store.tryClaim({ workbenchId: "wb_1" })).resolves.not.toBe(false);
  });

  test("releasing a claim nobody holds is a harmless no-op", async () => {
    const store = createInMemoryTurnClaimStore({ ttlMs: 60_000 });
    await expect(store.release({ workbenchId: "wb_never_claimed" }, "some_token")).resolves.toBe(
      false,
    );
  });
  ...
  test("releasing with a stale token — one the TTL already reassigned — is a no-op", async () => {
    ...
    await expect(store.release({ workbenchId: "wb_1" }, firstToken as string)).resolves.toBe(false);
    await expect(store.holds({ workbenchId: "wb_1" }, secondToken as string)).resolves.toBe(true);
  });
  ...
  test("a mismatched release evicts an already-expired entry, so the true original token then finds nothing left to release", async () => {
    ...
    await expect(store.release({ workbenchId: "wb_1" }, "not_the_real_token")).resolves.toBe(false);

    // Proof the entry is actually gone, not merely ignored: the true
    // original token — still technically correct — now finds nothing
    // left to release.
    await expect(store.release({ workbenchId: "wb_1" }, token as string)).resolves.toBe(false);
  });
  ...
  describe("holds", () => {
    test("is true for the current, unexpired holder's own token", async () => {
      const store = createInMemoryTurnClaimStore({ ttlMs: 60_000 });
      const token = await store.tryClaim({ workbenchId: "wb_1" });
      await expect(store.holds({ workbenchId: "wb_1" }, token as string)).resolves.toBe(true);
    });

    test("is false for a token nobody ever held", async () => {
      const store = createInMemoryTurnClaimStore({ ttlMs: 60_000 });
      await expect(store.holds({ workbenchId: "wb_1" }, "never_claimed")).resolves.toBe(false);
    });
    ...
    test("is false once the TTL expires, even without a fresh tryClaim", async () => {
      ...
      const token = await store.tryClaim({ workbenchId: "wb_1" });
      now += 1_000;
      await expect(store.holds({ workbenchId: "wb_1" }, token as string)).resolves.toBe(false);
    });
