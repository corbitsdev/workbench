 ...
 */
function stubFetch(responder: (index: number, call: FetchCall) => Response): void {
  let index = 0;
...
function getTool(overrides: Partial<typeof BASE_OPTS> = {}): RawTool {
  return createAskPrincipalTool({ ...BASE_OPTS, ...overrides }) as unknown as RawTool;
}
...
describe('tool definition', () => {
  test('exposes the ask_principal tool with action and resource required', () => {
    ...
    expect(Object.keys(t.definition.inputSchema.properties)).toContain('context');
  });
});
...
describe('create request', () => {
  test('POSTs the approval to the internal endpoint with auth and full payload', async () => {
    stubFetch((i) =>
      i === 0
        ...
        : makeResponse(200, { id: 'appr-1', status: 'approved', message: null })
    );

    const t = getTool();
    await t.handler(
      { id: 'call-1', arguments: { action: 'do x', resource: 'r://x', context: { k: 'v' } } },
  ...
  test('strips a trailing slash from hubHttpUrl', async () => {
    ...
    await t.handler(CALL, new AbortController().signal);
    expect(calls[0]!.url).toBe('https://hub.example.com/api/internal/approvals');
  });
...
describe('decision polling', () => {
  test('returns Approved with the message when the principal approves', async () => {
    stubFetch((i) =>
      i === 0
        ? makeResponse(201, { id: 'appr-1', status: 'pending', message: null })
        : makeResponse(200, { id: 'appr-1', status: 'approved', message: 'go ahead' })
    );
    const t = getTool();
    const result = await t.handler(CALL, new AbortController().signal);
  ...
  test('returns Rejected with the message when the principal rejects', async () => {
    stubFetch((i) =>
      i === 0
        ? makeResponse(201, { id: 'appr-1', status: 'pending', message: null })
        : makeResponse(200, { id: 'appr-1', status: 'rejected', message: 'not allowed' })
    );
    const t = getTool();
    const result = await t.handler(CALL, new AbortController().signal);
    expect(result.isError).toBeUndefined();
  ...
  test('polls the record by id with the tenantId query param', async () => {
    stubFetch((i) =>
      i === 0
        ...
        : makeResponse(200, { id: 'appr-99', status: 'approved', message: null })
    );
    const t = getTool();
    await t.handler(CALL, new AbortController().signal);
    const poll = calls[1]!;
  ...
  test('keeps polling while pending and resolves once status changes', async () => {
    stubFetch((i) => {
      if (i === 0) return makeResponse(201, { id: 'a', status: 'pending', message: null });
      if (i < 3) return makeResponse(200, { id: 'a', status: 'pending', message: null });
...
describe('cancellation', () => {
  ...
  test('does not poll if the signal is already aborted before the wait completes', async () => {
    stubFetch((i) =>
      makeResponse(i === 0 ? 201 : 200, { id: 'a', status: 'pending', message: null })
    );
    const controller = new AbortController();
    controller.abort();
    const t = getTool({ pollIntervalMs: 10_000 });
