  ...
  return (
    <div className="flex h-full flex-col gap-2 p-3">
      ...
      <div className="space-y-1.5">
        ...
        <div className="h-2 w-[92%] rounded-sm bg-foreground/8" />
        <div className="h-2 w-[78%] rounded-sm bg-foreground/8" />
      </div>
      {excerpt ? (
        <p className="mt-auto line-clamp-2 text-[10px] leading-snug text-muted-foreground">
          {excerpt}
        </p>
      ) : null}
    </div>
...
function SocialPreview({ fill, excerpt }: { fill: string; excerpt?: string }) {
  return (
    <div className="flex h-full flex-col p-3">
      ...
      <p className="mt-2 line-clamp-3 flex-1 text-[10px] leading-snug text-foreground/80">
  ...
  return (
    <div className="flex h-full flex-col gap-2 p-3 text-left">
      <div className="space-y-1 border-b border-border/60 pb-2">
        <div className="h-1.5 w-10 rounded-sm bg-foreground/12" />
        <div className={`h-2 w-[70%] rounded-sm ${fill} opacity-30`} />
      </div>
      <p className="line-clamp-4 flex-1 text-[10px] leading-snug text-muted-foreground">
  ...
  return (
    <div className="flex h-full gap-2 p-3">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1.5">
        <div className="h-2 w-[85%] rounded-sm bg-foreground/12" />
        <div className="h-1.5 w-full rounded-sm bg-foreground/8" />
        <div className="h-1.5 w-[90%] rounded-sm bg-foreground/8" />
        {excerpt ? (
...
function ComparisonPreview({ fill }: { fill: string }) {
  return (
    <div className="grid h-full grid-cols-2 gap-px bg-border/40 p-3">
      <div className="flex flex-col gap-1.5 rounded-sm bg-background/80 p-2">
        <div className={`h-1.5 w-8 rounded-full ${fill}`} />
        <div className="h-2 w-full rounded-sm bg-foreground/10" />
        <div className="h-2 w-[80%] rounded-sm bg-foreground/8" />
      </div>
      <div className="flex flex-col gap-1.5 rounded-sm bg-background/80 p-2">
        <div className="h-1.5 w-8 rounded-full bg-foreground/10" />
        <div className="h-2 w-full rounded-sm bg-foreground/10" />
        <div className="h-2 w-[70%] rounded-sm bg-foreground/8" />
      </div>
    </div>
  );
}
...
function PresentationPreview({ fill }: { fill: string }) {
  return (
    <div className="flex h-full items-center justify-center p-3">
      <div
        className={`flex aspect-[16/10] w-full max-w-[140px] flex-col items-center justify-center gap-2 rounded-md border border-border/50 bg-background/90 shadow-sm ${fill} bg-opacity-10`}
      >
        <div className={`h-2 w-16 rounded-sm ${fill} opacity-50`} />
        <div className="h-1.5 w-20 rounded-sm bg-foreground/10" />
        <div className="h-1.5 w-14 rounded-sm bg-foreground/8" />
      </div>
    </div>
  );
}
    ...
    <div className="flex h-full flex-col justify-end gap-1 p-3 pb-4">
      <div className="flex h-14 items-end justify-center gap-1.5">
        {[40, 65, 50, 80, 55].map((h, i) => (
          <div
            key={i}
            className={`w-3 rounded-t-sm ${i === 3 ? fill : "bg-foreground/15"}`}
            style={{ height: `${h}%` }}
          />
        ))}
      </div>
      <div className="mx-auto h-1 w-16 rounded-full bg-foreground/10" />
    </div>
  );
}
