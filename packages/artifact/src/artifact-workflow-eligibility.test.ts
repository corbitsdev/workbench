...
describe("workflowAcceptsArtifactKind", () => {
  it("pain-point-collateral accepts only its specific source kinds", () => {
    expect(
      workflowAcceptsArtifactKind("pain-point-collateral", "call-transcript"),
    ).toBe(true);
    expect(
      workflowAcceptsArtifactKind("pain-point-collateral", "pain-points"),
    ).toBe(true);
    expect(workflowAcceptsArtifactKind("pain-point-collateral", "email")).toBe(
      false,
    );
  });
  ...
  it("gamma-presentation-creator accepts any artifact kind (general source)", () => {
    expect(
      workflowAcceptsArtifactKind("gamma-presentation-creator", "email"),
    ).toBe(true);
    expect(
      workflowAcceptsArtifactKind("gamma-presentation-creator", "pain-points"),
    ).toBe(true);
    expect(
      workflowAcceptsArtifactKind("gamma-presentation-creator", "battlecard"),
    ).toBe(true);
  });

  it("rejects unknown workflow kinds", () => {
    expect(workflowAcceptsArtifactKind("nonexistent", "call-transcript")).toBe(
      false,
    );
  });
});
...
describe("sourceArtifactKindSkipsAnalysis", () => {
  it("skips analysis when collateral generation seeds from an already-analyzed pain-points artifact", () => {
    expect(
      sourceArtifactKindSkipsAnalysis("pain-point-collateral", "pain-points"),
    ).toBe(true);
  });
