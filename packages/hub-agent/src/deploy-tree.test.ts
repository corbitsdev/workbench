...
async function tempDir(): Promise<string> {
  const d = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'deploy-tree-test-'));
  tempDirs.push(d);
  return d;
}
...
describe('readDeployTree', () => {
  ...
  test('reads prompt.md from deploy directory', async () => {
    ...
    await fs.promises.writeFile(path.join(dir, 'deploy', 'prompt.md'), 'You are a test agent.');
  ...
  test('returns corrupt-JSON bytes as-is for the caller to handle', async () => {
    ...
    await fs.promises.writeFile(path.join(dir, 'deploy', 'tool-packages-manifest.json'), garbage);
    const result = await readDeployTree(dir);
