...
import type { BuildHarnessArgs, HarnessBuilder, HarnessBundle } from './harness-builder';
...
describe('SessionManager.updateSources', () => {
  ...
  test('installs the full ordered list on the running harness', async () => {
    ...
    expect(recorded.sources.map((s) => s.id)).toEqual(['test:test-model', 'test:fallback']);
  });
...
describe('SessionManager.startSession rollback when builder throws', () => {
  test('restores provisioned state and unregisters the transport', async () => {
    ...
    await manager.provisionAgent(cfg);
    await expect(manager.startSession('agent@local')).rejects.toThrow('builder boom');
...
describe('SessionManager.onAgentEvent per-agent fan-out', () => {
  test("delivers events only to the subscribed agent's listeners and respects the disposer", async () => {
    ...
    const aliceBuild = builder.buildCalls.find((c) => c.agentAddress === 'alice@local');
    const bobBuild = builder.buildCalls.find((c) => c.agentAddress === 'bob@local');
    ...
    const dispose = manager.onAgentEvent('alice@local', (e) => aliceEvents.push(e));
