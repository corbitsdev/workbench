...
async function buildAssetPack(files: Record<string, string>): Promise<BuiltPack> {
...
describe('applyAssetPack', () => {
  test('materializes pack contents at <workspaceRoot>/<mountPath>/', async () => {
    ...
    const skillBody = await fsp.readFile(path.join(mountDir, 'greet', 'SKILL.md'), 'utf-8');
    expect(skillBody).toContain('name: greet');

    const scriptBody = await fsp.readFile(path.join(mountDir, 'greet', 'script.sh'), 'utf-8');
  ...
  test('leaves sibling mounts untouched', async () => {
    ...
    const sibling = await fsp.readFile(path.join(siblingDir, 'keep.txt'), 'utf-8');
  ...
  test('does not leak a scratch .git into the workspace root', async () => {
    ...
    const scratchLeftover = entries.filter((e) => e.startsWith('.intx-asset-scratch-'));
  ...
  test('rejects mountPath that resolves to workspace root', async () => {
    ...
    await fsp.writeFile(path.join(sentinelDir, 'do-not-delete.txt'), 'sentinel');
    ...
    const sentinel = await fsp.readFile(path.join(sentinelDir, 'do-not-delete.txt'), 'utf-8');
