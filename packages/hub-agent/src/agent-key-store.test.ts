...
const stubCrypto = {
  signEd25519: (_privateKey: Uint8Array, _payload: Uint8Array) => new Uint8Array(64),
  verifySSHSig: (_payload: string, _signature: string, _publicKey: Uint8Array) => true,
};
...
describe('AgentKeyStore — load-or-generate', () => {
  ...
  test('missing one half of the key pair surfaces a corruption error', async () => {
    ...
    const pubPath = path.join(dataDir, 'agent_at_local', 'keys', 'id_ed25519.pub');
    await fs.rm(pubPath);

    await expect(store.loadOrGenerateKey('agent@local')).rejects.toThrow(/Corrupt key pair/);
  });
});
