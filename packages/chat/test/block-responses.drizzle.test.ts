...
describeIfDb("createDrizzleBlockResponseStore: concurrent notification claim", () => {
  ...
  test("two concurrent claims for the same key never throw, and exactly one wins", async () => {
    ...
    try {
      ...
      expect(rows[0]?.notification_claim_token).toBe(outcomes.find((outcome) => outcome !== false));
    } finally {
      await sql.end();
    }
  });
