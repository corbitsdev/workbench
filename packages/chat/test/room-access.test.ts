...
// workbench), one who created the workbench (and so is stamped as its
// `chat/createdBy`) and one who is only ever a member of the owning
// bench.
...
describe("room access", () => {
  ...
  test("a non-member is denied once a workbench is flipped to members-only", async () => {
    ...
    // Alice created the workbench, so creation stamped her as its
    // `chat/createdBy` — she keeps access.
    const aliceRead = await asAlice.request(`/workbenches/${body.id}/messages`);
    expect(aliceRead.status).toBe(200);

    // Bob holds no participant row on the workbench, and it no longer
  ...
  test("inviting a person records them as a human participant, unblocking a members-only flip", async () => {
    ...
    // (CL-6332) before the workbench ever flips to members-only — the
    // participant record lands on invite, not on first access.
    ...
    // the members-only check reads his participant row on the
    // workbench, not whether he was invited before or after the flip.
    const bobRead = await asBob.request(`/workbenches/${body.id}/messages`);
