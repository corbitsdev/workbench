...
const messages: ChatMessage[] = [
  { id: '1', role: 'agent', content: 'Hello', createdAt: '2026-06-04T00:00:00Z' },
  { id: '2', role: 'user', content: 'Hi back', createdAt: '2026-06-04T00:01:00Z' },
];
...
describe('ChatThread', () => {
  ...
  it('renders a tool narrative for agent messages with tool calls', () => {
    ...
    render(<ChatThread messages={withTools} formatToolSummary={() => 'searched the web'} />);
  ...
  it('does not render a tool narrative when toolCalls is empty', () => {
    ...
    render(<ChatThread messages={noTools} formatToolSummary={() => 'should not appear'} />);
  ...
  it('formats each activity variant with the agent name', () => {
    ...
    expect(screen.getByText('Ada is rate-limited, retrying in 3s')).toBeDefined();
  });
