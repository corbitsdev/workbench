 ...
 * `kind: "chat"` and a `definitionId` always find-or-reopens (CL-6981):
 * identity is the definition's ASSET via `resolveDefinitionAssetId`, and
 * the oldest workbench-tenancy `createdAt` wins. No title match.
 *
...
export function createChatRoutes(deps: CreateChatRoutesDeps): Hono<TenantEnv> {
  ...
  app.post(
    ...
    async (c) => {
      ...
      // Opening an agent always find-or-reopens its one conversation
      // (CL-6981): identity is the definition's asset, oldest createdAt
      // wins. Checked before anything is minted, and before the (cheaper,
      // in-memory) principal-self-chat validation below, since a found
      // match short-circuits the whole handler — a reopen must not launch
      // again.
