...
describe("summarizeWorkbenchActivity", () => {
  test("merges latest-message and unread-count aggregates by workbench", () => {
    const workbenchSessionIds = new Map([
      ["ch_general", "sess_1"],
      ["ch_random", "sess_2"],
    ]);
    const result = summarizeWorkbenchActivity(
      workbenchSessionIds,
      [
  ...
  test("a workbench with messages but nothing unread reports unreadCount: 0", () => {
    const result = summarizeWorkbenchActivity(
  ...
  test("a workbench with no messages yet reports unreadCount: 0 and no lastActivityAt", () => {
    const result = summarizeWorkbenchActivity(
  ...
  test("a workbench absent from the resolved session map never appears in the result", () => {
    const result = summarizeWorkbenchActivity(new Map(), [], []);
    expect(result).toEqual({});
  });
});
