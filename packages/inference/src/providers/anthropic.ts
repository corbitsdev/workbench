...
function isOpus48Model(model: string): boolean {
  ...
  return lower.includes("claude-opus-4-8") || lower.includes("opus-4-8");
}
...
function buildRequest(
  ...
  // WORKBENCH-LOCAL (CL-3766): Opus 4.8 thinking uses `effort` (not budget_tokens);
  // omit temperature when thinking is on for that model to avoid provider 400s.
  const anthropicEffort =
    options.providerOptions !== undefined &&
    typeof options.providerOptions["anthropicThinkingEffort"] === "string"
      ? options.providerOptions["anthropicThinkingEffort"]
      : undefined;
  const opusEffortModel = isOpus48Model(model);

  if (options.thinking?.enabled) {
    if (opusEffortModel && anthropicEffort !== undefined) {
      body["thinking"] = {
        type: "enabled",
  ...
  const thinkingOn = options.thinking?.enabled === true;
  if (options.temperature !== undefined && !(thinkingOn && opusEffortModel)) {
    body["temperature"] = options.temperature;
  }

  if (options.providerOptions !== undefined) {
    const { anthropicThinkingEffort: _effort, ...rest } =
