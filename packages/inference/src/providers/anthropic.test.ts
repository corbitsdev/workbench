...
describe("Anthropic adapter — tool-name codec round-trip", () => {
  ...
  test("opus 4.8 thinking with effort omits temperature (CL-3766)", () => {
    const adapter = createAnthropicAdapter(TEST_SOURCE);
    const req = adapter.buildRequest([], "claude-opus-4-8", {
      maxTokens: 100,
      temperature: 0.9,
      thinking: { enabled: true },
      providerOptions: { anthropicThinkingEffort: "medium" },
    });
    const body = JSON.parse(req.body) as Record<string, unknown>;
    expect(body["temperature"]).toBeUndefined();
    expect(body["thinking"]).toEqual({ type: "enabled", effort: "medium" });
  });

  test("opus 4.8 without thinking keeps temperature", () => {
    const adapter = createAnthropicAdapter(TEST_SOURCE);
    const req = adapter.buildRequest([], "claude-opus-4-8", {
      maxTokens: 100,
      temperature: 0.5,
