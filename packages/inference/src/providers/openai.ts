...
// WORKBENCH-LOCAL (CL-3766): Kimi K2.6 thinking via providerOptions.thinking;
// assistant turns need empty reasoning_content when thinking is enabled.
function isKimiThinkingEnabled(
...
function buildRequest(
  ...
  // WORKBENCH-LOCAL (CL-3766): merge reasoning_effort / thinking blobs from providerOptions.
  const kimiThinkingEnabled =
  ...
  if (kimiThinkingEnabled) {
    ...
    body["messages"] = options.systemPrompt
      ? [
          { role: "system", content: options.systemPrompt },
          ...convertedMessages,
        ]
