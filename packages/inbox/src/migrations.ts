// A URL-shaped wrapper around `@corbits/mailbox`'s own
// `runMailboxMigrations`, so scripts/db-setup.ts can apply it the same
// way it applies every other installed package's migrations. That
// package keeps its own ledger inside the `mailbox` schema.
