...
`@intx/agent`'s full provider surface. A caller installing the reviewer
roster into an agent-directory (`codeReviewAgentRequests`) imports it
through these subpaths for exactly that reason.
