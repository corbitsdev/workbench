...
import { CreateSessionResponse, SessionStatusResponse, SessionSummary } from './schemas';
...
export async function createSession(
  ...
  if (session instanceof type.errors) {
    throw new Error(`Browserbase create-session response invalid: ${session.summary}`);
  }
...
export async function waitForSessionRunning(
  ...
  while (Date.now() < deadline) {
    ...
    if (session instanceof type.errors) {
      throw new Error(`Browserbase session status response invalid: ${session.summary}`);
    }
