...
async function afterCreate(
  ...
  await runner.run(
    { id: "c0", name: "browser_create_session", arguments: {} },
    signal(),
  );
...
describe("createBrowserTools", () => {
  ...
  it("fails loud without a projectId", () => {
    expect(() => createBrowserTools({ apiKey: "k" })).toThrow(
...
describe("browser_navigate", () => {
  it("connects with the Browserbase-provided connectUrl, not a synthesized URL", async () => {
    ...
    expect(cdpControl.lastConnectUrl).not.toContain(
