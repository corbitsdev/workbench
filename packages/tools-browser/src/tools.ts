...
// Walk any frame chain (`<frame> >>> <inner>`) via frameLocator so iframe
// elements resolve.
...
async function resolveUnique(page: Page, ref: string): Promise<Locator> {
  ...
  return [
    ...
    stringTool(BROWSER_GET_TEXT_DEFINITION, async (args) => {
      ...
      return withSession(config, sessionId, async (page) => {
        ...
        return {
          text: await locator.first().innerText({ timeout: ACTION_TIMEOUT_MS }),
        };
      });
    }),
...
// `?projectId=` and is split out here.
