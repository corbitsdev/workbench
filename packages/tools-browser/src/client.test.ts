...
describe('createSession', () => {
  it('sends keepAlive:true, posts projectId and timeout, returns sessionId and connectUrl', async () => {
    ...
    const result = await createSession({ ...baseConfig, fetcher }, 600, signal());
  ...
  it('rejects a response with no id', async () => {
    ...
    await expect(createSession({ ...baseConfig, fetcher }, 600, signal())).rejects.toThrow(
      'id must be a string'
    );
  });
  ...
  it('surfaces API errors', async () => {
    ...
    await expect(createSession({ ...baseConfig, fetcher }, 600, signal())).rejects.toThrow(
      'Browserbase API error: 401 bad key'
    );
  });
});
...
describe('endSession', () => {
  ...
  it('surfaces API errors', async () => {
    ...
    await expect(endSession({ ...baseConfig, fetcher }, 's', signal())).rejects.toThrow(
      'Browserbase API error: 500'
    );
  });
});
...
describe('listRunningSessions', () => {
  it('requests RUNNING sessions and drops entries that fail validation', async () => {
    ...
    const sessions = await listRunningSessions({ ...baseConfig, fetcher }, signal());
  ...
  it('throws when the response is not an array', async () => {
    ...
    await expect(listRunningSessions({ ...baseConfig, fetcher }, signal())).rejects.toThrow(
      'not an array'
    );
  });
});
...
describe('reapStaleSessions', () => {
  ...
  it('skips sessions with no parseable age rather than guessing', async () => {
    ...
    const result = await reapStaleSessions({ ...baseConfig, fetcher }, 1000, now, signal());
...
describe('waitForSessionRunning', () => {
  ...
  it('resolves immediately when the session is already RUNNING', async () => {
    ...
    await expect(
      waitForSessionRunning({ ...baseConfig, fetcher }, 'sess-1', signal(), opts)
  ...
  it('polls until RUNNING then resolves', async () => {
    ...
    await waitForSessionRunning({ ...baseConfig, fetcher }, 'sess-1', signal(), opts);
