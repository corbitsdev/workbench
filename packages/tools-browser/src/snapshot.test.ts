...
import { buildRef, MAX_SNAPSHOT_ELEMENTS, pruneSnapshot, type RawSnapshot } from './snapshot';
...
describe('buildRef', () => {
  it('prefers id', () => {
    expect(buildRef(el({ id: 'submit', testId: 'x', nameAttr: 'y' }))).toBe('[id="submit"]');
  });

  it('falls back to data-testid', () => {
    expect(buildRef(el({ testId: 'login-btn', nameAttr: 'y' }))).toBe('[data-testid="login-btn"]');
  });

  it('falls back to a tag-qualified name attribute', () => {
    expect(buildRef(el({ tag: 'input', nameAttr: 'email' }))).toBe('input[name="email"]');
  });

  it('falls back to a tag-qualified aria-label', () => {
    expect(buildRef(el({ ariaLabel: 'Close dialog' }))).toBe('button[aria-label="Close dialog"]');
  });

  it('falls back to the structural path', () => {
    expect(buildRef(el({ path: 'body > div:nth-of-type(2) > a:nth-of-type(1)' }))).toBe(
      'body > div:nth-of-type(2) > a:nth-of-type(1)'
    );
  });
...
describe('pruneSnapshot', () => {
  ...
  it('frame-qualifies refs for elements inside an iframe', () => {
    ...
    const refs = pruneSnapshot(raw, 'https://x.com').elements.map((e) => e.ref);
    expect(refs).toEqual(['[id="top"]', 'iframe:nth-of-type(1) >>> [id="inner"]']);
  });

  it('dedupes per-frame so identical inner refs in different frames stay distinct', () => {
    const raw: RawSnapshot = {
      elements: [
        el({ id: null, name: 'A', path: 'button:nth-of-type(1)', frame: 'iframe:nth-of-type(1)' }),
        el({ id: null, name: 'B', path: 'button:nth-of-type(1)', frame: 'iframe:nth-of-type(2)' }),
      ],
      iframeCount: 2,
    };
    const refs = pruneSnapshot(raw, 'https://x.com').elements.map((e) => e.ref);
  ...
  it('caps the element list and flags truncation', () => {
    const elements = Array.from({ length: MAX_SNAPSHOT_ELEMENTS + 5 }, (_unused, i) =>
      el({ path: `body > button:nth-of-type(${i + 1})` })
    );
    const result = pruneSnapshot({ elements, iframeCount: 0 }, 'https://example.com');
    expect(result.elements).toHaveLength(MAX_SNAPSHOT_ELEMENTS);
    expect(result.truncated).toBe(true);
  });

  it('does not flag truncation under the cap', () => {
    const result = pruneSnapshot({ elements: [el({ id: 'a' })], iframeCount: 2 }, 'https://x.com');
  ...
  it('defaults role to the tag and name to text', () => {
    const result = pruneSnapshot(
      { elements: [el({ id: 'a', role: null, name: null, text: 'Hi' })], iframeCount: 0 },
      'https://x.com'
    );
    expect(result.elements[0]).toEqual({ ref: '[id="a"]', role: 'button', name: 'Hi' });
  });
});
