...
export function createAnalyticsRoutes({
  ...
  const summaryHandler = async (c: Context<AnalyticsRouteEnv>) => {
    ...
    try {
      ...
      log.error(
        "Analytics summary query failed for tenant {tenantId}: {error}",
        {
          tenantId,
  ...
  const byAgentHandler = async (c: Context<AnalyticsRouteEnv>) => {
    ...
    try {
      ...
      log.error(
        "Analytics by-agent query failed for tenant {tenantId}: {error}",
        {
          tenantId,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  const byInstanceHandler = async (c: Context<AnalyticsRouteEnv>) => {
    ...
    try {
      ...
      log.error(
        "Analytics by-instance query failed for tenant {tenantId}: {error}",
        {
          tenantId,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  const cacheBaselineHandler = async (c: Context<AnalyticsRouteEnv>) => {
    const tenantId = c.req.param("tenantId") ?? c.get("tenant").id;
    if (!tenantId) {
      return c.json(
        { error: { code: "bad_request", message: "Missing tenantId" } },
    ...
    try {
      ...
      log.error(
        "Analytics cache-baseline query failed for tenant {tenantId}: {error}",
        {
          tenantId,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  app.get("/cache-baseline", readGate, cacheBaselineHandler);
