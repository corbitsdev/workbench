...
describe('getToolCredential', () => {
  test('returns the injected credential', () => {
    const env = { [toolCredentialEnvKey('exa')]: { apiKey: 'k', baseURL: 'https://api' } };
    expect(getToolCredential(env, 'exa')).toEqual({ apiKey: 'k', baseURL: 'https://api' });
  });
...
describe('wire schemas', () => {
  test('ToolCredentialsRequest validates the documented shape', () => {
    const ok = ToolCredentialsRequest({ tenantId: 't', agentId: 'a', providerNames: ['exa'] });
