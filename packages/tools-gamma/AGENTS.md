...
- Gamma's REST API has no list-templates endpoint; workbench templates are a DB-backed resource (`workbench_template`, config `{ gammaId, description }`) managed over the hub REST API (`GET/POST/PUT/DELETE /api/v1/gamma-templates`, `POST .../:id/delegates`), with grant-based ownership. The agent lists them via the hub-backed `gamma_list_templates` tool (the `gammaTemplates` factory in `src/interchange-tools.ts`, executed hub-side via `HUB_BACKED_TOOLS`), which also works from workflow steps; the web uses the REST route
