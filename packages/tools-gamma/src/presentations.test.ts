  ...
  it("includes optional title when provided and omits it when absent", async () => {
    ...
    const fetcher: GammaFetch = async (_input, init) => {
      ...
      if (capturedBodies.length % 2 === 1) {
        return new Response(
          JSON.stringify({ generationId: `gen_${capturedBodies.length}` }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
    ...
    await tool.handler(
      { gammaId: "g_orig", title: "Q3 Copy" },
      new AbortController().signal,
    );
    const withTitle = JSON.parse(capturedBodies[0] ?? "{}") as Record<
      string,
      unknown
    >;
    ...
    await tool.handler({ gammaId: "g_orig" }, new AbortController().signal);
    const withoutTitle = JSON.parse(capturedBodies[2] ?? "{}") as Record<
      string,
      unknown
    >;
