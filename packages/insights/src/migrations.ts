...
// history from the platform drizzle journal.
  ...
  try {
    ...
    for (const migration of insightsMigrations) {
      const existing = await sql.unsafe(
        `SELECT 1 FROM ${quoteIdentifier(LEDGER_TABLE)} WHERE name = $1`,
        [migration.name],
      );
