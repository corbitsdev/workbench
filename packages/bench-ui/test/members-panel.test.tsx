// CL-6105: MembersPanel moved off a hand-rolled retry-less state onto
// `@corbits/api-query`'s `APIQuery` + `QueryView` — a failed member list
// load now offers the shared Retry affordance and a 401 renders as
// sign-in-required rather than a generic error.
...
import { MembersPanel } from "../src/members-panel";
...
function mount(): { container: HTMLDivElement; root: Root } {
  ...
  act(() => {
    root.render(<MembersPanel tenantId="tnt_1" />);
  });
  return { container, root };
}

describe("MembersPanel retry", () => {
