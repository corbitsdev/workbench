...
otherwise grow for the lifetime of the process that holds it.
...
package. `@corbits/chat`'s `createCryptoProviderCache` now consumes
`createExpiringMap` directly; `apps/hub/src/launch-caches.ts` is expected
to either do the same or contribute the size-capped-LRU shape here so
`BoundedCache` can retire in favor of one place for this problem.
