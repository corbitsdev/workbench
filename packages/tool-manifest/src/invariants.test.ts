...
function factory(
  overrides: Partial<ToolFactoryManifest> & Pick<ToolFactoryManifest, "factoryId">,
): ToolFactoryManifest {
  return {
...
describe("assertToolManifestFactoryInvariants", () => {
  test("rejects duplicate bare tool names across factories", () => {
    const factories = [
      factory({ factoryId: "a", bareToolNames: ["dup"], sideEffects: { dup: "read" } }),
      factory({ factoryId: "b", bareToolNames: ["dup"], sideEffects: { dup: "write" } }),
    ];
