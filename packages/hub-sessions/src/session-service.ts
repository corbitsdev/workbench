...
function resolveMountPath(row: AgentAssetWithAsset): string {
  switch (row.asset.kind) {
    ...
    default:
      // WORKBENCH-LOCAL (CL-4231): `row.asset.kind` is no longer a
      // closed enum (see `./repo-store/types.ts`), so this can no
      // longer be an exhaustiveness-checked `never` default — a
      // caller-registered custom kind reaches here too. Behavior for
      // the five named kinds above is unchanged; any other kind
      // (including a registered custom one with no default mount
      // convention) still throws loudly rather than guessing a path.
      throw new Error(
        `mount_path_required: no default mountPath for asset kind ${JSON.stringify(row.asset.kind)} on row ${row.id}`,
      );
  }
}

/**
 * WORKBENCH-LOCAL (CL-4231): the wire-boundary guard.
 *
 * The local `RepoId` (`./repo-store`) is widened to admit caller-
 * registered custom kinds; the WIRE `RepoId` (`@intx/types/sidecar`)
 * stays the closed five-kind enum, because pack-push frames are
 * validated against that closed shape on receipt (hub-agent's ws link
 * parses frames with the still-closed arktype `RepoId` validator). A
 * kind outside that closed set does NOT structurally satisfy the wire
 * shape, and there is nothing upstream of `sendAttachmentPack` that
 * filters custom kinds back out before they reach it — a custom-kind
 * asset attaches to a session exactly like a built-in one.
 *
 * Without this guard, attaching a custom-kind asset would silently send
 * a malformed frame that the receiving side drops as corrupt: no error
 * surfaces anywhere. This throws loudly and early instead, at the point
 * the widened kind is about to be forced across the still-closed wire
 * boundary, naming the offending kind so the failure is diagnosable.
 */
export function assertWireRepresentable(repoId: RepoId): void {
  const validated = WireRepoKind(repoId.kind);
  if (validated instanceof type.errors) {
    throw new Error(
      `wire_kind_unsupported: asset kind ${JSON.stringify(repoId.kind)} is not in the wire protocol's closed RepoKind set (@intx/types/sidecar) and cannot be attached to an agent session yet. The sidecar/hub-agent wire protocol has not been widened to accept caller-registered custom kinds — only the hub-side kind-handler seam has. Attaching this asset kind will silently produce a corrupt frame that hub-agent drops on receipt, so this is rejected here instead.`,
    );
  }
}
...
export function createSessionService(deps: SessionServiceDeps): SessionService {
  ...
  async function sendAttachmentPack(
    ...
    try {
      // WORKBENCH-LOCAL (CL-4231): `sidecarRouter.sendPack`'s `repoId` is
      // typed against the WIRE `RepoId` (`@intx/types/sidecar`, the closed
      // five-kind enum). The local `RepoId` this function works with
      // (`./repo-store`) is the widened kind-seam type, so it does NOT
      // structurally satisfy the wire shape in general — a caller-
      // registered custom kind reaches this cast just as easily as a
      // built-in one, and the cast fully suppresses the mismatch. Without
      // a runtime guard, an unrecognized kind would produce a frame that
      // parses as corrupt on the hub-agent side and gets silently
      // dropped: no error, no diagnostic, just a materialization that
      // never happens. `assertWireRepresentable` below is that guard —
      // it rejects a non-wire kind loudly, here, before the cast, rather
      // than letting it fail silently on the wire.
      assertWireRepresentable(repoId);
      await sidecarRouter.sendPack(agentAddress, pack, ref, sourceCommitSha, {
        mountPath,
        repoId: repoId as unknown as WireRepoId,
      });
    } catch (err) {
