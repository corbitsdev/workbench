...
import type {
  RepoKind as IntxRepoKind,
  RepoId as IntxRepoId,
  RepoAction,
} from "@intx/types/sidecar";

export type { RepoAction };

// WORKBENCH-LOCAL (CL-4231): `RepoKind` upstream is the closed
// `@intx/types/sidecar` enum (agent-state | skill | package-registry |
// workflow | workflow-run) — asset kinds are closed there by design, and
// that submodule is never edited locally. Widening happens here instead:
// `RepoKind` is the closed set of named kinds PLUS any other string, so a
// caller-registered custom kind (see `createAgentRepoStore`'s `handlers`
// option in `../agent-repo.ts`) typechecks against every signature in this
// package without touching the submodule. `(string & {})` (rather than a
// bare `string`) keeps the five named literals showing up in editor
// autocomplete instead of collapsing to a plain `string` widget.
// `RepoId` is re-derived locally against the widened kind rather than
// re-exported from `@intx/types/sidecar`, whose `RepoId.kind` stays the
// closed enum — wire-level pack frames validate against that closed shape,
// which is correct: the widening is a local-typing seam for the hub-side
// RepoStore/asset-service surface, not a wire-protocol change.
export type RepoKind = IntxRepoKind | (string & {});
export type RepoId = { kind: RepoKind; id: IntxRepoId["id"] };
