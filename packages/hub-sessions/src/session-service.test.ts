...
// WORKBENCH-LOCAL (CL-4231): the wire-boundary guard. `sendAttachmentPack`
// casts the widened local `RepoId` to the still-closed wire `RepoId`
// (`@intx/types/sidecar`) before handing it to `sidecarRouter.sendPack`.
// A caller-registered custom kind reaches that cast exactly like a
// built-in one — the cast alone does not stop it, and without a runtime
// check the resulting frame would be silently dropped as corrupt on
// receipt. `assertWireRepresentable` is the guard that must reject a
// non-wire kind loudly, here, before the cast ever runs.
describe("assertWireRepresentable (CL-4231 wire-boundary guard)", () => {
  test("passes silently for every built-in wire-representable kind", () => {
    const builtInKinds: RepoId["kind"][] = [
      "agent-state",
      "skill",
      "package-registry",
      "workflow",
    ...
    for (const kind of builtInKinds) {
      expect(() =>
        assertWireRepresentable({ kind, id: "some-id" }),
      ).not.toThrow();
    }
  });

  test("rejects a caller-registered custom kind loudly, naming the kind", () => {
    // "skill-draft" stands in for a registered custom kind (e.g. the
    // CL-4215 artifacts kind) — one that the hub-side kind-handler seam
    // accepts but that the sidecar/hub-agent wire protocol has not been
    // widened to carry.
    expect(() =>
      assertWireRepresentable({ kind: "skill-draft", id: "ast_draft_1" }),
    ).toThrow(/wire_kind_unsupported/);
    expect(() =>
      assertWireRepresentable({ kind: "skill-draft", id: "ast_draft_1" }),
    ).toThrow(/"skill-draft"/);
  });

  test("rejects any other unregistered/typo'd kind the same way", () => {
    expect(() =>
      assertWireRepresentable({ kind: "sklil", id: "ast_typo" }),
    ).toThrow(/wire_kind_unsupported/);
  });
});

// Integration-level companion: a custom-kind attachment (the shape a
// registered custom kind produces once threaded through `resolveAttachment`
// or `resolveDirectAssetAttachment`) must never reach `sidecarRouter.sendPack`
// undetected. `sendAttachmentPack` is a private closure inside
// `createSessionService` and not independently invokable, so this asserts
// the same invariant at the boundary it actually guards: the cast site
// calls `assertWireRepresentable` before `sendPack`, and the unit tests
// above prove that call throws for exactly the class of kind this PR
// opened up (a caller-registered kind outside the closed wire enum).
describe("attaching a custom kind never silently reaches the wire (CL-4231)", () => {
  test("assertWireRepresentable rejects the CL-4215 skill-draft shape end to end", () => {
    const customKindRepoId: RepoId = { kind: "skill-draft", id: "ast_1" };
    expect(() => assertWireRepresentable(customKindRepoId)).toThrow(
      /wire_kind_unsupported: asset kind "skill-draft" is not in the wire protocol's closed RepoKind set/,
    );
  });
});
