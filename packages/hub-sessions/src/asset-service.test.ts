...
// ---------------------------------------------------------------------------
// WORKBENCH-LOCAL (CL-4231): the kind seam, exercised through the full
// AssetService surface (createAsset -> populateAsset -> readAssetBlob),
// not just the underlying RepoStore. `registeredKinds` is the caller-side
// half of the seam: a kind present there passes createAsset/row-mapping,
// one absent from it still fails loudly, exactly like the five built-ins
// with the default registry.
// ---------------------------------------------------------------------------

async function createNoteRepoStore(): Promise<RepoStore> {
  if (signingKey === undefined) {
    signingKey = await generateKeyPair();
  }
  const dataDir = await makeTempDir("asset-svc-note-");
  const authorize: AuthorizeFn = () => ({ allowed: true });
  const noteHandler: KindHandler = {
    kind: "note",
    directoryPrefix: "assets/note",
    validatePush: async ({ readBlob, topLevelTreePaths }) => {
  ...
  return createRepoStore({
    dataDir,
    signingKey,
    handlers: { note: noteHandler },
    authorize,
  });
}

describe("AssetService custom kind (CL-4231)", () => {
  let dbFixture: DBStub;
  let repoStore: RepoStore;

  beforeEach(async () => {
    dbFixture = makeDB();
    repoStore = await createNoteRepoStore();
  });

  test("a registered custom kind gets real git-backed content through createAsset/populateAsset", async () => {
    const service = createAssetService({
      db: dbFixture.db,
      repoStore,
      registeredKinds: new Set(["note"]),
    });
    ...
    dbFixture.nextFindFirstAssetId(asset.id);
    const { commitSha } = await service.populateAsset({
      assetId: asset.id,
      ref: "refs/heads/main",
      tree: {
        files: { "NOTE.md": "hello from a custom kind" },
        message: "init",
      },
      principal: HUB,
    });
    ...
    dbFixture.nextFindFirstAssetId(asset.id);
    const blob = await service.readAssetBlob({
      assetId: asset.id,
      path: "NOTE.md",
    });
    expect(new TextDecoder().decode(blob)).toBe("hello from a custom kind");

    // Real git-backed content, not an identity-only row: the repo lives
    // under the handler's own directoryPrefix on disk.
    const dir = repoStore.getRepoDir({ kind: "note", id: asset.id });
    expect(dir).toContain(path.join("assets", "note"));
  });

  test("populateAsset still runs the custom kind's own validatePush", async () => {
    const service = createAssetService({
      db: dbFixture.db,
      repoStore,
      registeredKinds: new Set(["note"]),
    ...
    const err = await service
      .populateAsset({
        assetId: asset.id,
        ref: "refs/heads/main",
        tree: { files: { "NOTE.md": "" }, message: "init" },
        principal: HUB,
      })
      .catch((e: unknown) => e);
    expect(err).toBeInstanceOf(AssetServiceError);
    if (!(err instanceof AssetServiceError)) throw new Error("unreachable");
    expect(err.reason).toBe("path_violation");
    expect(err.message).toContain("NOTE.md must not be empty");
  });

  test("createAsset still rejects a kind absent from registeredKinds", async () => {
    const service = createAssetService({
      db: dbFixture.db,
      repoStore,
      // Deliberately does not register "note" here, unlike the tests
      // above -- the same handler exists on the RepoStore, but the
      // asset service's own registry is what createAsset checks.
      registeredKinds: new Set(["skill"]),
    });

    const err = await service
      .createAsset({ tenantId: "tnt_1", kind: "note", name: "x" })
      .catch((e: unknown) => e);
    expect(err).toBeInstanceOf(AssetServiceError);
    if (!(err instanceof AssetServiceError)) throw new Error("unreachable");
    expect(err.reason).toBe("unsupported_kind");
  ...
  test("omitting registeredKinds falls back to the five built-ins (unchanged default)", async () => {
    const skillStore = await createSkillRepoStore();
    const defaultService = createAssetService({
      db: dbFixture.db,
      repoStore: skillStore,
    });

    const asset = await defaultService.createAsset({
      tenantId: "tnt_1",
      kind: "skill",
      name: "greet",
    });
    ...
    const err = await defaultService
      .createAsset({ tenantId: "tnt_1", kind: "agent-state", name: "x" })
      .catch((e: unknown) => e);
    expect(err).toBeInstanceOf(AssetServiceError);
    if (!(err instanceof AssetServiceError)) throw new Error("unreachable");
    expect(err.reason).toBe("unsupported_kind");
