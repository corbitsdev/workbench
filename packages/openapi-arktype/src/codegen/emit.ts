...
import type { JSONSchema, OASPathItem, OASOperation, OASParameter } from "../parse/spec-types.js"
...
export function emitOperations(
  ...
  const operationBlocks: Array<{ name: string; code: string }> = []
  const methods = [
    "get",
    "post",
    "put",
  ...
  for (const [pathStr, pathObj] of Object.entries(paths)) {
    ...
    for (const method of methods) {
      const op = pathObj[method as keyof OASPathItem] as OASOperation | undefined
      ...
      if (respObj.content) {
        ...
        for (const [mediaType, mtObj] of Object.entries(respObj.content)) {
          if (mtObj.schema) {
            ...
            contentParts.push(`${i3}${JSON.stringify(mediaType)}: ${wrapDsl(expr)}`)
          }
        }
...
function emitParamsObject(
  ...
  for (const param of params) {
    const key = param.required ? JSON.stringify(param.name) : JSON.stringify(`${param.name}?`)
...
function schemaToExpression(
  ...
  if (ctx.seen.has(schema)) {
    ctx.diagnostics.push({
      level: "warn",
      path: schemaPath,
      message: "Circular schema reference detected in codegen, emitting type.unknown",
  ...
  ctx.seen.add(schema)
  try {
    return schemaToExpressionInner(schema, ctx, schemaPath, referencedSchemaNames)
  } finally {
...
function schemaToExpressionInner(
  ...
  if (typeIsObject && schema.properties) {
    let expr = emitObjectExpression(schema, ctx, schemaPath, referencedSchemaNames)
  ...
  if ("allOf" in schema && Array.isArray(schema.allOf)) {
    const exprs = (schema.allOf as Record<string, unknown>[]).map((s, i) =>
      schemaToExpression(s, ctx, [...schemaPath, "allOf", String(i)], referencedSchemaNames)
    )
  ...
  if ("anyOf" in schema && Array.isArray(schema.anyOf)) {
    const exprs = (schema.anyOf as Record<string, unknown>[]).map((s, i) =>
      schemaToExpression(s, ctx, [...schemaPath, "anyOf", String(i)], referencedSchemaNames)
    )
    if (exprs.length === 0) return "type.unknown"
  ...
  if ("oneOf" in schema && Array.isArray(schema.oneOf)) {
    const exprs = (schema.oneOf as Record<string, unknown>[]).map((s, i) =>
      schemaToExpression(s, ctx, [...schemaPath, "oneOf", String(i)], referencedSchemaNames)
    )
  ...
  if (Array.isArray(schema.type)) {
    ...
    const types = schemaTypes.map(
      (t) => `type("${mapPrimitiveType(t)}")`
    )
...
function emitObjectExpression(
  ...
  for (const [propName, propSchema] of propEntries) {
    ...
    const expr = schemaToExpression(propSchema, propCtx, [
      ...schemaPath,
      "properties",
      propName,
    ], referencedSchemaNames)
