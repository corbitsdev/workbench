...
function buildValidator(
  ...
  try {
    ...
    if (Array.isArray(schema.type)) {
      ...
      return validators.reduce((acc, v) => acc.or(v) as Type<unknown>);
    }
...
function buildObjectValidator(
  ...
  for (const [propName, propSchema] of Object.entries(properties)) {
    ...
    const propValidator = buildValidator(
      propSchema,
      diagnostics,
      propPath,
      seen,
...
function buildComposition(
  ...
  return validators.reduce(
    (acc, v) => (op === "and" ? acc.and(v) : acc.or(v)) as Type<unknown>,
  );
}
...
function buildPrimitiveValidator(
  ...
  const dsl =
    constraints.length > 0 ? `${baseType}${constraints.join("")}` : baseType;

  let result = type(dsl as never) as Type<unknown>;

  // Pattern
  if (typeof schema.pattern === "string") {
    result = (result as any).matching(
