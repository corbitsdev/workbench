...
export async function loadSpec(
  ...
  if (typeof raw === "object") {
    ...
    const toDeref = version.startsWith("3.0") ? upgradeSpec(raw as Record<string, unknown>) : raw as Record<string, unknown>
  ...
  if (version.startsWith("3.0")) {
    const upgraded = upgradeSpec(
      (result.specification as Record<string, unknown>) ?? (rawSpec as Record<string, unknown>)
    )
    return deref(upgraded)
  }

  return validateSpec(rawSpec as Record<string, unknown>, result.version ?? "3.1")
}

function validateSpec(raw: Record<string, unknown>, version: string): LoadedSpec {
...
function deref(input: Record<string, unknown>): LoadedSpec {
  ...
  return validateSpec(result.schema as Record<string, unknown>, result.version ?? "3.1")
}
...
function upgradeSpec(
  ...
  return (result as { specification?: Record<string, unknown> })
    .specification ?? spec
}
