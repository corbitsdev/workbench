...
describe("createClient", () => {
  ...
  it("validates enum schema", async () => {
    ...
    expect(client.schemas.Status("invalid") instanceof type.errors).toBe(true);
  });
  ...
  it("includes path-level parameters in operations", async () => {
    ...
    expect(showPet!.pathParams!({}) instanceof type.errors).toBe(true);
  });
  ...
  it("applies format validation on schemas", async () => {
    ...
    expect(client.schemas.Pet(withInvalidEmail) instanceof type.errors).toBe(
      true,
    );
  });
});
