...
const logCalls: Array<{ message: string; meta: unknown }> = [];
...
mock.module('@intx/db', () => ({
  schema: {
    ...
    agentInstance: { id: 'ai.id', principalId: 'ai.principalId', tenantId: 'ai.tenantId' },
...
function makeSessionService(overrides: Partial<SessionServiceFake> = {}): SessionServiceFake {
  return {
...
function makeContext(opts: {
  ...
  return {
    ...
    buildToolDefinitions:
      opts.buildToolDefinitions ?? ((names: string[]) => names.map((n) => ({ name: n }))),
...
function getHandler(context: ReturnType<typeof makeContext>) {
  ...
  return entry.handler as (args: Record<string, unknown>, signal: AbortSignal) => Promise<string>;
}
...
describe('DISPATCH_AGENT_DEFINITION', () => {
  test('declares the dispatch_agent contract', () => {
    ...
    expect(DISPATCH_AGENT_DEFINITION.inputSchema.required).toEqual(['agentDefinitionId', 'task']);
    expect(DISPATCH_AGENT_DEFINITION.inputSchema.properties).toHaveProperty('agentDefinitionId');
    expect(DISPATCH_AGENT_DEFINITION.inputSchema.properties).toHaveProperty('task');
  });
});
...
describe('dispatch_agent handler — argument validation', () => {
  ...
  test('rejects missing agentDefinitionId', async () => {
    const db = makeFakeDb({});
    const handler = getHandler(makeContext({ db }));
    await expect(handler({ task: 'x' }, new AbortController().signal)).rejects.toThrow(
      'agentDefinitionId is required'
    );
  });
  ...
  test('rejects non-string agentDefinitionId (coerced to empty)', async () => {
    ...
    const handler = getHandler(makeContext({ db }));
    await expect(
      handler({ agentDefinitionId: 123, task: 'x' }, new AbortController().signal)
  ...
  test('rejects missing task', async () => {
    const db = makeFakeDb({});
    const handler = getHandler(makeContext({ db }));
    await expect(handler({ agentDefinitionId: 'a' }, new AbortController().signal)).rejects.toThrow(
      'task is required'
    );
  });
  ...
  test('rejects non-string task (coerced to empty)', async () => {
    ...
    const handler = getHandler(makeContext({ db }));
    await expect(
      handler({ agentDefinitionId: 'a', task: { not: 'string' } }, new AbortController().signal)
...
describe('dispatch_agent handler — agent-row guards', () => {
  test('rejects when agent definition is not found', async () => {
    ...
    await expect(handler(VALID_ARGS, new AbortController().signal)).rejects.toThrow(
      'Agent definition not found: agent_def_1'
    );
  });
  ...
  test('rejects when agent definition is not deployed', async () => {
    ...
    const handler = getHandler(makeContext({ db }));
    await expect(handler(VALID_ARGS, new AbortController().signal)).rejects.toThrow(
      'Agent definition is not deployable (status: draft)'
    );
  });

  test('rejects when agent definition has no system prompt', async () => {
    const db = makeFakeDb({ agentRow: { ...DEPLOYED_AGENT, systemPrompt: null } });
    const handler = getHandler(makeContext({ db }));
    await expect(handler(VALID_ARGS, new AbortController().signal)).rejects.toThrow(
      'Agent definition has no system prompt'
    );
  });
...
describe('launchAgentInstance — via handler', () => {
  ...
  test('rejects when there are no resolvable inference sources', async () => {
    ...
    const db = makeFakeDb({ agentRow: DEPLOYED_AGENT, tenantRow: { domain: 'corp.test' } });
    const handler = getHandler(makeContext({ db }));
    await expect(handler(VALID_ARGS, new AbortController().signal)).rejects.toThrow(
  ...
  test('marks session ended and instance error when launch throws, then rethrows', async () => {
    ...
    const updates = db.ops.filter((op) => op.kind === 'update') as Array<{
      kind: 'update';
      set: { status?: string };
    }>;
...
describe('dispatch_agent handler — full success path', () => {
  test('persists, launches, creates collector, sends message, returns running status', async () => {
    ...
    const sessionService = makeSessionService();
    const buildToolDefinitions = mock((names: string[]) => names.map((n) => ({ name: n })));
    ...
    expect(buildToolDefinitions).toHaveBeenCalledWith(['search', 'search', 'email']);
    ...
    const launchArg = sessionService.launchSession.mock.calls[0]![0] as {
      config: { sources: Array<{ id: string }>; defaultSource: string; tools: unknown[] };
      agentAddress: string;
    };
  ...
  test('falls back to dispatcher@system when caller instance is not found', async () => {
    ...
    const sendArg = sessionService.sendUserMessage.mock.calls[0]![0] as { from: string };
...
describe('DISPATCH_HUB_TOOLS registry entry', () => {
  test('exposes the dispatch_agent definition', () => {
    expect(DISPATCH_HUB_TOOLS.dispatch_agent.definition).toBe(DISPATCH_AGENT_DEFINITION);
  });
  ...
  test('throws when orchestration context is incomplete', () => {
    ...
    expect(() => DISPATCH_HUB_TOOLS.dispatch_agent.createTools(partial)).toThrow(
