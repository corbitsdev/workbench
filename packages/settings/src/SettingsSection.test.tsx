...
const section: SettingsSectionDescriptor = {
  ...
  title: 'Profile',
  fields: [
    { key: 'displayName', label: 'Display name', kind: 'text', description: 'Shown publicly' },
    { key: 'emailNotifications', label: 'Email notifications', kind: 'toggle', disabled: true },
    {
      key: 'theme',
      label: 'Theme',
      kind: 'select',
...
describe('SettingsSection', () => {
  it('renders field descriptions when present', () => {
    render(
      React.createElement(SettingsSection, {
        section,
        values: { displayName: 'Jane', emailNotifications: false, theme: 'light' },
        onChange: () => {},
      })
    );
  ...
  it('disables a control when the field is marked disabled', () => {
    ...
    const toggle = screen.getByRole('switch', { name: 'Email notifications' }) as HTMLButtonElement;
