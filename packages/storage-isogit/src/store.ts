...
function parseMetadata(raw: unknown): MetadataData {
  ...
  if (result instanceof type.errors) {
    throw new Error(`metadata.json has unexpected structure: ${result.summary}`);
  }
  return {
...
async function readCommitLog(dir: string, limit: number): Promise<ContextCommit[]> {
...
function parseTurns(text: string): ConversationTurn[] {
  ...
  for (const raw of lines) {
    ...
    if (result instanceof type.errors) {
      throw new Error(`turns.jsonl has unexpected structure: ${result.summary}`);
    }
    turns.push(result);
  }
  return turns;
...
export class IsogitStore implements ContextStore, AuditStore {
  ...
  async commit(options: { message: string }, _signal?: AbortSignal): Promise<ContextCommit> {
  ...
  private async describeHead(expectedOid: string, message: string): Promise<ContextCommit> {
  ...
  async readAt(hash: string, _signal?: AbortSignal): Promise<ConversationTurn[]> {
  ...
  async readBlob(key: string, _signal?: AbortSignal): Promise<Uint8Array> {
    ...
    const match = entries.find((entry) => entry === safeKey || entry.startsWith(`${safeKey}.`));
    if (match === undefined) {
      throw new Error(`Blob not found for key: ${JSON.stringify(key)}`);
    }
  ...
  async writePrompt(turns: ConversationTurn[], _signal?: AbortSignal): Promise<void> {
    await fs.promises.writeFile(path.join(this.dir, PROMPT_FILE), encodeJsonlLines(turns));
  }

  async writeResponse(turn: AssistantTurn, _signal?: AbortSignal): Promise<void> {
    await fs.promises.writeFile(path.join(this.dir, RESPONSE_FILE), encodeJsonlLines([turn]));
  }

  async writeManifest(records: TransformRecordType[], _signal?: AbortSignal): Promise<void> {
    await fs.promises.writeFile(path.join(this.dir, MANIFEST_FILE), encodeJsonlLines(records));
  }

  async writeTurns(turns: ConversationTurn[], _signal?: AbortSignal): Promise<void> {
    await fs.promises.writeFile(path.join(this.dir, TURNS_FILE), encodeJsonlLines(turns));
  }
  ...
  async readManifestHistory(limit: number, _signal?: AbortSignal): Promise<TransformRecordType[]> {
    if (limit <= 0) return [];
    ...
    for (const entry of entries) {
      ...
      for (const raw of parsedLines) {
        ...
        if (result instanceof type.errors) {
          throw new Error(`Invalid manifest record at commit ${entry.oid}: ${result.summary}`);
        }
  ...
  async commitAudit(records: AuditRecordType[], _signal?: AbortSignal): Promise<void> {
    if (records.length === 0) return;
    ...
    for (const record of records) {
      ...
      const filepath = path.join(AUDIT_DIR, record.sessionId, `${safeCallId}.json`);
      const fullPath = path.join(this.dir, filepath);

      try {
        await fs.promises.access(fullPath);
        throw new Error(`Duplicate audit record: ${record.sessionId}/${record.callId}`);
      } catch (e) {
  ...
  async commitErrors(records: ErrorRecord[], _signal?: AbortSignal): Promise<void> {
    if (records.length === 0) return;
    ...
    for (const record of records) {
      ...
      const filepath = path.join(ERRORS_DIR, record.sessionId, `${seq}-${sanitizedCategory}.json`);
      const fullPath = path.join(this.dir, filepath);

      try {
        await fs.promises.access(fullPath);
        throw new Error(`Duplicate error record: ${record.sessionId}/${seq}-${sanitizedCategory}`);
      } catch (e) {
        if (e instanceof Error && 'code' in e && e.code === 'ENOENT') {
          // Expected: file does not exist yet.
        } else {
  ...
  async loadAudit(sessionId: string, _signal?: AbortSignal): Promise<AuditRecordType[]> {
