...
describe('audit store', () => {
  ...
  test('rejects corrupted audit files on load', async () => {
    ...
    const corruptPath = path.join(dir, 'state', 'audit', 'session-1', 'call-1.json');
...
describe('error store', () => {
  ...
  test('commitErrors duplicate in batch leaves no orphaned files', async () => {
    ...
    const extraPath = path.join(dir, 'state', 'errors', 'session-1', '00000002-second.json');
...
describe('load reads from the working tree', () => {
  ...
  test('load throws when turns.jsonl is malformed', async () => {
    ...
    await fs.promises.writeFile(path.join(dir, 'turns.jsonl'), 'NOT VALID JSON');
  ...
  test('load returns empty defaults on a fresh agent repo', async () => {
    const dir = await tempDir();
    const store = await createIsogitStore(dir);
    const { turns, pendingOperations, tokenUsage, connectorState } = await store.load();
    expect(turns).toEqual([]);
    expect(pendingOperations).toEqual([]);
    expect(tokenUsage).toEqual(ZERO_USAGE);
...
function makeTransformRecord(overrides: Partial<TransformRecord> = {}): TransformRecord {
  return {
    strategy: 'size-cap',
    version: '1',
...
describe('writeBlob / readBlob', () => {
  ...
  test('writeBlob rejects callIds containing path traversal', async () => {
    ...
    try {
      ...
      thrownTraversal = cause instanceof Error ? cause : new Error(String(cause));
    }
    expect(thrownTraversal?.message).toContain('unsafe characters');

    await store.writeBlob('a/b', new Uint8Array([4]));
    const read = await store.readBlob('a/b');
    expect(Array.from(read)).toEqual([4]);
    const expectedPath = path.join(dir, 'tool-output', 'a_b');
    expect(fs.existsSync(expectedPath)).toBe(true);
  });
...
describe('writeTurns / writePrompt / writeResponse / writeManifest', () => {
  test('writeTurns writes parseable JSONL to turns.jsonl', async () => {
    ...
    const raw = await fs.promises.readFile(path.join(dir, 'turns.jsonl'), 'utf-8');
  ...
  test('writePrompt writes JSONL to prompt.jsonl', async () => {
    ...
    const raw = await fs.promises.readFile(path.join(dir, 'prompt.jsonl'), 'utf-8');
    const lines = raw.split('\n').filter((l) => l.length > 0);
    expect(lines.length).toBe(2);
    expect(JSON.parse(lines[0] ?? '')).toEqual(turns[0]);
    expect(JSON.parse(lines[1] ?? '')).toEqual(turns[1]);
  ...
  test('writeResponse writes single-line JSONL with an AssistantTurn', async () => {
    ...
    const raw = await fs.promises.readFile(path.join(dir, 'response.jsonl'), 'utf-8');
    const lines = raw.split('\n').filter((l) => l.length > 0);
  ...
  test('writeManifest writes TransformRecord entries in order', async () => {
    ...
    const raw = await fs.promises.readFile(path.join(dir, 'manifest.jsonl'), 'utf-8');
    const lines = raw.split('\n').filter((l) => l.length > 0);
    expect(lines.length).toBe(2);
...
describe('commit({ message }) overload', () => {
  ...
  test('blobs written via writeBlob land in the commit tree', async () => {
    ...
    await store.writeBlob('spill-1', new TextEncoder().encode('big payload'), 'text/plain');
...
describe('readManifestHistory', () => {
  test('returns records from the most recent commits, newest first', async () => {
    ...
    expect(history.map((r) => r.strategy)).toEqual(['third-a', 'third-b', 'second', 'first']);
  });
  ...
  test('respects the limit parameter', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await store.writeManifest([makeTransformRecord({ strategy: `cycle-${String(i)}` })]);
