...
async function createPackFromRepo(sourceDir: string, oids: string[]): Promise<Uint8Array> {
...
describe('applyPack', () => {
  test('applies a packfile and updates the ref', async () => {
    ...
    await applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'test-transfer-1');
  ...
  test('retains pack and index in objects/pack after success', async () => {
    ...
    await applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'cleanup-test');

    const packPath = path.join(targetDir, '.git', 'objects', 'pack', 'pack-recv-cleanup-test.pack');
    const idxPath = path.join(targetDir, '.git', 'objects', 'pack', 'pack-recv-cleanup-test.idx');
  ...
  test('checks out files to the working tree', async () => {
    ...
    await applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'checkout-test');
  ...
  test('preserves executable mode on checked-out files', async () => {
    ...
    await applyPack(targetDir, pack, 'refs/heads/deploy', commitSha, 'exec-test');

    const stat = await fs.promises.stat(path.join(targetDir, 'deploy', 'run.sh'));
  ...
  test('removes stale files when a second deploy drops content', async () => {
    ...
    await fs.promises.writeFile(path.join(turnsDir, 'turn-1.json'), '{"id":"turn-1"}');
    await fs.promises.writeFile(path.join(responsesDir, 'response-1.json'), '{"id":"response-1"}');
    await git.add({
      fs,
      dir: sourceDir,
    ...
    await fs.promises.access(path.join(targetDir, 'state', 'responses', 'response-1.json'));
  ...
  test('removes stale top-level directories absent from new tree', async () => {
    ...
    await fs.promises.writeFile(path.join(sourceDir, 'deploy', 'prompt.md'), 'hello');
    await fs.promises.writeFile(path.join(sourceDir, 'config', 'settings.json'), '{}');
  ...
  test('cleans up pack files after failure', async () => {
    ...
    await expect(
      applyPack(targetDir, garbagePack, 'refs/heads/deploy', 'abc123', 'fail-cleanup')
    ).rejects.toThrow();

    const packPath = path.join(targetDir, '.git', 'objects', 'pack', 'pack-recv-fail-cleanup.pack');
    const idxPath = path.join(targetDir, '.git', 'objects', 'pack', 'pack-recv-fail-cleanup.idx');
...
async function makeSignedSourceRepo(keyPair: {
  ...
  const commitSha = await git.commit({
    ...
    onSign: async ({ payload }) => ({
      signature: createSSHSignature(payload, keyPair.privateKey, keyPair.publicKey),
    }),
  });
...
describe('applyPack signature verification', () => {
  test('accepts a correctly signed pack', async () => {
    ...
    await applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'signed-ok', verifier);
  ...
  test('rejects a pack signed with the wrong key', async () => {
    ...
    await expect(
      applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'wrong-key', verifier)
  ...
  test('rejects an unsigned commit when verifier is provided', async () => {
    ...
    await expect(
      applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'unsigned', verifier)
  ...
  test('skips verification when no verifier is provided', async () => {
    ...
    await applyPack(targetDir, pack, 'refs/heads/deploy', source.commitSha, 'no-verify');
...
async function makeRepoWithPaths(paths: { filepath: string; content: string }[]): Promise<{
  dir: string;
  commitSha: string;
  oids: string[];
}> {
...
describe('receivePackObjects tree validation', () => {
  test('accepts a state-only tree when validator requires state', async () => {
    const source = await makeRepoWithPaths([{ filepath: 'state/turns.jsonl', content: '' }]);
    const pack = await createPackFromRepo(source.dir, source.oids);

    const targetDir = await tempDir();
    await initAgentRepo(targetDir);

    const validator: TreeValidator = (paths) => paths.every((p) => p === 'state');
  ...
  test('surfaces top-level files to the validator alongside directories', async () => {
    ...
    const validator: TreeValidator = (paths) => {
      ...
      return paths.every(
        (p) => p === 'tarballs' || p === '.gitignore' || p === 'package-registry.json'
      );
    };
...
describe('receivePackObjects CAS', () => {
  ...
  test('returns the previous sha after a second update', async () => {
    const sourceA = await makeRepoWithPaths([{ filepath: 'state/v1.jsonl', content: 'v1' }]);
    ...
    const sourceB = await makeRepoWithPaths([{ filepath: 'state/v2.jsonl', content: 'v2' }]);
...
function publishedPackFile(targetDir: string, transferId: string): string {
  return path.join(targetDir, '.git', 'objects', 'pack', `pack-recv-${transferId}.pack`);
}

function publishedIdxFile(targetDir: string, transferId: string): string {
  return path.join(targetDir, '.git', 'objects', 'pack', `pack-recv-${transferId}.idx`);
}
...
describe('receivePackObjects unpublishes on post-publish rejection', () => {
  test('non_fast_forward removes .pack and .idx; the accepted earlier pack stays', async () => {
    ...
    expect(await fileExists(publishedPackFile(targetDir, 'rejected-nff'))).toBe(false);
    expect(await fileExists(publishedIdxFile(targetDir, 'rejected-nff'))).toBe(false);
    expect(await fileExists(publishedPackFile(targetDir, 'first-ok'))).toBe(true);
    expect(await fileExists(publishedIdxFile(targetDir, 'first-ok'))).toBe(true);
  });

  test('sha_mismatch removes .pack and .idx', async () => {
    const source = await makeRepoWithPaths([{ filepath: 'state/v1.jsonl', content: 'v1' }]);
    const pack = await createPackFromRepo(source.dir, source.oids);
    ...
    await expect(
      receivePackObjects(targetDir, pack, 'refs/heads/state', bogusExpected, 'rejected-sha', null)
    ).rejects.toThrow('sha_mismatch');

    expect(await fileExists(publishedPackFile(targetDir, 'rejected-sha'))).toBe(false);
    expect(await fileExists(publishedIdxFile(targetDir, 'rejected-sha'))).toBe(false);
  });

  test('path_violation removes .pack and .idx', async () => {
    const source = await makeRepoWithPaths([{ filepath: 'deploy/prompt.md', content: 'evil' }]);
    const pack = await createPackFromRepo(source.dir, source.oids);

    const targetDir = await tempDir();
    await initAgentRepo(targetDir);

    const stateOnly: TreeValidator = (paths) => paths.every((p) => p === 'state');
    ...
    expect(await fileExists(publishedPackFile(targetDir, 'rejected-pv'))).toBe(false);
    expect(await fileExists(publishedIdxFile(targetDir, 'rejected-pv'))).toBe(false);
  });
});
...
describe('applyPack unpublishes on post-publish rejection', () => {
  test('sha_mismatch removes .pack and .idx', async () => {
    ...
    const bogusExpected = '0'.repeat(40);
    await expect(
      applyPack(targetDir, pack, 'refs/heads/deploy', bogusExpected, 'apply-rejected-sha')
    ).rejects.toThrow('sha_mismatch');

    expect(await fileExists(publishedPackFile(targetDir, 'apply-rejected-sha'))).toBe(false);
    expect(await fileExists(publishedIdxFile(targetDir, 'apply-rejected-sha'))).toBe(false);
  });
});
