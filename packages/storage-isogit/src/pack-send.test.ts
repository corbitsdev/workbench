...
async function makeDeployRepo(): Promise<string> {
  ...
  await fs.promises.writeFile(path.join(dir, 'deploy', 'prompt.txt'), 'You are a helpful agent.');
...
describe('createDeployPack', () => {
  test('produces a packfile from a repo with a deploy tree', async () => {
    ...
    const { pack, commitSha } = await createDeployPack(sourceDir, 'refs/heads/main');
  ...
  test('throws for a nonexistent ref', async () => {
    ...
    await expect(createDeployPack(dir, 'refs/heads/nonexistent')).rejects.toThrow();
  });
});
...
async function makeLinearRepo(): Promise<{
  ...
  const c1 = await writeAndCommit(dir, [{ filepath: 'a.txt', content: 'a-v1' }], 'first');
  const c2 = await writeAndCommit(dir, [{ filepath: 'a.txt', content: 'a-v2' }], 'second');
  const c3 = await writeAndCommit(dir, [{ filepath: 'a.txt', content: 'a-v3' }], 'third');
...
async function makeForkedRepo(): Promise<{
  ...
  const base = await writeAndCommit(dir, [{ filepath: 'base.txt', content: 'base' }], 'base');

  const branchA = await writeAndCommit(dir, [{ filepath: 'a.txt', content: 'alpha' }], 'branch A');
  ...
  const branchB = await writeAndCommit(dir, [{ filepath: 'b.txt', content: 'bravo' }], 'branch B');
...
describe('createNegotiatedPack', () => {
  ...
  test('throws when wants is empty', async () => {
    ...
    await expect(createNegotiatedPack(dir, [], [])).rejects.toThrow('wants must be non-empty');
  });
