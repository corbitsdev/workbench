...
describe('createSentrySink', () => {
  it('captures an exception when the record carries an Error', () => {
    ...
    createSentrySink(client)(record({ properties: { error: err, workflowId: 'wf-1' } }));
    ...
    expect(ctx.extra).toMatchObject({ workflowId: 'wf-1', logMessage: 'something failed' });
  });

  it('captures a message when an error-level record has no Error object', () => {
    const client = makeClient();
    createSentrySink(client)(record({ rawMessage: 'no credential configured', properties: {} }));

    expect(client.captureMessage).toHaveBeenCalledTimes(1);
    expect(client.captureException).not.toHaveBeenCalled();
    expect(client.captureMessage.mock.calls[0]![0]).toBe('no credential configured');
  });

  it('captures fatal records at fatal level', () => {
    const client = makeClient();
    const err = new Error('dead');
    createSentrySink(client)(record({ level: 'fatal', properties: { error: err } }));
