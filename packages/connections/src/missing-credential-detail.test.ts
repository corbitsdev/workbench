...
import { missingCredentialDetail, parseMissingCredentialDetail } from "./missing-credential-detail";
...
describe("missingCredentialDetail / parseMissingCredentialDetail", () => {
  ...
  test("rejects a tool result's detail that isn't this shape", () => {
    ...
    expect(parseMissingCredentialDetail({ kind: "missing-credential" })).toBeUndefined();
  });
});
