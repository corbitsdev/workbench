...
// families by convention — the same signal first pass used
...
//
// `embed(ding)?` carries no required delimiter after it, unlike
...
// "does not support chat". The other short abbreviations here (`minilm`, `bge`, `gte`,
...
//
// Person-facing pickers and default selection also drop Ollama's
 ...
 * Hugging Face / GGUF path names. Returns an empty list when
 * every offering is excluded — an embedding model must never
 ...
 * behind guided state ("Ollama is connected, but no chat model
 * is installed"). */
