// The connector registry: the descriptor shape, the generic helpers a
...
// `oauth` config shape `oauth-routes.ts` reads.
export {
 ...
 * connector set get `ConnectorRegistry`'s type checking on the literal
 * without importing the type by hand. */
    ...
    oauth: {
      ...
      exchange: async ({ code, codeVerifier }) => {
        try {
          ...
          // report-error-ignore: a failed Codex exchange is surfaced to
          // the caller in the returned message, never an incident.
          return {
            ok: false,
    ...
    oauth: {
      ...
      exchange: async ({ code, codeVerifier }) => {
        try {
          ...
          // report-error-ignore: a failed xAI exchange is surfaced to
          // the caller in the returned message, never an incident.
          return {
            ok: false,
