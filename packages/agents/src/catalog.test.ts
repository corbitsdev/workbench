...
describe('templateModelRequirements', () => {
  test('derives a single requirement from the template model', () => {
    ...
    expect(templateModelRequirements(template)).toEqual([{ model: templateModelName(template) }]);
  });
...
describe('AGENT_CATALOG coverage', () => {
  ...
  test('every agent inference provider has a catalog provider entry', () => {
    ...
    for (const template of AGENT_TEMPLATES) {
      const req = template.credentialRequirements.find((r) => r.source === 'tenant');
      if (!req?.name) throw new Error(`template ${template.key} has no tenant credential`);
      expect(providers).toContain(req.name);
    }
  });

  test('every agent has an offering pairing its model and provider', () => {
    const pairs = new Set(AGENT_CATALOG.offerings.map((o) => `${o.model} ${o.provider}`));
    for (const template of AGENT_TEMPLATES) {
      const req = template.credentialRequirements.find((r) => r.source === 'tenant');
      if (!req?.name) throw new Error(`template ${template.key} has no tenant credential`);
  ...
  test('the expected provider/model matrix is present and de-duplicated', () => {
    ...
    const anthropic = AGENT_CATALOG.providers.find((p) => p.name === 'anthropic-api');
  ...
  test('each template declares exactly one tenant inference credential', () => {
    ...
    for (const template of AGENT_TEMPLATES) {
      const tenantCreds = template.credentialRequirements.filter((r) => r.source === 'tenant');
