...
function makeHarness(): Harness {
  ...
  h.transport = {
    ...
    subscribe: mock(
      (path: string, onEvent: (e: unknown) => void, opts?: { eventName?: string }) => {
        h.path = path;
...
describe('createToolNameTracker', () => {
  it('subscribes to the instance event stream with the agent.event channel', () => {
    const h = makeHarness();
    createToolNameTracker(h.transport, { tenantId: 'tnt_1', instanceId: 'ins_1' });
  ...
  it('records callId -> name from inference.tool_call.start events', () => {
    const h = makeHarness();
    const tracker = createToolNameTracker(h.transport, { tenantId: 't', instanceId: 'i' });
  ...
  it('also records names from inference.tool_call.end events', () => {
    ...
    h.emit({
      ...
      seq: 2,
      data: { callId: 'call_99_zzz9999999', name: 'web_fetch', arguments: {}, partial: {} },
    });
