...
describe('createWorkbenchDirectorRegistry', () => {
  it('resolves every Workbench director id', () => {
    ...
    for (const id of [
      ...
      expect(registry.resolve({ id, config: { allowedSenders: [] } }).id).toBe(id);
    }
  });
  ...
  it('throws for an unknown director id', () => {
    const registry = createWorkbenchDirectorRegistry();
    expect(() => registry.resolve({ id: '@workbench/agents/nope', config: {} })).toThrow();
  });
});

describe('director build() refs', () => {
  it('stamps the id and config into a sender-filter ref', () => {
    const ref = personalAgentDirector.build({ allowedSenders: ['ada@workbench.dev'] });
    expect(ref).toEqual({
  ...
  it('stamps the granola id into its ref', () => {
    expect(granolaDirector.build({ allowedSenders: [] }).id).toBe('@workbench/agents/granola');
  });
