...
describe('geralt agent definition', () => {
  ...
  it('does not declare the gamma tool credential as an inference source', () => {
    const providerNames = GERALT_CREDENTIAL_REQUIREMENTS.map((r) => r.providerName);
  ...
  it('defaultTools and requiredTools both derive from GERALT_TOOL_NAMES', () => {
    expect(GERALT_DEPLOY_DESCRIPTOR.defaultTools).toEqual([...GERALT_TOOL_NAMES]);
    expect(GERALT_DEPLOY_DESCRIPTOR.requiredTools).toEqual([...GERALT_TOOL_NAMES]);
  });
  ...
  it('has the correct label and name', () => {
    expect(GERALT_DEPLOY_DESCRIPTOR.label).toBe('Geralt — Presentation Builder');
