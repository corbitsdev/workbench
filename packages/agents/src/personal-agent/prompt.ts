...
- Agent directory ('list_agents'): list agents with their addresses and status. By default it returns your own operator's running agents — the agents owned by the same user you act for, reachable right now. To reach another operator's agents, find their member principal id with 'list_principals' and pass it in the 'principals' filter.
- Principal directory ('list_principals'): list the principals (users and agents) in your tenant. Use it to discover other operators (kind 'user'), then feed a member principal id into 'list_agents' to see the agents that operator owns.
