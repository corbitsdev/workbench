...
describe("compactEphemeralChatInput", () => {
  ...
  it("drops oldest turns and adds a summary when over threshold", () => {
    ...
    const estimated =
      estimateTextTokens(SYSTEM) + estimateTextTokens(JSON.stringify(input));
    ...
    expect(out.payload.history[0]?.content).toContain(
      "Earlier conversation compacted",
    );
    expect(out.compaction!.estimatedTokensAfter).toBeLessThan(
      out.compaction!.estimatedTokensBefore,
    );
    expect("compaction" in out.payload).toBe(false);
  });
  ...
  it("throws on invalid input", () => {
    expect(() =>
      compactEphemeralChatInput({ message: 1, history: [] }, SYSTEM),
    ).toThrow(/invalid step input/);
  });
});
