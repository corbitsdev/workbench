...
import { FIRECRAWL_CREDENTIAL_REQUIREMENTS, FIRECRAWL_DEPLOY_DESCRIPTOR } from './definition';
...
describe('firecrawl agent definition', () => {
  ...
  it('does not declare the firecrawl tool credential as an inference source', () => {
    const providerNames = FIRECRAWL_CREDENTIAL_REQUIREMENTS.map((r) => r.providerName);
