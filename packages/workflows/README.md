...
every authoring path renders/reads), `./detail` (the definition detail
read a workflow's own page uses), and `./authoring` (letting an agent
author, republish, and deploy a workflow through Interchange's native
source pipeline).
