...
// chat sidebar / workbench listing. `launchTask`'s `persistExtra` only
// ever writes to `@corbits/tasks`' own `task` table — never to
// `@corbits/chat`'s `workbench_settings` — so a tenant with both a real
// workbench and a task has exactly one row in `listWorkbenchSettings`,
...
function scratchUrlFor(e2eUrl: string): string {
  ...
  url.pathname = `/${database}_tasks_not_in_workbench_list_test`;
  return url.toString();
}
...
describeIfDb("a task never appears in the workbench listing", () => {
  ...
  test("listWorkbenchSettings sees the one real workbench, never the task's run", async () => {
    ...
    try {
      ...
      await chatStore.createWorkbenchSettings({
        tenantId: TENANT_ID,
        workbenchId: "run_workbench_host_1",
      ...
      const workbenches = await chatStore.listWorkbenchSettings(TENANT_ID);
      expect(workbenches).toHaveLength(1);
      expect(workbenches[0]?.workbenchId).toBe("run_workbench_host_1");
      expect(workbenches.some((row) => row.workbenchId === "run_task_1")).toBe(
        false,
      );
