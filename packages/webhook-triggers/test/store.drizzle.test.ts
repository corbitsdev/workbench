...
describeIfDb("createDrizzleWebhookTriggerStore: secret encryption at rest", () => {
  ...
  test("rotateSecret re-encrypts and the new secret round-trips", async () => {
    ...
    try {
      ...
      const rotated = await store.rotateSecret(TENANT_ID, created.id, "rotated-secret");
  ...
  test("a noop cipher (dev/test default) round-trips as plaintext identity", async () => {
    ...
    try {
      const db = drizzle(sql);
      const store = createDrizzleWebhookTriggerStore(db, createNoopCredentialCipher());
