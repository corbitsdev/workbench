...
describe("ResearchItem", () => {
  ...
  test("rejects item with invalid source", () => {
    expect(
      ResearchItem({ ...baseItem, source: "unknown" }) instanceof type.errors,
    ).toBe(true);
  });

  test("rejects item with invalid provenance", () => {
    expect(
      ResearchItem({ ...baseItem, provenance: "unknown" }) instanceof
        type.errors,
    ).toBe(true);
  });
...
describe("Citation", () => {
  ...
  test("rejects citation missing required fields", () => {
    expect(
      Citation({ url: "https://example.com", source: "web" }) instanceof
        type.errors,
    ).toBe(true);
  });
});
...
describe("TopComment", () => {
  test("accepts a comment with author", () => {
    const result = TopComment({
      text: "Where is the limewire link",
      author: "u/joe",
      score: 1338,
    });
    expect(result instanceof type.errors).toBe(false);
...
describe("ReportStats", () => {
  ...
  test("accepts stats without a dateRange (empty coverage window)", () => {
    expect(
      ReportStats({ sourceCount: 0, itemCount: 0 }) instanceof type.errors,
    ).toBe(false);
  });
});
