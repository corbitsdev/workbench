  ...
  turns: [
    {
      id: "t1",
      speakerId: "s1",
      text: "Tell me about your deploys.",
      startSeconds: 5,
    },
    {
      id: "t2",
      speakerId: "s2",
      text: "We spend too much time deploying.",
      startSeconds: 42,
    },
  ],
};
    ...
    const selected: SelectedPainPointContext[] = [
      {
        id: "p1",
        severity: "high",
        context: "Slow deploys",
        quote: "too much time deploying",
      },
    ];
    const props: TranscriptReviewProps = {
      transcript,
      selectedPainPoints: selected,
    };
...
describe("TranscriptReview rendering", () => {
  ...
  it("keeps a stable scroll container for long transcripts", () => {
    ...
    const { container } = render(
      React.createElement(TranscriptReview, { transcript: long }),
    );
  ...
  it("uses theme tokens rather than hardcoded colors (dark/light aware)", () => {
    const { container } = render(
      React.createElement(TranscriptReview, { transcript }),
    );
  ...
  it("calls onSelectTurn when a turn is clicked", () => {
    ...
    const button = screen
      .getByText("Tell me about your deploys.")
      .closest("button");
    button?.click();
