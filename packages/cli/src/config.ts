...
const SeedEnv = type({
  ...
  "WORKBENCH_SEED_CATALOG_TEST_WORKFLOWS?": type("string").describe(
    "set to 1 to also deploy the zero-cost catalog-test workflows (heartbeat); a dev/CI-only opt-in, never set for a real bench",
  ),
});
...
export type SeedConfig = {
   ...
   * the zero-cost catalog-test workflows (heartbeat) alongside the real
   * default set. Unset for a real bench — these exist only to exercise
   * the platform, not for a real user.
   */
