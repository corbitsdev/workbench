          ...
          {wizardStep === 'template' && (
            ...
            >
              ...
              {!gammaTemplates.isLoading && gammaTemplates.isError && (
                <div className="text-[12px] text-text-3 py-2">Could not load templates.</div>
              )}
              {!gammaTemplates.isLoading &&
                !gammaTemplates.isError &&
                (gammaTemplates.data ?? []).length === 0 && (
                  <div className="rounded-lg border border-border bg-surface-2 px-4 py-3">
                    <p className="text-[13px] text-text-2">No templates configured yet.</p>
                    {manageTemplatesHref && (
                      <a
                        href={manageTemplatesHref}
                  ...
                  <div className="grid gap-2">
                    {(gammaTemplates.data ?? []).map((tmpl) => (
                      <div
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') setSelectedTemplateId(tmpl.gammaId);
                        }}
