...
function makeProps(
  overrides: Partial<Parameters<typeof PresentationGenerationWizard>[0]> = {},
) {
  return {
    onCreated: mock(() => {}),
    onClose: mock(() => {}),
    ...
    geraltInstances: {
      isLoading: false,
      data: [{ id: "inst-1", agentName: "Geralt" }],
    },
    gammaTemplates: {
      isLoading: false,
      isError: false,
    ...
    renderRecentPicker: mock(
      ({
        ...
        onSelect: (d: PresentationSourceData) => void;
        isLoading: boolean;
      }) =>
        React.createElement(
          "button",
          {
            type: "button",
            onClick: () =>
              onSelect({ source: "paste", transcript: "recent transcript" }),
          },
          "Pick recent",
        ),
    ),
...
async function advanceToStep(targetText: string | RegExp, fn: () => void) {
  ...
  await waitFor(() =>
    expect(screen.queryByText(targetText, { exact: false })).not.toBeNull(),
  );
}
...
describe("PresentationGenerationWizard", () => {
  describe("Template step", () => {
    ...
    it("advances to brief step after selecting a template", async () => {
      ...
      fireEvent.click(screen.getByText("Sales Deck"));
      await advanceToStep("Describe the presentation goal.", () =>
        submitForm(),
      );
    ...
    it("selects a template via the Enter key", async () => {
      const props = makeProps();
      render(<PresentationGenerationWizard {...props} />);
      const salesDeckRadio = screen
        .getByText("Sales Deck")
        .closest('[role="radio"]');
      ...
      await advanceToStep("Choose the source for this presentation.", () =>
        submitForm(),
      );
  ...
  describe("Brief step", () => {
    ...
    it("shows a Back button that returns to the template step", async () => {
      await renderAtBrief();
      await advanceToStep("Choose a template", () =>
    ...
    it("always includes templateId in the brief step submission", async () => {
      ...
      const calls = (props.submitStep.mutateAsync as ReturnType<typeof mock>)
        .mock.calls;
    ...
    it("shows error message when createWorkflow fails", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.queryByText("Network error", { exact: false }),
        ).not.toBeNull(),
      );
    });
  });
  ...
  describe("Source step", () => {
    ...
    it("submits source step with paste transcript", async () => {
      ...
      fireEvent.change(
        screen.getByPlaceholderText(
          "Speaker 1: Thanks for taking the time today...",
        ),
        {
          target: {
            value:
              "Speaker 1: Here is a long enough transcript to pass validation.",
          },
        },
      );
      await act(async () => {
          ...
          renderRecentPicker: ({ onSelect }) =>
            React.createElement(
              "button",
              {
                type: "button",
                onClick: () =>
                  onSelect({ source: "granola", granolaId: "gr-9" }),
              },
              "Pick granola",
            ),
        }),
    ...
    it("surfaces an error when saving the source fails", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.queryByText("Source save boom", { exact: false }),
        ).not.toBeNull(),
      );
    });
  });
  ...
  describe("Generate step", () => {
    ...
    it("shows empty state when no instances are running", async () => {
      const props = makeProps({
        geraltInstances: { isLoading: false, data: [] },
      });
      await renderAtGenerate(props);
      expect(
        screen.queryByText("No presentation agents are running.", {
          exact: false,
        }),
      ).not.toBeNull();
    });
    ...
    it("surfaces an error when dispatching the brief fails", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.queryByText("Dispatch boom", { exact: false }),
        ).not.toBeNull(),
      );
