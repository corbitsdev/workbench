...
const PRINCIPAL_STATUSES = ['active', 'suspended', 'invited', 'deactivated'] as const;
...
export const LIST_PRINCIPALS_DEFINITION: ToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      kind: {
        type: 'string',
        description: 'Filter by principal kind: user or agent. Omit to include both.',
      },
      status: {
        type: 'string',
        description:
      ...
      limit: {
        type: 'number',
        description: 'Maximum number of principals to return (1-200, default 50).',
      },
    },
    required: [],
  },
...
export function resolvePrincipalKind(value: unknown): PrincipalKind | undefined {
...
export function resolvePrincipalStatusFilter(value: unknown): PrincipalStatus | undefined {
...
export function createPrincipalsTools(context: ListPrincipalsContext): AgentTool[] {
  return [
    {
      kind: 'string',
      ...
      handler: async (args) => {
        ...
        const conditions = [eq(intxSchema.principal.tenantId, context.tenantId)];
        if (kind !== undefined) {
