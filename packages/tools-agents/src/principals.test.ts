...
describe('LIST_PRINCIPALS_DEFINITION', () => {
  it('takes no required arguments and is registered under list_principals', () => {
    ...
    expect(AGENTS_HUB_TOOLS.list_principals.definition).toBe(LIST_PRINCIPALS_DEFINITION);
  });
});
...
describe('resolvePrincipalStatusFilter', () => {
  ...
  it('throws including all for an unknown status', () => {
    expect(() => resolvePrincipalStatusFilter('banana')).toThrow(/status must be one of:.*all/);
  });
});
...
describe('list_principals handler', () => {
  ...
  it('accepts kind and status filters', async () => {
    ...
    expect(
      JSON.parse((await handler(context)({ kind: 'agent', status: 'all' })) as string)
    ).toEqual({ principals: [] });
  });

  it('rejects an unknown kind', async () => {
    const { context } = makeContext([]);
    await expect(handler(context)({ kind: 'robot' })).rejects.toThrow(/kind must be one of/);
  });
});
