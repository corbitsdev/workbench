...
composer, and the pure helpers behind channel settings, mentions, slash
...
conversation only — the channel list lives in the host shell's own
...
- `chat-workspace.tsx` — resolves the signed-in account's channels and
  deployed agents, wires the timeline and composer for the selected channel
...
- `channel-settings/` — the per-channel settings surface (`ChannelSettingsSurface`),
  including context-window controls
- `blocks/` — typed rendering for approval, poll, and form message parts
- `api.ts` — the chat HTTP client: channels, messages, threads, reactions,
  pins, runs, and invitable agent definitions
- `default-agent-channel.ts` / `shared-channels.ts` / `direct-channel.ts` —
  pure lookup and channel-provisioning helpers
- `use-channel-stream.ts` — the live-update hook for an open channel
