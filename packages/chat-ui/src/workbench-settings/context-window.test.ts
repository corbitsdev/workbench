...
describe("contextWindowControlState", () => {
  test("an inheriting workbench renders the bench-default mode with the resolved value", () => {
    expect(contextWindowControlState({ value: 20, source: "inherit" })).toEqual(
      { mode: "inherit", displayValue: 20 },
    );
  });

  test("an overriding workbench renders the override mode with its own value", () => {
