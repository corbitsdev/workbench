...
// CL-6087: the second "+ New chat" creation froze on "Creating…" whenever a
// bare-root land into the shell (no channel id in the URL yet) preceded it.
// Root cause was a race between this component's own "pick the first
// channel" fallback (for when nothing is named in the URL) and an
// already-open New Channel dialog: both called `setActiveChannelId` /
// `onChannelChange` around the same moment, over the same URL. The fix
// (see `chat-workspace.tsx`'s auto-select effect) makes that fallback defer
// entirely while the dialog is open, since the dialog is the user
// explicitly picking a channel — there is nothing for the fallback to
// second-guess.
describe("the auto-select-first-channel fallback never races an open New Channel dialog (CL-6087)", () => {
  test("channels resolving while the dialog is already open does not steal the URL out from under it", async () => {
    stubFetch();
    const channelChanges: string[] = [];
    const harness = mount({
      tenant: { kind: "ready", tenantId: "tnt_1" },
      channelId: null,
      onChannelChange: (id: string) => channelChanges.push(id),
    });

    // Fires before `stubFetch`'s channel lists have resolved — the dialog
    // is open and channels are still loading, exactly CL-6087's race
    // window.
    act(() => {
    ...
    // `CHANNEL_WIRE` (a real channel) has resolved by now — the old,
    // unguarded fallback would have auto-picked it and reported it as the
    // active channel out from under the still-open dialog.
    expect(channelChanges).toEqual([]);
    expect(
      harness.container.ownerDocument.body.querySelector('[role="dialog"]'),
    ).not.toBeNull();
    harness.unmount();
  });

  test("closing the dialog without creating anything lets the fallback pick a channel normally", async () => {
    stubFetch();
    const channelChanges: string[] = [];
    const harness = mount({
      tenant: { kind: "ready", tenantId: "tnt_1" },
    ...
    const cancelButton = [
      ...harness.container.ownerDocument.body.querySelectorAll("button"),
    ].find((button) => button.textContent === "Cancel");
    act(() => cancelButton?.click());
    await harness.settle();

    expect(channelChanges).toEqual([CHANNEL_WIRE.id]);
    harness.unmount();
  });
});
