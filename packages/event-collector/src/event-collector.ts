...
//
// Provenance: this is a faithful copy of Interchange's
// `@intx/hub-sessions` event-collector, lifted into the workbench so the
// registry that drives it can serialize events per agent (see
// ./event-collector-registry). The collector logic itself is unchanged from
// upstream — keeping it byte-for-byte equivalent is deliberate so this whole
// package can be upstreamed back into interchange once the serialized registry
