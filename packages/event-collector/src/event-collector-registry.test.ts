...
function createFakeDb(opts: { turnInsertDelayMs?: number; throwOnPartOnce?: boolean } = {}) {
        ...
        async values(vals: Record<string, unknown>) {
          ...
          if (table === turnPart) {
            ...
            if (!turns.has(turnId)) {
              ...
              throw new Error(`turn_part violates foreign key: turn ${turnId} not present`);
            }
            parts.push({
              turnId,
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- fake DB satisfies the runtime surface the collector uses
...
function event(type: string, seq: number, data: unknown): InferenceEvent {
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- test helper: type string is the discriminant
