...
function getHandler(
  ...
  if (!tool || tool.kind !== "string")
    throw new Error("bluesky_search tool not found");
  return tool.handler;
}
...
describe("BLUESKY_HUB_TOOLS", () => {
  it("exposes bluesky_search as a credential tool with providerName bluesky", () => {
    ...
    expect(
      (BLUESKY_HUB_TOOLS.bluesky_search as { providerName: string })
        .providerName,
    ).toBe("bluesky");
  });
});
...
describe("bluesky_search authenticated path", () => {
  it("creates an AT Protocol session then sends Bearer token on searchPosts", async () => {
    ...
    const fetcher = async (
      ...
      if (url.includes("createSession")) {
        return new Response(
          JSON.stringify({ accessJwt: "tok123", did: "did:plc:test" }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
    ...
    const createSessionCall = captured.find((r) =>
      r.url.includes("createSession"),
    );
  ...
  it("retries searchPosts exactly once after a 401 by creating a fresh session", async () => {
    ...
    const fetcher = async (
      ...
      if (url.includes("createSession")) {
        return new Response(
          JSON.stringify({ accessJwt: "fresh-tok", did: "did:plc:test" }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
    ...
    const sessionCalls = captured.filter((r) =>
      r.url.includes("createSession"),
    );
...
describe("bluesky_search behavior", () => {
  it("requires a query", async () => {
    ...
    await expect(handler({}, new AbortController().signal)).rejects.toThrow(
      "query is required",
    );
  });
  ...
  it("drops posts older than the window", async () => {
    ...
    const items = JSON.parse(
      await handler({ query: "x", days: 30 }, new AbortController().signal),
    );
    expect(items).toHaveLength(1);
  ...
  it("returns an empty array when nothing falls in the window", async () => {
    const handler = getHandler(
      capturingFetcher(
        { posts: [makePost({ text: "old", createdAt: OLD, indexedAt: OLD })] },
        [],
      ),
    );
    const items = JSON.parse(
  ...
  it("surfaces the response body on a non-OK status", async () => {
    const handler = getHandler(
      capturingFetcher({ error: "RateLimitExceeded" }, [], 429),
    );
    await expect(
      handler({ query: "x" }, new AbortController().signal),
    ).rejects.toThrow(/Bluesky API error: 429/);
  });
});
