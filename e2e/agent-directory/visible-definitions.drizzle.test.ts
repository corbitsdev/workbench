...
import { listVisibleAgentDefinitions } from "../../packages/agent-directory/src/visible-definitions";
...
async function seedDefinition(
  ...
  const assetId = input.assetId === undefined ? `asset_${input.id}` : input.assetId;
  if (assetId !== null) {
...
describeIfDb("listVisibleAgentDefinitions: tenant ancestor-chain inheritance", () => {
  ...
  test("a child tenant sees a parent tenant's agent definition", async () => {
    ...
    try {
      ...
      const definitions = await listVisibleAgentDefinitions(db, "tnt_dm_sees_parent_child");
  ...
  test("a child's own definition shadows a parent's same-name definition", async () => {
    ...
    try {
      ...
      const definitions = await listVisibleAgentDefinitions(db, "tnt_dm_shadow_child");
  ...
  test("a sibling tenant never sees another sibling's definition", async () => {
    ...
    try {
      ...
      const definitions = await listVisibleAgentDefinitions(db, "tnt_dm_sibling_b");
  ...
  test("a workbench host anchor is never listed as a DM-able definition", async () => {
    ...
    try {
      ...
      const definitions = await listVisibleAgentDefinitions(db, "tnt_dm_host_guard");
  ...
  test("a seeded workflow-catalog utility is never listed as a DM-able definition", async () => {
    ...
    try {
      ...
      const definitions = await listVisibleAgentDefinitions(db, "tnt_dm_catalog_guard");
  ...
  test("a definition with no materialized asset is not launchable yet, so it isn't listed", async () => {
    ...
    try {
      ...
      const definitions = await listVisibleAgentDefinitions(db, "tnt_dm_no_asset");
